...
def cv_cnn(method, frequency, normalization, k_folds, log_tag):
    ...
    log_file = "log_cv_" + log_tag + "_" + method + "_frequency_" + str(frequency) + "_normalization_" \
        + str(normalization) + "_cnn_" + otherStyleTime + "_table7"
...
def cv_bpnn(method, frequency, normalization, k_folds, log_tag):
    import time
    startTimeStamp = time.time()
    timeArray = time.localtime(startTimeStamp)
    otherStyleTime = time.strftime("%Y-%m-%d %H:%M:%S", timeArray)
    log_file = "log_cv_" + log_tag + "_" + method + "_frequency_" + str(frequency) + "_normalization_" \
               + str(normalization) + "_bpnn_" + otherStyleTime + "_table7"
    ...
    for i in range(k_folds):
        ...
        generate_matrix_infogain(i)
        '''
        train_matrix = np.loadtxt(fname="./train-test-matrix/train_" + method + ".dat" + str(i), dtype=np.int,
                                  delimiter=",")
        # exclude label column
        dims = train_matrix.shape[1] - 1
        trainX = train_matrix[:, range(dims)]
        ...
        # print trainX[:10,:10]
        # print trainX.shape
        test_matrix = np.loadtxt(fname="./train-test-matrix/test_" + method + ".dat" + str(i), dtype=np.int,
                                 delimiter=",")
        # exclude label column
        dims = test_matrix.shape[1] - 1
        testX = test_matrix[:, range(dims)]
        ...
        model.add(Dense(50, input_dim=trainX.shape[1], init='normal', activation='relu'))
...
def cv_rnn(method, frequency, normalization, k_folds, log_tag):
    import time
    startTimeStamp = time.time()
    timeArray = time.localtime(startTimeStamp)
    otherStyleTime = time.strftime("%Y-%m-%d %H:%M:%S", timeArray)
    log_file = "log_cv_" + log_tag + "_" + method + "_frequency_" + str(frequency) + "_normalization_" \
               + str(normalization) + "_rnn-lstm_" + otherStyleTime + "_table7"
    ...
    for i in range(k_folds):
        ...
        model = Sequential()
        model.add(LSTM(output_dim=100, input_dim=trainX.shape[2], activation='sigmoid', return_sequences=True))
...
if __name__ == '__main__':
    ...
    cv_cnn(method="infogain_five_add", frequency=False, normalization=False, k_folds=5, log_tag="")
    cv_bpnn(method="infogain_five_add", frequency=False, normalization=False, k_folds=5, log_tag="")
    cv_rnn(method="infogain_five_add", frequency=False, normalization=False, k_folds=5, log_tag="")
