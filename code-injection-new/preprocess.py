...
def generate_dataset_ten_fold():
    normal_feature_vector_dir = "./all-feature-vector/normal/"
    normal_packagenames = os.listdir(normal_feature_vector_dir)
    vulnerable_feature_vector_dir = "./all-feature-vector/vulnerable/"
    vulnerable_packagenames = os.listdir(vulnerable_feature_vector_dir)
    normal_ten_percent = len(normal_packagenames) / 10
    vulnerable_ten_percent = len(vulnerable_packagenames) / 10
    normal_begin_pos = 0
    ...
    for i in range(10):
        if i == 9:
            ...
            serialize_list(train_normal, "./dataset/train_normal" + str(i))
            serialize_list(test_normal, "./dataset/test_normal" + str(i))
            serialize_list(train_vulnerable, "./dataset/train_vulnerable" + str(i))
            serialize_list(test_vulnerable, "./dataset/test_vulnerable" + str(i))
...
def feature_select(i, method, k=400):
    ...
    train_normal = get_list_from_file("./dataset/train_normal" + str(i))
    # test_normal = get_list_from_file("./dataset/test_normal"+str(i))
    train_vulnerable = get_list_from_file("./dataset/train_vulnerable" + str(i))
    ...
    if(method == "infogain"):
        ...
        serialize_list(best_features, "./selected-features/infogain" + str(i))
        np.savetxt("./selected-features/train_infogain.dat" + str(i), X=selected_data, fmt="%d", delimiter=',', newline='\n')
        return best_features, selected_data
    elif(method == "chi"):
        from TextAnalysisChi import chi
        best_feature_indexes, selected_data = chi(dataSet, k)
        best_features = [features[index] for index in best_feature_indexes]
        serialize_list(best_features, "./selected-features/chi" + str(i))
        np.savetxt("./selected-features/train_chi.dat" + str(i), X=selected_data, fmt="%d", delimiter=',', newline='\n')
        return best_features, selected_data
    elif(method == "df"):
        from TextAnalysisDF import df
        best_feature_indexes, selected_data = df(dataSet, k)
        best_features = [features[index] for index in best_feature_indexes]
        serialize_list(best_features, "./selected-features/df" + str(i))
        np.savetxt("./selected-features/train_df.dat" + str(i), X=selected_data, fmt="%d", delimiter=',', newline='\n')
        return best_features, selected_data
...
#     startTimeStamp = time.time()
    ...
    np.savetxt("./train-test-matrix/train_infogain.dat" + str(i), X=train_matrix, fmt="%d", delimiter=',', newline='\n')
    ...
    np.savetxt("./train-test-matrix/test_infogain.dat" + str(i), X=test_matrix, fmt="%d", delimiter=',', newline='\n')
...
#     startTimeStamp = time.time()
    ...
    train_matrix = np.c_[selected_data, train_added_columns]
    train_matrix = np.c_[train_matrix, trainY]
    np.savetxt("./train-test-matrix/train_chi.dat" + str(i), X=train_matrix, fmt="%d", delimiter=',', newline='\n')
    print "generate train matrix done!"
    ...
    test_matrix = np.array(test_matrix)
    np.savetxt("./train-test-matrix/test_chi.dat" + str(i), X=test_matrix, fmt="%d", delimiter=',', newline='\n')
    print "generate test matrix done!"
...
#     startTimeStamp = time.time()
    ...
    train_matrix = np.c_[selected_data, train_added_columns]
    train_matrix = np.c_[train_matrix, trainY]
    np.savetxt("./train-test-matrix/train_df.dat" + str(i), X=train_matrix, fmt="%d", delimiter=',', newline='\n')
    print "generate train matrix done!"
    ...
    test_matrix = np.array(test_matrix)
    np.savetxt("./train-test-matrix/test_df.dat" + str(i), X=test_matrix, fmt="%d", delimiter=',', newline='\n')
    print "generate test matrix done!"
...
def generate_matrix_infogain_ten():
    for i in range(10):
        generate_matrix_infogain(i)
def generate_matrix_chi_ten():
    for i in range(10):
        generate_matrix_chi(i)
        
def generate_matrix_df_ten():
    for i in range(10):
        generate_matrix_df(i)

if __name__ == '__main__':
    # generate_dataset_ten_fold()
    # generate_matrix_infogain_ten()
    #generate_matrix_chi_ten()
    generate_matrix_df_ten()
    #pass
