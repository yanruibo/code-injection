...
def cv_crnn(method, frequency, normalization, k_folds, log_tag, conv_num):
    ...
    log_file = "log_cv_" + log_tag + "_" + method + "_frequency_" + str(frequency) + "_normalization_" \
        + str(normalization) + "_crnn_" + otherStyleTime + "_table1_" + str(conv_num)
    ...
    for i in range(k_folds):
        ...
        model.add(Convolution1D(input_shape=(1, trainX.shape[2]), nb_filter=conv_num, filter_length=3, border_mode='same', activation='sigmoid', subsample_length=1))
        model.add(Lambda(max_1d, output_shape=(conv_num,)))
        
        model.add(Reshape((1, conv_num)))
        ...
        val_call = ValidationCallback(validation_data=(validationX, validationY), patience=500, filepath=weight_save_path)
...
if __name__ == '__main__':
    ...
    cv_crnn(method="infogain_five_add", frequency=False, normalization=False, k_folds=5, log_tag="",conv_num=50)
    cv_crnn(method="infogain_five_add", frequency=False, normalization=False, k_folds=5, log_tag="",conv_num=100)
    cv_crnn(method="infogain_five_add", frequency=False, normalization=False, k_folds=5, log_tag="",conv_num=150)
    cv_crnn(method="infogain_five_add", frequency=False, normalization=False, k_folds=5, log_tag="",conv_num=200)
    cv_crnn(method="infogain_five_add", frequency=False, normalization=False, k_folds=5, log_tag="",conv_num=250)
    cv_crnn(method="infogain_five_add", frequency=False, normalization=False, k_folds=5, log_tag="",conv_num=300)
    cv_crnn(method="infogain_five_add", frequency=False, normalization=False, k_folds=5, log_tag="",conv_num=350)
    cv_crnn(method="infogain_five_add", frequency=False, normalization=False, k_folds=5, log_tag="",conv_num=400)
    #predict_crnn_once(i=0, method="infogain_five_add", frequency=False, normalization=False)
    pass
