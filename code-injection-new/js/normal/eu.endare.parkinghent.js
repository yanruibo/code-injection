        ...
        //Determine language and activate i18n
        language = navigator.language.split("-");
        language_root = (language[0]);
        if(language_root != 'fr' || language_root != 'nl')
        {
            language_root = 'en';    
        }
        translator = new Translator();
        translator.translate(language_root);
...
(function() {
  ...
  ParkingListController = (function(_super) {
    __extends(ParkingListController, _super);

    function ParkingListController() {
      this.processParkings = __bind(this.processParkings, this);
      var self;

      ParkingListController.__super__.constructor.apply(this, arguments);
      self = this;
      this.pullList = new Lungo.Element.Pull("#parkinglistarticle", {
        onPull: translator.get("pullDown"),
        onRelease: translator.get("releaseToRefresh"),
        onRefresh: translator.get("refreshing"),
        callback: function() {
          return Lungo.Service.json("http://datatank.gent.be/Mobiliteitsbedrijf/Parkings11.json", "", self.processParkings, "json");
        }
      });
      Lungo.Service.json("http://datatank.gent.be/Mobiliteitsbedrijf/Parkings11.json", "", this.processParkings, "json");
    }

    ParkingListController.prototype.processParkings = function(data) {
      var parking, parkings, _fn, _fn1, _i, _j, _len, _len1;

      parkings = data.Parkings11.parkings;
      if (__Model.Parking.all().length === 0) {
        parkings = Lungo.Core.orderByProperty(parkings, 'description', 'asc');
        _fn = function(parking) {
          var parkingObject, parkingView;

          parkingObject = __Model.Parking.create({
            name: parking.name,
            description: parking.description,
            address: parking.address,
            contactInfo: parking.contactInfo,
            openingHours: parking.openingHours,
            open: parking.open,
            full: parking.full,
            suggestedRGB: parking.suggestedRGB,
          ...
          parkingView = new __View.Parking({
            model: parkingObject
          });
          return parkingView.append(parkingObject);
        };
        for (_i = 0, _len = parkings.length; _i < _len; _i++) {
          parking = parkings[_i];
          _fn(parking);
        }
        return Lungo.Router.article("parkinglist", "parkinglistarticle");
      } else {
        _fn1 = function(parking) {
          var parkingObject;

          parkingObject = __Model.Parking.findBy("name", parking.name);
          return parkingObject.updateAttributes({
            "open": parking.open,
            "full": parking.full,
            "suggestedRGB": parking.suggestedRGB,
            "availableCapacity": parking.availableCapacity
        ...
        for (_j = 0, _len1 = parkings.length; _j < _len1; _j++) {
          parking = parkings[_j];
          _fn1(parking);
        }
        return this.pullList.hide();
      }
    };
  ...
  __Controller.ParkingListController = new ParkingListController("section#parkinglist");
...
// Generated by CoffeeScript 1.6.2
(function() {
  var ParkingDetailController,
    __bind = function(fn, me){ return function(){ return fn.apply(me, arguments); }; },
    __hasProp = {}.hasOwnProperty,
    __extends = function(child, parent) { for (var key in parent) { if (__hasProp.call(parent, key)) child[key] = parent[key]; } function ctor() { this.constructor = child; } ctor.prototype = parent.prototype; child.prototype = new ctor(); child.__super__ = parent.prototype; return child; };

  ParkingDetailController = (function(_super) {
    __extends(ParkingDetailController, _super);

    ParkingDetailController.prototype.elements = {
      "span.title": "title",
      "div#openingHoursValue": "openingHours",
      "div#openValue": "open",
      "div#totalCapacityValue": "totalCapacity",
      "div#availableCapacityValue": "availableCapacity",
      "div#addressValue": "address"
    };

    function ParkingDetailController() {
      this.viewParking = __bind(this.viewParking, this);      ParkingDetailController.__super__.constructor.apply(this, arguments);
      this.routes({
        "parkingdetail/:uid": this.viewParking
      });
      Monocle.Route.listen();
    }

    ParkingDetailController.prototype.viewParking = function(params) {
      var parking, parkingCoordinates;

      parking = __Model.Parking.find(params.uid);
      this.title.html(parking.description);
      this.openingHours.text(parking.openingHours);
      this.open.text(parking.open ? translator.get('ja') : translator.get('nee'));
      this.totalCapacity.text(parking.totalCapacity);
      this.availableCapacity.text(parking.availableCapacity);
      this.address.html(parking.address);
      Lungo.Router.section("parkingdetail");
      parkingCoordinates = {
        latitude: parking.latitude,
        longitude: parking.longitude
      };
      ...
      Lungo.Sugar.GMap.addMarker(parkingCoordinates, null, false);
      return $$('#map').tap(function(event) {
        var url;

        url = "geo:" + parking.latitude + "," + parking.longitude + "?q=" + parking.latitude + "," + parking.longitude + "(Parking+" + parking.description + ")";
        console.log(url);
  ...
  __Controller.ParkingDetail = new ParkingDetailController("section#parkingdetail");
...
(function() {
  ...
  __Model.Parking = (function(_super) {
    __extends(Parking, _super);

    function Parking() {
      _ref = Parking.__super__.constructor.apply(this, arguments);
      return _ref;
    }

    Parking.fields("name", "description", "address", "contactInfo", "openingHours", "open", "full", "suggestedRGB", "totalCapacity", "availableCapacity", "latitude", "longitude", "covered");
...
(function() {
  ...
  __View.Parking = (function(_super) {
    __extends(Parking, _super);

    Parking.prototype.container = "section#parkinglist article#parkinglistarticle ul";

    Parking.prototype.template = "<li class=\"selectable\">\n    <div class=\"right capacitybox\"><span style=\"color: {{suggestedRGB}}\">\n        {{#full}}\n            VOL\n        {{/full}}\n        {{^full}}\n            {{availableCapacity}}\n        {{/full}}\n    </span></div>\n    <strong>{{description}}</strong>\n</li>";

    Parking.prototype.events = {
      "tap li": "onTap"
    };
    ...
    function Parking() {
      ...
      Parking.__super__.constructor.apply(this, arguments);
      self = this;
      __Model.Parking.bind("change", function(p) {
        return self.refresh();
      });
    }

    Parking.prototype.onTap = function(event) {
      return this.url("parkingdetail", this.model.uid);
    };
