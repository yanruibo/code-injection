    		...
    		$(document).ready(function() {
                $("#shareButtonLink").click(function() {
                    window.plugins.share.show({
                        subject: "Share The Ant and the Grasshopper Fan",
                        text: "https://play.google.com/store/apps/details?id=com.a1824500890528f4cb6d55772a.a70086859a"},
  ...
  return string;
        ...
        $(document).ready(function() {
                          $('#youtubevideos').youTubeChannel({ 
                                                             userName: 'The Ant and the Grasshopper Fan', 
                                                             channel: "uploads", 
                                                             hideAuthor: true,
                                                             numberToDisplay: 30,
...
function onRateReminderConfirm(buttonIndex) {
    ...
    switch(btn) {
        case 1:
            //open page
            setAllDone = true;window.plugins.childBrowser.openExternal("market://details?id=com.a1824500890528f4cb6d55772a.a70086859a");;
            break;
        case 2:
            //grab next reminder
  ...
  return string;
                      ...
                      $(document).ready(function() {
                          $("#submit").click(function() {
                              ...
                              var extras = {};
                              extras[WebIntent.EXTRA_SUBJECT] = "The Ant and the Grasshopper Fan App Contact Form";
                              extras[WebIntent.EXTRA_TEXT] = messageBody;
                              extras[WebIntent.EXTRA_EMAIL] = "ferrernestor43@gmail.com";
  ...
  return string;
                ...
                function  rssReader() {
                   // alert("rssReader");
                    ...
                    //Title of the blog
                   //RSS url
                    var RSS = "http://api.flickr.com/services/feeds/photos_public.gne?tags=The+Ant+and+the+Grasshopper&format=rss2";
                    //Stores entries
                    var entries = [];
                    var selectedEntry = "";
                    //alert("1");
...
function initializeData(){	
	...
	if (localStorage.getItem("disclaimer") == null || localStorage.getItem("disclaimer") != 1){
		//alert(disclaimerMessage + ", The Ant and the Grasshopper Fan");
				
		document.addEventListener("deviceready", function(){
			navigator.notification.alert(disclaimerMessage, false, "The Ant and the Grasshopper Fan", "OK");
...
function saveLocation(){
	...
	if(localStorage.getItem("acc") != null)
		navigator.notification.confirm(confirmOverwrite, onConfirmSave, "The Ant and the Grasshopper Fan", "OK,Cancel");
	else
		onConfirmSave(1);
...
function checkAccuracy(start){
	...
	if (geoSuccess == false){
		navigator.notification.alert(noGPSMessage, false, "The Ant and the Grasshopper Fan", "OK");
		...
		navigator.notification.alert(notifyPositionSaved, false, "The Ant and the Grasshopper Fan", "OK");
	}
		...
		if (!allowReducedAcc && !gettingInput){
			...
			navigator.notification.confirm(allowReducedAccMessage, function(button){
				...
				if (allowReducedAcc){
					...
					gpsLoading.style.visibility = "hidden";
					//alert(errorPoorAcc);
					navigator.notification.alert(errorPoorAcc, false, "The Ant and the Grasshopper Fan", "OK");
				}
				gettingInput = false;
			}, "The Ant and the Grasshopper Fan", "OK,Cancel");
		}
...
function loadCanvas()
{
	...
	if(canvas.getContext("2d"))
	{
  	...
  	{
  		navigator.notification.alert(errorUnsupportedFeature, false, "The Ant and the Grasshopper Fan", "OK");
  	}
   ...
   return tmp.textContent||tmp.innerText;
                ...
                function  rssReader() {
                   // alert("rssReader");
                    ...
                    //RSS url
                    var RSS = "http://news.google.com/news?q=The+Ant+and+the+Grasshopper+Fan&output=rss&num=100";
