...
function buybook(bookId) {
    ...
    jQuery.mobile.loading( 'show', {
        text: 'Connecting to Store, Please Wait!',
        textVisible: true
    });
    if (deviceType == 'iPad' || deviceType == 'iPhone') {
        if (actualDownload != '') {
            jQuery.mobile.loading('hide' );
            alert('Already downloading, please wait!');
            jQuery.mobile.loading( 'show', {
                text: 'Scarico '+ title,
                textVisible: true
            });
...
function startDownload(label) {
	...
	if (label == undefined)
		label = "Downloading video tutorial, please wait...";
	if (downloaddiv == null) {
...
var BASEDIR = 'Android/data/it.hachettefascicoli.iStitches/',
ROOT='',
SECTIONDIR,
isAjax = false;
...
function bookExist(book) {
    ...
    jQuery.ajax({
        ...
        async: false,
        success: function(response) {
            liElem = '<li><a href="#" rel="external" class="external" onclick="moveTo(\'book.html?bookname='+jQuery(book).attr('name')+'\');"><span class="wrap-img"><img src="'+filename+'" alt="'+jQuery(book).attr('titolo')+'" /></span>'+jQuery(book).attr('titolo')+' <span class="button">read</span></a></li>';
        ...
        error: function(x, t, m) {
            ...
            var failCall = function() {
            	if ( jQuery(book).attr('prezzo') == '')  	
            		liElem = '<li id="buy_product_'+jQuery(book).attr('code')+'"><a href="javascript:void(0);" rel="external" onclick="buybook(\''+ jQuery(book).attr('code') +'\');"><span class="wrap-img"><img src="'+filename+'" alt="'+jQuery(book).attr('titolo')+'" /> </span>'+jQuery(book).attr('titolo')+'<span class="button">downloading</span></a></li>';               
            	else
                	liElem = '<li id="buy_product_'+jQuery(book).attr('code')+'"><a href="javascript:void(0);" rel="external" onclick="buybook(\''+ jQuery(book).attr('code') +'\');"><span class="wrap-img"><img src="'+filename+'" alt="'+jQuery(book).attr('titolo')+'" /> </span>'+jQuery(book).attr('titolo')+'<span class="button">purchase '+ jQuery(book).attr('prezzo') +'&euro;</span></a></li>';
                jQuery('#book_list').append(liElem);
                jQuery('#bookpop_list').append(liElem);
                jQuery('#bookpop_list').listview('refresh');
                jQuery('#book_list').listview('refresh');
