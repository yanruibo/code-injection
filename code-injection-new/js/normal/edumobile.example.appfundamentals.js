...
function visible (vis,hid){ document.getElementById('hebr').style.width="235px"; document.getElementById(vis).style.display="block"; document.getElementById(vis).style.visibility="visible"; document.getElementById(hid).style.display="none"; document.getElementById(hid).style.visibility="hidden"; } 
...
function hy(){
    iframes.getElementById("hy").innerHTML = " ";
    iframes.getElementById("hy").outerHTML = "<input id=\"hy\" style=\"width: 49px\" name=\"YearH\" />";
    iframes.getElementById("chy").innerHTML = "<a href=\"javascript:location.reload(true)\">תצוגה קודמת</a>";
	    alert("הזן את השנים בספרות, דוגמא: 5770");}
...
nDateH1 = nDateH
	 if ( iframes.form.chatzot.checked == true )
    {
        nDateH1 = nDateH1 - 1;
    }
 
    var strRes = (FormatDateH(nMonthH + "/" + number2hebrew(nDateH) + "/" + number2hebrew(nYearH)) + "<BR/>" + FormatDate(HebToGreg(nYearH, nMonthH, nDateH1)));
    iframes.getElementById("result").innerHTML = strRes;
    ("<p>")
  }
  function G2H(nYearG, nMonthG, nDateG) {
    ...
    dGreg   = new Date(nYearG, nMonthG - 1, nDateG)
     if ( iframes.form.shkia.checked == true )
    {
        nDateG = nDateG + 1;
    }
    ...
    iframes.getElementById("result1").innerHTML = FormatDate(dGreg) + " התאריך הלועזי הינו: " + FormatDateH(GregToHeb(dGreg1));
   ("<p>")
  }
var XMLHttpRequestObject = false; 
if(window.XMLHttpRequest){ XMLHttpRequestObject = new XMLHttpRequest();  }
else if(window.ActiveXObject){XMLHttpRequestObjec = new ActiveXObject("Microsoft.XMLHTTP");  }
if(XMLHttpRequestObject) { 
XMLHttpRequestObject.open("GET", '/date.txt'); 
XMLHttpRequestObject.onreadystatechange = function() { 
if (XMLHttpRequestObject.readyState == 4 && XMLHttpRequestObject.status == 200) { 
document.getElementById("taarich").innerHTML=XMLHttpRequestObject.responseText;}}
XMLHttpRequestObject.send(null);}
var iframes = document;addOption('DateG1',DateG);addOption('MonthG1',MonthG);addOption('DateH1',DateH);
 var year = new Date();
 form.YearG1.value = year.getFullYear();
function changename(){
iframes.getElementById("hy").name = "YearH1";
}
