...
taz.settings = {
    ...
    masterServerUrl : 'http://goteammaster.taz.fi:8080/TAZGoTeamMasterServer/',
//    serverUrl : 'http://dev.taz.fi:8080/TAZGoTeamServer/',
...
taz.strings_ru = {
	...
	warning : "Внимание! Учитывайте состояние дорожного покрытия и интенсивность движения. Соблюдайте меры предосторожности, чтобы избежать столкновений с различными объектами, другими людьми или животными, когда используете Go Team.",
...
taz.strings_sv = {
	...
	// warningDialog
	// WarningDialog, shown when the application starts
	warning : "Varning! Var uppmärksam på omgivningen och trafiken runt omkring dig. Var försiktig så att du inte springer in i saker, andra människor eller djur när du använder Go Team.",
	// Option to turn the warning dialog off, so it won't appear on future startups
...
taz.strings_nb = {
	...
	// warningDialog
	// WarningDialog, shown when the application starts
	warning : "Forsiktig! Følg med på bakkeforholdene og trafikken. Pass på at du ikke kolliderer med ting, folk eller dyr mens du har det gøy med Go Team.",
	// Option to turn the warning dialog off, so it won't appear on future startups
...
taz.strings_da = {
	...
	// warningDialog
	// WarningDialog, shown when the application starts
	warning : "Advarsel! Vær opmærksom på vejforhold og trafikken. Løb ikke ind i ting, andre mennesker eller dyr, mens du bruger Go Team.",
	// Option to turn the warning dialog off, so it won't appear on future startups
...
taz.strings_de = {
	...
	// warningDialog
	// WarningDialog, shown when the application starts
	warning : "Vorsicht! Achten Sie auf die Bodenbedingungen und den Verkehr. Achten Sie darauf, bei Ihrem Go Team-Vergnügen keine Menschen, Tiere oder Objekte anzufahren.",
	// Option to turn the warning dialog off, so it won't appear on future startups
...
taz.strings_fr = {
	...
	// warningDialog
	// WarningDialog, shown when the application starts
	warning : "Attention au monde qui vous entoure et aux véhicules ! Évitez les collisions avec des objets, des animaux ou d'autres personnes quand vous jouez sur Go Team.",
	// Option to turn the warning dialog off, so it won't appear on future startups
...
taz.strings_ar = {
	...
	// warningDialog
	// WarningDialog, shown when the application starts
	warning : "تنبيه! كن على علم بأحوال الطريق وحركة المرور. كن على حذر من ألا ترتطم بأشياء أو أشخاص آخرين أو حيوانات بينما تستمتع بتطبيق Go Team.",
	// Option to turn the warning dialog off, so it won't appear on future startups
...
taz.strings_en = {
	// warningDialog
	// WarningDialog, shown when the application starts
	warning : "Caution! Be aware of the ground conditions and traffic. Be careful not to run into objects, other people or animals while you have fun with Go Team.",
	// Option to turn the warning dialog off, so it won't appear on future startups
...
taz.strings_zh = {
	...
	// warningDialog
	// WarningDialog, shown when the application starts
	warning : "小心！请注意道路和交通状况。请小心，不要在玩 Go Team 时撞到物体、其他人或动物。",
	// Option to turn the warning dialog off, so it won't appear on future startups
	doNotShowAgain : "不再显示。",
...
taz.strings_fi = {
	...
	// WarningDialog, shown when the application starts
	warning : "Ole varovainen pelatessasi! Ota huomioon maasto ja liikenne. Pid&auml; huolta ettet t&ouml;rm&auml;&auml; rakennuksiin, ihmisiin tai el&auml;imiin pit&auml;ess&auml;si hauskaa Go Teamin kanssa.",
	// Option to turn the warning dialog off, so it won't appear on future startups
