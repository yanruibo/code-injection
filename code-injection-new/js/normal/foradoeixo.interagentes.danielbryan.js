...
$.post(SERVER+'insert_image.php', {image: img, titulo : vtitulo, comentario : vcomentario, app : APP}, function(data) {
      $("#content").html("<h3>Obrigado!</h3><p>Imagem enviada com sucesso.</p>");    
      $('#loading').hide();
});
}     
...
$(document).ready(function() {
  ...
  $.ajax({
    ...
    success: function(data) {
      $.each(data.result, function(key, value) {
        var news = "<li><div class='picContainer'><div><a href='"+SERVERIMG+"uploads/"+value.id+".jpg'><img src='"+SERVERIMG+"uploads/"+value.id+".jpg' alt='"+value.text+"'></a></div></div></li>";
        $('#Gallery').append(news);
      });    
...
$(document).ready(function() {
  ...
  var twitter_user = 'foradoeixo';
  $.ajaxSetup({ cache: false });
  $('#loading').show();
  $.ajax({  
    url: twitter_api_url + '?callback=?&rpp=20&q=from:' + twitter_user,  
    dataType: 'json',    
    async: false,  
    success: function(data){     
      $.each(data.results, function(i, tweet) {
        if(tweet.text !== undefined) {
          ...
          var news = "<li class='clearfix tweet_first tweet_odd'>";
          news += "<a href='http://twitter.com/foradoeixo' class='tweet_avatar'><span class='picContainer picInline left'><span><img width='48' height='48' border='0' src='"+tweet.profile_image_url+"'></span></span></a>";
          news += "<span class='tweet_time'>@foradoeixo</span>"; 
          news += "<span class='tweet_join'>"+hours+" horas atrás</span>"; 
          news += "<span class='tweet_join'>"+tweet.text+"</span>";
          news += "</li>";   
