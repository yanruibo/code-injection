...
$(document).ready(function() 
{
    ...
    $('a.winopen').bind('click', function(ev){
        ev.preventDefault();
        var ref = window.open($(this).attr('href'), '_blank', 'location=no');
    });
                  
    $('.winopenMap').bind('click', function(ev){
        ev.preventDefault();
        var ref = window.open("https://maps.google.fr/maps/ms?msa=0&msid=206724290379991799987.0004cb1293da1b6557cbe&ie=UTF8&ll=43.550166,7.024555&spn=0.058537,0.10952&t=h&z=14&vpsrc=6&iwloc=0004de50cd28fa9984870",'_blank', 'location=no');
    });
	...
	// open close
	function toggleEventOpen (eventdiv, doOpen) {
		
		var isOpening = doOpen;
		
		eventdiv.find("a").bind("transitionend webkitTransitionEnd oTransitionEnd MSTransitionEnd", 
			function(){
		...
		if(doOpen){
			eventdiv.removeClass("fold");
			eventdiv.addClass("open");
			refreshScroll();
		} else {
			if( !eventdiv.hasClass("unfold") ) eventdiv.addClass("fold");
			eventdiv.removeClass("open");
			refreshScroll();
		}
	}

	// toggle buttons
	$("div.datebar a[alt='toggle']").click( function () {
		var eventdiv = $(this).parents("div.event");
		if( eventdiv.hasClass("open") ) {
			toggleEventOpen(eventdiv, false);
		} else {
			toggleEventOpen(eventdiv, true);
		}
		// refreshScroll();
		event.preventDefault();
	})

	// open / close: tap on title
	...
	var bgMgr = {
		...
		currentBg:null,
		nextBg:null,
		bgList:[$("#cover17"), $("#cover18"), $("#cover19"), $("#cover20"), $("#cover21"),$("#cover22")],
		bluredList:[],
		currentImg:null,
		ypos:0,
		faded:false,
		...
		reset: function(arg) {
			...
			bgMgr.startX = mainScroll.x; 
			bgMgr.currentNum = Math.abs( Math.round(mainScroll.x / bgMgr.pageWidth) );
			bgMgr.currentBg = bgMgr.bgList[bgMgr.currentNum];
			
			bgMgr.currentImg = bgMgr.currentBg.find("img.sharp");
			bgMgr.currentImg.css("opacity", Math.max(0.3, Math.abs(75/mainScroll.y)) );
			// bgMgr.currentImg.css("opacity", Math.max(0.15, Math.abs(25/mainScroll.y)) );
			for (var i = bgMgr.bgList.length - 1; i >= 0; i--) {
				bgMgr.bgList[i].css("opacity", 0);
			};
			//console.log("bgMgr.currentNum", bgMgr.currentNum);
			//console.log("bgMgr.currentNum", mainScroll.x);
			//waterbug.log("bgMgr.currentNum", mainScroll.x);

			bgMgr.currentBg.css("opacity", 1);
			bgMgr.faded = false;


		},

		update: function(iScrollObj) {
			
			bgMgr.currentImg = bgMgr.currentImg || bgMgr.currentBg.find("img.sharp");
			
			// Scroll X
			if( bgMgr.faded || Math.abs(iScrollObj.x - bgMgr.startX) > 100)
			{
				bgMgr.faded = true;
				bgMgr.currentPos = Math.abs( iScrollObj.x / bgMgr.pageWidth );

				// Define bgs
				if(bgMgr.currentNum !== Math.floor(bgMgr.currentPos))
				{
					bgMgr.currentNum = Math.floor(bgMgr.currentPos);
					bgMgr.currentBg = bgMgr.bgList[bgMgr.currentNum];
				}

				if(bgMgr.nextNum !== Math.ceil(bgMgr.currentPos))
				{
					bgMgr.nextNum = Math.ceil(bgMgr.currentPos);
					if(bgMgr.nextNum >= bgMgr.bgList.length) {
						bgMgr.nextNum = bgMgr.bgList.length-1;
						return;
					}
					
					bgMgr.nextBg = bgMgr.bgList[bgMgr.nextNum];
				}


				bgMgr.currentBg.css("opacity", Math.pow((bgMgr.nextNum-bgMgr.currentPos), 2) );
				bgMgr.nextBg.css("opacity", Math.pow((bgMgr.currentPos-bgMgr.currentNum), 2) || 1 );
			

			// Scroll Y
			} else if( !bgMgr.blured && iScrollObj.y < -100) {
				bgMgr.blured = true;
				// bgMgr.currentImg.css("opacity",0.3);				
				bgMgr.currentImg.fadeTo('fast', 0.5);				
			} else if(bgMgr.blured && iScrollObj.y > -100) {
				bgMgr.blured = false;
				bgMgr.currentImg.fadeTo('fast', 1);
			}
	...
	function onLoadMobile() {
		window.removeEventListener('load', onLoadMobile, false);
		
		mainScroll = new TWIS('#wrapper', 
							{ 	x:1,
								y:0,
								onScrollStart:bgMgr.reset,
								onScrollEnd:bgMgr.reset,
								onUpdatePos:bgMgr.update
	...
	function changeDay (event) {
		...
		var startDay = $(event.target).parents("div.day").attr("id");
		var destDay = parseInt(startDay) - 17 + event.data.dir
		
		mainScroll.scrollToPage(destDay, 500);
		event.preventDefault();
	}
	...
	if($(window).height() < $(window).width() && $(window).height()<640 ){
 		viewWidth = 320;	   
	}

	// TODO calcule le jour
	var today = 0;
	
	// need help lists
	$("a.help").on('click', function (event) {

		var clickLocalY = event.pageY - $(this).offset().top;
		console.log("clickLocalY", clickLocalY);

		if(!$(this).hasClass("unfold")){
			$("a.help").removeClass("unfold");
			$("div.day ul").removeClass("unfold");
		}
		$(this).toggleClass("unfold");
		$( "#"+this.rel ).toggleClass("unfold");

		console.log("event", event);
		console.log(($(this).offset().top - event.pageY))

		mainScroll.scrollY( mainScroll.y -($(this).offset().top - event.pageY) - clickLocalY );
	...
	$("div.day li span").on('click', function () {
		$(this).parents("li").toggleClass("unfold");
		refreshScroll();
	});
	...
	// {
		...
		// change day nav by click
		$(".datenav a.right").on('click', { dir: 1 }, changeDay );
		$(".datenav a.left").on('click', { dir: -1 }, changeDay );

		// go to need help
		$("header a.on").on('click', function () {
				// $("#help").toggleClass("open");
				// helpScroll = new iScroll('#wrapperhelp');
				mainScroll.scrollToPage(5, 500);
				bgMgr.backNum = bgMgr.currentNum < 5 ? bgMgr.currentNum : today;

			});
		$("header a.off").on('touchend click', function () {
				// $("#help").removeClass("open");
				mainScroll.scrollToPage(bgMgr.backNum, 500);
			});


		$("div.day").css("width", viewWidth);
		$("#slider").css("width", viewWidth*6);
		$("#orient").css("height", viewWidth);
		// 
		bgMgr.pageWidth =  $("#slider").width() / $("#slider").children().length;
		bgMgr.currentNum = bgMgr.backNum = 0;
		bgMgr.currentBg = bgMgr.bgList[bgMgr.currentNum];
		bgMgr.currentBg.css("opacity","1");

		document.addEventListener('touchmove', function (e) { e.preventDefault(); }, false);
		window.addEventListener('load', onLoadMobile, false);
		window.scrollTo( 0, 1);
	...
	// 		window.removeEventListener('load', onLoadDesktop, false);
	// 		// $("#splash").fadeOut( 500);
	// 	}
	// 	window.addEventListener('load', onLoadDesktop, false);		
	// }
