...
/* Funziona di caricamento dei dati di configuraionze tramite file JSON */
function caricaDati() {
	...
	$.ajax({
		...
		success: function(dati){
			NCCPhone=dati.NCCPhone;
			NCCEmail=dati.NCCEmail;
			latlng = new google.maps.LatLng(dati.NCCMapLat,dati.NCCMapLon);	
		    myOptions = {
					center: latlng,
					zoom:10,
					mapTypeId: google.maps.MapTypeId.ROADMAP
				} ;	
			$("#NCCTitle").html(dati.NCCTitle);		
			$("#spanInfoText").html(dati.NCCInfo);		
			NCCTariffe = dati.Tariffe;												
			caricaLocalitaPredefinite(dati);					
			caricaSpecialTransfer(dati);	
								 
		},
        error:  function (dati){
            console.log(dati);            	
            console.log('Errore Ajax: ./dati/dati.json-> ' + dati['responseText']);         
        }
...
function caricaLocalitaPredefinite(dati) {
	strHtmlDA="";
	strHtmlA="";
	for(i=0;i<dati.LocalitaPredefinite.length;i++) {

		strHtml='<li class="arrow"><a class="slide" href="#item2" id="predefinitaDa'+i+'" ';
		strHtml+='onClick="setLocalita (';
		strHtmlDA +=strHtml+'\'DA\'';
		strHtmlA +=strHtml+'\'A\'';
		strHtml = ',\''+dati.LocalitaPredefinite[i].localita+'\');"><span class="itemLocalita" >'+dati.LocalitaPredefinite[i].deslocalita+'</span> </a></li>';		
		strHtmlDA+=strHtml;
		strHtmlA+=strHtml;
	}

	$("#PredefiniteDaList").html(strHtmlDA);
	$("#PredefiniteAList").html(strHtmlA);	
}

function caricaSpecialTransfer(dati) {
	strHtml="";

	for(i=0;i<dati.SpecialTransfer.length;i++) {

		strHtml+='<li class="arrow"><a class="slide" href="#item4b" id="specialTransfer'+i+'" ';
		strHtml+='onClick="setSpecialTransfer ('+dati.SpecialTransfer[i].prezzo+',\''+dati.SpecialTransfer[i].locda+'\' , \''+dati.SpecialTransfer[i].loca+'\' ); " ><span class="itemLocalita" >'+dati.SpecialTransfer[i].deslocda+' - '+dati.SpecialTransfer[i].desloca+' ('+dati.SpecialTransfer[i].tipo+') '+dati.SpecialTransfer[i].prezzo+'E</span> </a></li>';		
		
	}

	$("#specialTransferList").html(strHtml);
}
...
function calcolaPreventivo ( astr_tipoPreventivo , an_km )
{
	...
	if ( astr_tipoPreventivo == "V" )
	{
		...
		*/
		...
		lbTrovato = false
		for (i=0;i<NCCTariffe[strVeicolo].fascia.length;i++) {
			daKM=NCCTariffe[strVeicolo].fascia[i].da;
			aKM=NCCTariffe[strVeicolo].fascia[i].a;
			tariffaKM=NCCTariffe[strVeicolo].fascia[i].tariffa;
			if ( an_km >= daKM && an_km <= aKM  ) {
				nPreventivo = tariffaKM ;		
				lbTrovato=true;	
				break;
			}			
		}
		/* Se non e' stata trovata una fascia allora moltiplica per il costo a KM */
		if (!lbTrovato) {
			nPreventivo = ( an_km * 2 ) * NCCTariffe[strVeicolo].costoKM ;	
		}
...
function sendEmail ()
{
	...
	//window.plugins.emailComposer.showEmailComposer ( 'Preventivo/prenotazione', strBody , NCCEmail , '' ,'', true ) ;
	//document.location.href = "mailto:" + NCCEmail + "?subject=Preventivo prenotazione" + "&body=" + strBody ;
