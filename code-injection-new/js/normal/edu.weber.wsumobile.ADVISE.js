...
CUSTOM.selectedAdvisor = "";

if (window.localStorage.getItem("edu_weber_wsumobile_Advisors") == null) {
    window.localStorage.setItem("edu_weber_wsumobile_Advisors", "<advisors><advisor recordnumber='0'><program><![CDATA[College Advisor]]></program><college><![CDATA[for general education advisement]]></college></advisor></advisors>");
}

CUSTOM.showMyAdvisors = function (pageID, containerID, unavailableID) {

    try {
        var xList = API.parseXML(window.localStorage.getItem("edu_weber_wsumobile_Advisors"));
        var advisors = xList.getElementsByTagName("advisor");

        if (API.debug == true) {
            console.log("Processing " + advisors.length + " data items");
        }
        ...
        li.setAttribute("data-icon", "false");
        var link = document.createElement("a");
        link.appendChild(document.createTextNode("Instructions"));
        $(link).bind("click", function () {
        ...
        for (var i = 0; i < advisors.length; i++) {
            ...
            var headingText = $("program", advisors[i])[0].firstChild.nodeValue;
            ...
            li.setAttribute("data-icon", "false");
            var link = document.createElement("a");
            link.setAttribute("recordnumber", advisors[i].getAttribute("recordnumber"));
            link.appendChild(document.createTextNode($("program", advisors[i])[0].firstChild.nodeValue));
            if (i == 0) {
                var college = $("college", advisors[i])[0].firstChild.nodeValue;
                link.setAttribute("college", college);
                $(link).bind("click", function () {
                    CUSTOM.selectedAdvisor = this.getAttribute("recordnumber");
                    $.mobile.changePage("#genedadvisor");
                });
                li.appendChild(link);
                var pCollege = document.createElement("p");
                pCollege.appendChild(document.createTextNode(college));
                pCollege.style.marginLeft = "30px";
                pCollege.style.color = "#542E64";
                pCollege.style.fontSize = "14px";
                li.appendChild(pCollege);
            }
            else {
                $(link).bind("click", function () {
                    CUSTOM.selectedAdvisor = this.getAttribute("recordnumber");
                    $.mobile.changePage("#programadvisor");
                });
                li.appendChild(link);
            }
        ...
        //create Add Advisor link
        var li = document.createElement("li");
        li.setAttribute("data-icon", "plus");
        var link = document.createElement("a");
        link.appendChild(document.createTextNode("Add Program Advisor"));
        ...
        pProgram.appendChild(document.createTextNode("for major/minor advisement"));
        pProgram.style.marginLeft = "30px";
        pProgram.style.color = "#542E64";
        pProgram.style.fontSize = "14px";
        li.appendChild(pProgram);
        ulList.appendChild(li);
        ...
        $("#" + containerID).html("<p>Temporarily unavailable.</p>");
    }

}

CUSTOM.showGenEdAdvisor = function (app, containerID) {
    var container = document.getElementById(containerID)
    container.innerHTML = "";
    if (CUSTOM.selectedAdvisor == 0) {
        var pNone = document.createElement("p");
        pNone.appendChild(document.createTextNode("College advisor not selected."));
        container.appendChild(pNone);
    }
    else {
        var callback = function (data) {
            $("body").removeClass("ui-loading");
            if (data == "error") {
                container.innerHTML = "<p>Temporarily unavailable</p>";
            }
            else {
                var xAdvisor = API.parseXML(data);
                var advisors = xAdvisor.getElementsByTagName("dataitem");
                var detail = $("detail", advisors[0])[0].firstChild.nodeValue;
                container.innerHTML = detail;
            }
        }

        var obj = { App: app, Action: "GetDataRecord", RecordNumber: CUSTOM.selectedAdvisor };
        var request = new API.datastoreRequest(obj);
        $("body").addClass("ui-loading");
        callback(request.submitRequest());
    }

}

CUSTOM.showProgramAdvisor = function (app, containerID) {
...
CUSTOM.showPageContent = function (app, page, containerID) {
    var request = new API.datastoreRequest({ App: app, Action: "GetData", Key1: "Page", Key2: page });
    ...
    var callback = function (data) {
        ...
        if (data == "error") {
            ...
            $("#" + containerID).removeClass("wsu-content-panel");
            $("#" + containerID).html("<p>Temporarily unavailable.</p>");
            return false;
        }
        ...
        try {
            ...
            $("#" + containerID).removeClass("wsu-content-panel");
            $("#" + containerID).html("<p>Temporarily unavailable.");
        }
    };
...
CUSTOM.showSubtitledList = function (app, form, containerID, listTitle, headerTitle, swatch) {
    //API.showList = function (app, form, containerID, listTitle, headerTitle, swatch) {
    ...
    var callback = function (data) {
        ...
        try {
            ...
            for (var i = 0; i < dataItems.length; i++) {
                ...
                var recordnumber = dataItems[i].getAttribute("RecordNumber");
                var headingText = $("title", dataItems[i])[0].firstChild.nodeValue;
                var subtitleText = $("subtitle", dataItems[i])[0].firstChild.nodeValue;
                var detail = $("detail", dataItems[i])[0].firstChild.nodeValue;
                ...
                //create link
                var li = document.createElement("li");
                li.setAttribute("data-icon", "false"); /// NEW
                var link = document.createElement("a");
                link.setAttribute("href", "#pg_" + dataItems[i].getAttribute("RecordNumber"));
                ///link.setAttribute("data-transition", "fade"); /// NEW
                link.appendChild(document.createTextNode($("title", dataItems[i])[0].firstChild.nodeValue));
                ...
                var page = document.createElement("div");
                page.setAttribute("data-role", "page");
                page.setAttribute("data-theme", swatch);
                page.setAttribute("data-add-back-btn", "true");
                page.id = "pg_" + dataItems[i].getAttribute("RecordNumber");
                page.setAttribute("data-url", page.id);
                page.setAttribute("data-add-back-btn", "true");
                ...
                $(grid).addClass("ui-grid-a");

                var blockA = document.createElement("div");
                $(blockA).addClass("ui-block-a")

                var link = document.createElement("a");
                link.setAttribute("href", "");
                link.setAttribute("data-role", "button");
                link.setAttribute("recordnumber", recordnumber);
                link.setAttribute("program", headingText);
                link.appendChild(document.createTextNode("Select"));

                $(link).bind("click", function () {

                    var xLocalStorage = API.parseXML(window.localStorage.getItem("edu_weber_wsumobile_Advisors"));
                    var thisAdvisor = "<advisor recordnumber='" + this.getAttribute("recordnumber") + "'>";
                    thisAdvisor += "<program>";
                    thisAdvisor += "<![CDATA[";
                    thisAdvisor += "College Advisor";
                    thisAdvisor += "]]>";
                    thisAdvisor += "</program>";
                    thisAdvisor += "";
                    thisAdvisor += "<college>";
                    thisAdvisor += "<![CDATA[";
                    thisAdvisor += this.getAttribute("program");
                    thisAdvisor += "]]>";
                    thisAdvisor += "</college>";
                    thisAdvisor += "</advisor>";
                    thisAdvisor += "";

                    var xNewAdvisor = API.parseXML(thisAdvisor);
                    var newAdvisor = xNewAdvisor.getElementsByTagName("advisor")[0];
                    var newAdvisorNode = xLocalStorage.importNode(newAdvisor, true);

                    var currentAdvisorNode = xLocalStorage.getElementsByTagName("advisor")[0];
                    xLocalStorage.documentElement.insertBefore(newAdvisorNode, currentAdvisorNode);
                    currentAdvisorNode.parentNode.removeChild(currentAdvisorNode);

                    window.localStorage.setItem("edu_weber_wsumobile_Advisors", API.serializeXML(xLocalStorage));
                    //console.log(CUSTOM.localStorage);

                    CUSTOM.selectedAdvisor = this.getAttribute("recordnumber");
                    $.mobile.changePage("#my_advisors");
                });

                blockA.appendChild(link);
                grid.appendChild(blockA);

                var blockB = document.createElement("div");
                $(blockB).addClass("ui-block-b");
...
CUSTOM.showPrograms = function (containerID, headerTitle, swatch) {
    //console.log("showing");
    try {
        ...
        $("#" + containerID)[0].innerHTML = "";

        $("#" + containerID)[0].innerHTML += html;
        $("#" + containerID).trigger("create");
        CUSTOM.attachProgramsHandler(containerID, headerTitle, swatch);
    }
    catch (e) {
        if (API.debug == true) {
            console.warn("Error showing A-Z index form");
...
CUSTOM.attachProgramsHandler = function (containerID, headerTitle, swatch) {
    //console.log("attaching handlers");
    $("#collapsibleContainer div", $("#" + containerID)).each(function () {
        this.onexpand = function () {
            var firstLetter = this.id;
            var container = $("div", this)[0];
            $("body").addClass("ui-loading");
            $.ajax({
                type: "GET",
                url: API.server + "/Platform2/Apps/ADVISE/Programs.aspx?firstLetter=" + firstLetter,
                success: function (data) {
                    if (API.debug == true) {
                        console.log("A-Z index reply:");
                        console.log(data);
                    ...
                    try {
                        var xTitles = API.parseXML(data);
                        var titles = xTitles.getElementsByTagName("program");
                        ...
                        for (var i = 0; i < titles.length; i++) {
                            ...
                            var headingText = $("title", titles[i])[0].firstChild.nodeValue;
                            var subtitleText = $("subtitle", titles[i])[0].firstChild.nodeValue;
                            var detail = $("detail", titles[i])[0].firstChild.nodeValue;
                            var recordnumber = titles[i].getAttribute("recordnumber");
                            ...
                            li.setAttribute("data-icon", "false"); /// NEW
                            var link = document.createElement("a");
                            link.setAttribute("href", "#pg_" + recordnumber);
                            ///link.setAttribute("data-transition", "fade"); /// NEW
                            link.appendChild(document.createTextNode(headingText));
                            //link.style.marginBottom = "-20px";
                            li.appendChild(link);

                            if ($("subtitle", titles[i])[0] != null) {
                                if ($("subtitle", titles[i])[0].firstChild.nodeValue != "") {
                                    var pSubtitle = document.createElement("p");
                                    pSubtitle.appendChild(document.createTextNode(subtitleText));
                                    pSubtitle.style.fontSize = "14px";
                                    pSubtitle.style.marginLeft = "25px";
                            ...
                            $(link).bind("click", function () {
                                ...
                                var newAdvisor = xNewAdvisor.getElementsByTagName("advisor")[0];
                                var newAdvisorNode = xLocalStorage.importNode(newAdvisor, true);
                                xLocalStorage.documentElement.appendChild(newAdvisorNode);
                                //console.log(API.serializeXML(xLocalStorage));
                                window.localStorage.setItem("edu_weber_wsumobile_Advisors", API.serializeXML(xLocalStorage));
...
CUSTOM.removeProgramAdvisor = function () {
    var xLocalStorage = API.parseXML(window.localStorage.getItem("edu_weber_wsumobile_Advisors"));
    var selector = "[recordnumber=" + CUSTOM.selectedAdvisor + "]"
    var advisorNode = $(selector, xLocalStorage)[0];
    advisorNode.parentNode.removeChild(advisorNode);
    window.localStorage.setItem("edu_weber_wsumobile_Advisors", API.serializeXML(xLocalStorage));
    $.mobile.changePage("#my_advisors");
}
...
API.showUnstructuredContent = function (app, form, pageID, containerID, unavailableID) {
    ...
    request.submitAsyncRequest(callback);
        ...
        API.app = "ADVISE";
        API.appType = "native";
        API.appReady = "edu_weber_wsumobile_ADVISE_Ready";
        API.appRefreshed = "edu_weber_wsumobile_ADVISE_Refreshed";
        ...
        var f2home = function () {
            $.mobile.changePage("#involvement_submenu");
        }
        var f3home = function () {
            API.showPage(CUSTOM.Home);
        }
        var f4home = function () { 
            API.manualRefresh(); // refreshes data and constants without checking refreshInterval
        }
        $("#home").bind("pagebeforecreate", function () {
            ...
            var image1024 = "images/Graduation-1024.png";
            var image768 = "images/Graduation-768.png";
            var image480 = "images/Graduation-480.png";
            var image320 = "images/Graduation-320.png";
    ...
    $("#my_advisors").bind("pagebeforeshow", function () {
        if ($("#my_advisors").attr("data-dirty") == "true") {
            try {
                CUSTOM.showMyAdvisors("my_advisors", "div0my_advisors", "p0my_advisors");
            }
            catch (e) {
                $("#p0my_advisors").removeClass("hidden");
                if (API.debug == true) {
    ...
    $("#instructions").bind("pagebeforeshow", function () {
        if ($("#instructions").attr("data-dirty") == "true") {
            try {
                API.showUnstructuredContent("ADVISE", "Instructions", "instructions", "div0instructions", "p0instructions");
            }
            catch (e) {
                $("#p0instructions").removeClass("hidden");
                if (API.debug == true) {
                    console.log(e.message);
                }
            }
        ...
        $("#genedadvisor").bind("pagebeforeshow", function () {
            try {
                $("#div0genedadvisor").addClass("wsu-content-panel");
                $("#div0genedadvisor").addClass("hidden");
                $("#p0genedadvisor").addClass("hidden");
                CUSTOM.showGenEdAdvisor("ADVISE", "div0genedadvisor");
                $("#div0genedadvisor").removeClass("hidden");
            }
            catch (e) {
                $("body").removeClass("ui-loading");
                if (API.debug == true) {
                ...
                $("#p0genedadvisor").removeClass("hidden");
            }
        });
        $("#a0genedadvisor").bind("click", function () {
            $.mobile.changePage("#colleges");
        });
    

        $("#programadvisor").bind("pagebeforeshow", function () {
            try {
                $("#div0programadvisor").addClass("wsu-content-panel");
                $("#div0programadvisor").addClass("hidden");
                $("#p0programadvisor").addClass("hidden");
                CUSTOM.showProgramAdvisor("ADVISE", "div0programadvisor");
                $("#div0programadvisor").removeClass("hidden");
            }
            catch (e) {
                $("body").removeClass("ui-loading");
                if (API.debug == true) {
                ...
                $("#p0programadvisor").removeClass("hidden");
            }
        });
        $("#a0programadvisor").bind("click", function () {
            CUSTOM.removeProgramAdvisor();
        });
    

        $("#colleges").bind("pagebeforeshow", function () {
            if ($("#colleges").attr("data-dirty") == "true") {
                try {
                    $("#div0colleges").addClass("wsu-content-panel");
                    $("#div0colleges").addClass("hidden");
                    $("#p0colleges").addClass("hidden");
                    CUSTOM.showPageContent("ADVISE", "CollegeIntroduction", "div0colleges");
                    //API.showList("ADVISE", "CollegeAdvisors", "div1colleges", "College Advisors", "Colleges", "a");
                    CUSTOM.showSubtitledList("ADVISE", "CollegeAdvisors", "div1colleges", "College Advisors", "Colleges", "a");
                    $("#div0colleges").removeClass("hidden");
                    $("#colleges").attr("data-dirty", "false");
                }
                catch (e) {
                    $("body").removeClass("ui-loading");
                    if (API.debug == true) {
                    ...
                    $("#p0colleges").removeClass("hidden");
                }
            }
        });
    

        $("#programs").bind("pagebeforeshow", function () {
            if ($("#programs").attr("data-dirty") == "true") {
                try {
                    $("#div0programs").addClass("wsu-content-panel");
                    $("#div0programs").addClass("hidden");
                    $("#p0programs").addClass("hidden");
                    CUSTOM.showPageContent("ADVISE", "ProgramIntroduction", "div0programs");
                    //API.showList("ADVISE", "ProgramAdvisors", "div1programs", "Program Advisors", "Programs", "a");
                    CUSTOM.showPrograms("div1programs", "Programs", "a");
                    $("#div0programs").removeClass("hidden");
                    $("#programs").attr("data-dirty", "false");
                }
                catch (e) {
                    $("body").removeClass("ui-loading");
                    if (API.debug == true) {
    ...
    var f0involvement_submenu = function () {
        $.mobile.changePage("#activities");
    }
    var f1involvement_submenu = function () {
        $.mobile.changePage("#community");
    }
    $("#involvement_submenu").bind("pagebeforecreate", function () {
        $("#involvement_submenu a").each(function () {
            this.onclick = eval(this.getAttribute("data-onlineclick"));
        });
    });
