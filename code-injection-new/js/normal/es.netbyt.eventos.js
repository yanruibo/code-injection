	...
	if (isArray(response) && response.length == 2) {
		...
		if (objResultado.hasOwnProperty("error")) {
			...
			if (objEvento.info_contacto != "") {
				...
				$('#datos_contacto').html("<br /><p>" + objEvento.info_contacto + "</p>");
	...
	if (isArray(response) && response.length == 3) {
		...
		if (objResultado.hasOwnProperty("error")) {
			...
			if (datos_cabecera.descripcion != "") {
				if (!primero)
					codigo +="<br />";
				codigo += "<p>" + datos_cabecera.descripcion + "</p>";
				primero = false;
			}
...
var _nb_id_empresa = "9";
var segundos_cache = 120;
var _nb_directorio_ficheros = "/data/data/es.netbyt.eventos/ficheros";
...
function inicializarVariables() {
	...
	//almacenLocalGuardarClave(_nb_clave_app_ts_leer_notas_prensa, "");
	//almacenLocalGuardarClave(_nb_clave_app_ts_leer_avisos, "");
	...
	if (isArray(response) && response.length == 2) {
		...
		if (objResultado.hasOwnProperty("error")) {
			...
			for (var i=1 ; i<= contador_notas_prensa ; i++) {
				...
				codigo += "<li><a href='nota_prensa.html?idnp=" + nota_prensa[0] + "' data-ajax='false'><br /><p style='color:#0000CC'>" + nota_prensa[2] + "</p>";
				if (nota_prensa[3] != "")
...
function onDeviceReady() {
	...
	texto_es = {
		...
		"alerta_agenda_vacia" : "La agenda está vacía",
	...
	texto_en = {
		...
		"titulo" : "Agenda of activities",
		"alerta_agenda_vacia" : "The agenda is empty",
		...
		if (objResultado.hasOwnProperty("error")) {
			...
			for (var i=1 ; i<= contador_actividades ; i++) {
				...
				if (fecha_anterior != actividad[4]) {
					codigo += "<li data-role='list-divider'>" + actividad[4] + "</li>";
				...
				codigo += "</td><td><p><strong>" + actividad[1] + "</strong></p>";
				codigo += "<p>" + actividad[5] + "<br />" + actividad[2] + "<br />" + actividad[3] + "</p></td></tr></table></a></li>";
			}
	...
	if (isArray(response) && response.length == 2) {
		...
		if (objResultado.hasOwnProperty("error")) {
			...
			if (datos_cabecera.fecha != "") {
				codigo += "<p style='color: #0000CC;'>" + datos_cabecera.fecha + "</p><br />";
				hay_datos = true;
			}
	...
	if (isArray(response) && response.length == 3) {
		...
		if (objResultado.hasOwnProperty("error")) {
			...
			if (datos_cabecera.desc_edificio != "") {
				codigo += "<p>" + datos_cabecera.desc_edificio + "</p><br />";
				hay_datos = true;
			}
	...
	if (isArray(response) && response.length == 2) {
		...
		if (objResultado.hasOwnProperty("error")) {
			...
			for (var i=1 ; i<= contador_expositores ; i++) {
				...
				codigo += "<td>";
				if (expositor[3] != 0) {
					codigo += "<img class='nb-icono-menu' id='nbe_img" + expositor[3] + "' src='images/loading.gif' />";
					var logo = {};
					logo.id = expositor[3];
					logo.ts = expositor[4];
					array_logos.push(logo);
...
function onDeviceReady() {
	...
	texto_en = {
		...
		"descargar_texto" : "<p>This option allows you to download the event's information content. The advantages are:<ul><li>If you use this option using a Wi-Fi internet connection, the internet data consumption while using the application will be lower because the information will already be downloaded in the device.</li><li>The application will work faster, because less information will have to be downloaded when it is needed.</li></ul></p><p>Although the event's information is downloaded, an internet connection is required in order to guarantee that the information presented by the application is up to date. The data traffic will be very low though.</p>",
	...
	if (isArray(response) && response.length == 2) {
		...
		if (objResultado.hasOwnProperty("error")) {
			...
			for (var i=1 ; i<= contador_colaboradores ; i++) {
				...
				codigo += "<li><table border='0'><tr valign='center'><td>";
				if (colaborador[5] != 0) {
				...
				codigo += "</td><td class='nb-texto-menu'><p>" + colaborador[2] + "</p></td></tr>";
				if (colaborador[4] != "") {
