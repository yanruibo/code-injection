			...
			$(function(){
				MENU.core.init('#contenedor-menu');
			});
			...
			function onLoad() {
				document.addEventListener("deviceready", function() {
					...
					MENU.lightVersion = parseFloat(device.version)<4;
				}, false);
			}
			...
			$(function() {
				var evento = 'ontouchstart' in document.documentElement?'touchend':'click';

				$.mobile.ajaxLinksEnabled = false;
				$.mobile.defaultPageTransition = 'none';

				$('div[data-role="navbar"]').find('li').each(function() {
					var fun = (function(pagina) {
						return function() {
					...
					$(this).bind(evento, fun);
					$(this).addClass('menu-item');
				});

				$('.btn-back').bind(evento, function() {
						window.location.href = 'index.html';
				});

				$('.btn-atras').unbind(evento);
				$('.btn-atras').bind(evento, function() {
					$('#contenido').empty();
					navigator.app.backHistory();
				});

				$('#noticias').on('pagebeforeshow', TWEETS.loadHashTag);

				$('#curiosidades').on('pagebeforeshow', CURIOSIDADES.ready);
				$('#hermandades').on('pagebeforeshow', HERMANDADES.ready);
				$('#datosinteres').on('pagebeforeshow', DATOS.ready);
				$('#datosInteres-autobuses').on('pagebeforeshow', BUS.ready);
				$('#datosInteres-planRomero').on('pagebeforeshow', PLAN.ready);

				$('#galeria').on('pagebeforeshow', GALERIA.ready);
				$('#videos').on('pagebeforeshow', VIDEOS.ready);

				$('#mapa').on('pagebeforeshow', MAPA.ready);
				$('#vivo').on('pagebeforeshow', RADIO.ready);
				$('#eventos').on('pagebeforeshow', EVENTOS.ready);

				$('a[target="_blank"]').bind('tap',function(e){
				...
				arrayBanner.push({img: 'images/anuncios/animacionArtenovia.gif', link: 'http://www.celsoprieto.com/'});
				arrayBanner.push({img: 'images/anuncios/animacionBrikasa.gif', link: null});
				arrayBanner.push({img: 'images/anuncios/animacionMacha2.gif', link: 'http://www.barmacha.com/'});
				arrayBanner.push({img: 'images/anuncios/animacionJamones.gif', link: 'http://www.jamonessanchez.com/'});

				var random = Math.floor(Math.random()*arrayBanner.length);
				
				$('.anuncio').css("background-image", "url("+arrayBanner[random].img+")");
				if(arrayBanner[random].link) {
					$('.anuncio').bind(evento, function() {
						navigator.app.loadUrl(arrayBanner[random].link, { openExternal:true });
					});
				}
			});
			...
			function onLoad() {
				document.addEventListener("deviceready", function() {
					...
					navigator.geolocation.getCurrentPosition(MAPA.onPosicion, MAPA.onError, {timeout: 10000});
					MUSICA.ready();
				}, false);
			}

			function back() { navigator.app.backHistory(); };
			function index() { window.location.href = 'index.html'; };

			function setToBack() {
				document.removeEventListener('backbutton', back, false);
				document.removeEventListener('backbutton', index, false);
			...
			function setToIndex() {
				document.removeEventListener('backbutton', back, false);
				document.removeEventListener('backbutton', index, false);
				document.addEventListener('backbutton', index, false);
			}
...
CURIOSIDADES.loadData = function(d) {
	...
	for(var i in d) {
		...
		curiosidad.appendTo(contenedor);
	}

	contenedor.listview('refresh');
};

CURIOSIDADES.ready = function() {
	if(!$('#listado-curiosidades').children('li').length) {
		$.getJSON('data/curiosidades.json', CURIOSIDADES.loadData)
		.fail(function() { console.log( 'JSON load error' ); });
	}
...
HERMANDADES.rutaDisponible = [];

HERMANDADES.loadData = function(d) {
	var contenedor = $('#listado-hermandades').empty();

	for(var i in d) {
		var hermandad = $('<li></li>').addClass('recuadro');
		hermandad.append($('<div></div>').addClass('titulo').text(d[i].nombreCorto));
		hermandad.append($('<div></div>').addClass('subtitulo').text('Huelva'));
		hermandad.append($('<div></div>').addClass('img').css('background-image', 'url(images/medallas/'+d[i].medalla+')'));

		var evento = 'ontouchstart' in window?'tap':'click';
		hermandad.bind(evento, function(id) {
			return (function() {
				$.mobile.changePage('#pagina-contenido');
				CONTENIDO.loadContenido(id, 0);
			});
		}(d[i].id));
		hermandad.appendTo(contenedor);
	}

	contenedor.listview('refresh');
};

HERMANDADES.loadDisponibles = function(d) {
	HERMANDADES.rutaDisponible = [];

	if(!d.blockCamino)
		for(var i in d)
			if(d[i].id) HERMANDADES.rutaDisponible.push(parseInt(d[i].id));
}


HERMANDADES.ready = function() {
	if(!$('#listado-hermandades').children('li').length) {
		$.getJSON('data/hermandades.json', HERMANDADES.loadData)
		.fail(function() { console.log( 'JSON load error' ); });

		$.getJSON('http://www.oxik.net/proyectos/iRocio/tracker/getData.php?t=true', HERMANDADES.loadDisponibles)
		.fail(function() { console.log( 'JSON load error' ); });
	}
...
PLAN.ready = function() {
	...
	$('.planRomero-icon').unbind(evento);
	$('.planRomero-icon').bind(evento, function() {			
		MAPA.showServicio(parseInt($(this).attr('marker')));
...
CONTENIDO.loadHermandad = function(d) {
	var contenedor = $('#contenido').empty();

	var ul = $('<ul></ul>')
		.attr('data-role', 'listview')
		.attr('data-inset', 'false');

	var li = $('<li></li>').addClass('recuadro');
	$('<div></div>').addClass('titulo-full')
		.text(d.nombre)
		.appendTo(li);
	li.appendTo(ul);

	if(d.posicion) {
		li = $('<li></li>').addClass('recuadro');
		li.append($('<div></div>').addClass('imgIzq'));
		$('<div></div>').addClass('titulo-margin')
			.html('Ver en el Mapa<br>Casa Hermandad')
			.appendTo(li);

		var evento = 'ontouchstart' in window?'tap':'click';
		li.bind(evento, function(id) {
			return (function() {
				MAPA.showMarker(id);
				$.mobile.changePage('#mapa');
			});
	...
	if(HERMANDADES.rutaDisponible.indexOf(d.id) != -1 ) {
		li = $('<li></li>').addClass('recuadro');
		li.append($('<div></div>').addClass('imgIzq'));
		$('<div></div>').addClass('titulo-margin')
			.html('Seguimiento en<br>tiempo real')
			.appendTo(li);

		var evento = 'ontouchstart' in window?'tap':'click';
		li.bind(evento, function(id) {
			return (function() {
				MAPA.caminoHermandad(id);
				$.mobile.changePage('#mapa');
			});
		}(d.id));
		li.appendTo(ul);
	...
	ul.appendTo(contenedor);
	ul.listview();

	var texto = $('<div></div>').addClass('contenido-texto').html(d.texto);
	texto.children('.img-medalla').css('background-image', 'url(images/medallas/'+d.medalla+')');
	texto.children('.img-simpecado').css('background-image', 'url(images/simpecados_foto/'+d.simpecadoFoto+')');
	texto.appendTo($('#contenido'));
};

CONTENIDO.loadCuriosidad = function(d) {
	var contenedor = $('#contenido').empty();
	...
	$('<div></div>').addClass('contenido-texto')
		.html(d.texto)
		.appendTo($('#contenido'));
};

CONTENIDO.loadEnd = function() {
	$('.imagen-contenido').each(function() {
		var esto = $(this);
		var parent = esto.parent();

		var contenedor = $('<div></div>').addClass('contenedor-imagen');
		contenedor.insertBefore(esto);

		esto.remove();
		esto.appendTo(contenedor);


		resize = (function(alturaInicial) {
			return function() {
				var h = $(this).children('img').height();
				if($(this).height() == alturaInicial)
					$(this).height(h);
				else
					$(this).height(alturaInicial);
			};
		})(contenedor.height());
		
		var evento = 'ontouchstart' in window?'tap':'click';
		contenedor.bind(evento, resize);
	});
};

CONTENIDO.loadContenido = function(id, tipo) {
	ready&&setToBack();
	
	switch(tipo) {
		case 0:
			$('.encabezado').text('Hermandad');
			$.getJSON('data/hermandades.json', function(d) {
				if(d[id]){
					CONTENIDO.loadHermandad(d[id]);
					CONTENIDO.loadEnd();
				} else console.log( 'Los datos de la hermandad '+id+' no existen' );
			}).fail(function() { console.log( 'JSON load error' ); });
			break;

		case 1:
			$('.encabezado').text('Curiosidad');
			$.getJSON('data/curiosidades.json', function(d) {
				if(d[id]) {
					CONTENIDO.loadCuriosidad(d[id]);
					CONTENIDO.loadEnd();
				} else console.log( 'Los datos de la curiosidad '+id+' no existen' );
			}).fail(function() { console.log( 'JSON load error' ); });
			break;
	}
};
...
DATOS.ready = function() {
	...
	$('#di-btn-plan-romero').bind(evento, function() {
		$.mobile.changePage('#datosInteres-planRomero');
	});

	$('#di-btn-bus').bind(evento, function() {
		$.mobile.changePage('#datosInteres-autobuses');
	});

	$('#di-btn-telefonos').bind(evento, function() {
		$.mobile.changePage('#datosInteres-telefonos');
	});
	
	$('#di-btn-agradecimientos').bind(evento, function() {
		$.mobile.changePage('#datosInteres-agradecimientos');
	});
	
	$('#di-btn-estentor').bind(evento, function() {
		$.mobile.changePage('#datosInteres-estentor');
	});
...
RADIO.onSuccess = function () {
	console.log("playAudio():Audio Success");
};

RADIO.onError = function(error) {
	console.log('code: '+error.code+'\n' +'message: '+error.message+'\n');
};

RADIO.onStatus = function(stat) {
	RADIO.status = stat;
	var elem = $('#player-radio-boton')
		.removeClass('buffer')
		.removeClass('stop');

	switch(stat) {
		case Media.MEDIA_STARTING:
			elem.addClass('buffer');
			break;
...
RADIO.go = function(src) {
	if(RADIO.status == Media.MEDIA_STARTING || RADIO.status == Media.MEDIA_RUNNING) {
		RADIO.actual.stop();
		RADIO.onStatus(0);
	} else {
		RADIO.actual = new Media(src, RADIO.onSuccess, RADIO.onError, RADIO.onStatus);
		RADIO.actual.play();
	}
};
...
RADIO.ready = function() {
	var evento = 'ontouchstart' in window?'tap':'click';
	if(!RADIO.actual) {
		$('#player-radio').bind(evento, function() {
			RADIO.go('http://195.10.10.206/rtva/canalsurradio_master.mp3');
		});
	}
};
...
BUS.ready = function() {
	if($('#contenido-autobuses').is(':empty')) {
		$.ajax({
			url: 'data/bus.html',
			success: function(data) {
				$('#contenido-autobuses').html(data);

				var evento = 'ontouchstart' in window?'tap':'click';
				$('.ver-mapa').bind(evento, function() {			
					MAPA.showServicio(parseInt($(this).attr('marker')));
					$.mobile.changePage('#mapa');
				});
			}
...
var MUSICA = {};
MUSICA.actual = null;
MUSICA.mediaTimer = null;
MUSICA.status = 0;

MUSICA.onSuccess = function () {
	console.log("playAudio():Audio Success");
};

MUSICA.onError = function(error) {
	console.log('code: '+error.code+'\n' +'message: '+error.message+'\n');
};

MUSICA.play = function(src) {
	if(MUSICA.actual) MUSICA.actual.stop();
	MUSICA.actual = new Media(src, MUSICA.onSuccess, MUSICA.onError);
	MUSICA.actual.play();
	MUSICA.status = 1;

	if(MUSICA.mediaTimer == null) {
		MUSICA.mediaTimer = setInterval(function() {
			MUSICA.actual.getCurrentPosition(
				function(position) {
					if(position > -1)
						MUSICA.setAudioPosition(position);
				},
				MUSICA.onError
			);
		}, 1000);
...
MUSICA.pauseAudio = function() {
	if(MUSICA.actual) {
		if(MUSICA.status === 1) {
			MUSICA.actual.pause();
			MUSICA.status = 2;
		} else if(MUSICA.status === 2) {
			MUSICA.actual.play();
			MUSICA.status = 1;
		}
	} // else MUSICA.play($('.sonido').first().attr('ref'));
};

MUSICA.stopAudio = function() {
	if(MUSICA.actual) MUSICA.actual.stop();
	MUSICA.status = 0;
	clearInterval(MUSICA.mediaTimer);
	MUSICA.mediaTimer = null;
	MUSICA.setAudioPosition(0);
};

MUSICA.setAudioPosition = function(position) {
	var percentage = (position/MUSICA.actual.getDuration())*100;
	$('#player-bar-progress').css('width', percentage+'%');
};

MUSICA.ready = function() {
	var evento = 'ontouchstart' in window?'tap':'click';
	if(!MUSICA.actual) {
		$('.sonido').bind(evento, function() {
			MUSICA.play($(this).attr('ref'));
		});

		$('#player-play').bind(evento, function() {
			MUSICA.pauseAudio();
		});
		
		$('#player-stop').bind(evento, function() {
			MUSICA.stopAudio();
		});
	}
};
...
VIDEOS.loadData = function(d) {
	var contenedor = $('#contenedor-videos').empty();
	var h = $('body').width()/1.777777778;
	if(h<200) h=200;
	...
	for(var i in d) {
		var ul = $('<ul></ul>')
			...
			.attr('data-inset', 'false');
		var li = $('<li></li>').addClass('recuadro');
		li.append($('<div></div>').addClass('titulo-full').text(d[i].titulo));
		li.append($('<div></div>').addClass('subtitulo-full').text(d[i].descripcion));
		li.appendTo(ul);
		ul.appendTo(contenedor);
		ul.listview();
		...
		$('<iframe></iframe>')
			.attr('width', '100%')
			.attr('height', h+'px')
			.attr('src', 'http://www.youtube.com/embed/'+d[i].vinculo)
			.attr('frameborder', '0')
...
VIDEOS.ready = function() {
	if(!$('#contenedor-videos').children('ul').length) {

		if(!navigator || !navigator.network || !navigator.network.connection || navigator.network.connection.type == Connection.NONE) {
			alert('Actualmente no te encuentras conectado a internet. Conéctate para poder acceder a los vídeos.');
		} else {
			$.getJSON('data/videos.json', VIDEOS.loadData)
			.fail(function() { console.log( 'JSON load error' ); });
		}
	}
};
...
MAPA.caminos;
MAPA.coloresCaminos = ['#d7a72b', '#d72b3f', '#2b8cd7', '#3db248', '#797979'];

MAPA.loadData = function(d) {
	if(!MAPA.map) {
		L.Control.Zoom({ options: {
				position: 'bottomleft'
			}
		});
		MAPA.map = L.map('contenedor-mapa');

		L.tileLayer(
			'http://{s}.tile.cloudmade.com/5339a9099f474c20871329290deeff1a/997/256/{z}/{x}/{y}.png',
			{ attribution: '', maxZoom: 18, touchZoom: false }
		).addTo(MAPA.map);

		MAPA.hermandades = d;
		for(var i in d) {
			var casa = d[i].nombreCorto==='Hermandad Matriz'?'Casa ':'Casa Hermandad de ';
			if(d[i].posicion) {
				MAPA.markers.push(
					L.marker(d[i].posicion)
						.bindPopup('<div class="imagen" style="background-image: url(\'images/simpecados/'+d[i].simpecado+'\')"></div><b>'+casa+d[i].nombreCorto+'</b>')
						.addTo(MAPA.map)
				);
			}
			else
				MAPA.markers.push(null);
		}

		// Iconos
		var iconParking = L.icon({
			iconUrl: 'js/images/iconParking.png',
			iconSize: [25, 31],  iconAnchor: [17.5, 15.5], popupAnchor: [-5, -12]
		});
		var iconOficinaMunicipal = L.icon({
			iconUrl: 'js/images/iconOficinaMunicipal.png',
			iconSize: [25, 31],  iconAnchor: [17.5, 15.5], popupAnchor: [-5, -12]
		});
		var iconGasolinera = L.icon({
			iconUrl: 'js/images/iconGasolinera.png',
			iconSize: [25, 31],  iconAnchor: [17.5, 15.5], popupAnchor: [-5, -12]
		});
		var iconPoliciaLocal = L.icon({
			iconUrl: 'js/images/iconPoliciaLocal.png',
			iconSize: [25, 31],  iconAnchor: [17.5, 15.5], popupAnchor: [-5, -12]
		});
		var iconFarmacia = L.icon({
			iconUrl: 'js/images/iconFarmacia.png',
			iconSize: [25, 31],  iconAnchor: [17.5, 15.5], popupAnchor: [-5, -12]
		});
		var iconCentroCoordinacion = L.icon({
			iconUrl: 'js/images/iconCentroCoordinacion.png',
			iconSize: [25, 31],  iconAnchor: [17.5, 15.5], popupAnchor: [-5, -12]
		});
		var iconGuardiaCivil = L.icon({
			iconUrl: 'js/images/iconGuardiaCivil.png',
			iconSize: [25, 31],  iconAnchor: [17.5, 15.5], popupAnchor: [-5, -12]
		});
		var iconServiciosPublicos = L.icon({
			iconUrl: 'js/images/iconServiciosPublicos.png',
			iconSize: [25, 31],  iconAnchor: [17.5, 15.5], popupAnchor: [-5, -12]
		});
		var iconErmita = L.icon({
			iconUrl: 'js/images/iconErmita.png',
			iconSize: [153, 85], iconAnchor: [76.5, 42.5], popupAnchor: [5, -15]
		});
		var iconBaseEmergencias1 = L.icon({
			iconUrl: 'js/images/iconGrea.png',
			iconSize: [25, 31],  iconAnchor: [17.5, 15.5], popupAnchor: [-5, -12]
		});
		var iconBaseEmergencias2 = L.icon({
			iconUrl: 'js/images/iconUnidadPoliciaAndalucia.png',
			iconSize: [25, 31],  iconAnchor: [17.5, 15.5], popupAnchor: [-5, -12]
		});
		var iconBaseEmergencias3 = L.icon({
			iconUrl: 'js/images/iconBomberos.png',
			iconSize: [25, 31],  iconAnchor: [17.5, 15.5], popupAnchor: [-5, -12]
		});
		var iconBaseEmergencias4 = L.icon({
			iconUrl: 'js/images/iconAsistenciaSanitaria.png',
			iconSize: [25, 31],  iconAnchor: [17.5, 15.5], popupAnchor: [-5, -12]
		});
		var iconBaseEmergencias5 = L.icon({
			iconUrl: 'js/images/icon112.png',
			iconSize: [25, 31],  iconAnchor: [17.5, 15.5], popupAnchor: [-5, -12]
		});
		var iconAutobus = L.icon({
			iconUrl: 'js/images/iconBus.png',
			iconSize: [25, 31],  iconAnchor: [17.5, 15.5], popupAnchor: [-5, -12]
		});

		// Marcas
		MAPA.markers.push( L.marker([37.135090224383300, -6.4732946624755860], {icon: iconParking}           ).addTo(MAPA.map).bindPopup("<b>Parking</b>") );
		MAPA.markers.push( L.marker([37.138937700794290, -6.4796590805053710], {icon: iconParking}           ).addTo(MAPA.map).bindPopup("<b>Parking</b>") );
		MAPA.markers.push( L.marker([37.140135061730320, -6.4865684509277340], {icon: iconParking}           ).addTo(MAPA.map).bindPopup("<b>Parking</b>") );
		MAPA.markers.push( L.marker([37.132835067759250, -6.4945503425598145], {icon: iconParking}           ).addTo(MAPA.map).bindPopup("<b>Parking</b>") );                            //3
		MAPA.markers.push( L.marker([37.139000654989910, -6.4890445938110350], {icon: iconOficinaMunicipal}  ).addTo(MAPA.map).bindPopup("<b>Oficina Municipal</b>") );                  //4
		MAPA.markers.push( L.marker([37.136000654989910, -6.4890445938110350], {icon: iconGasolinera}        ).addTo(MAPA.map).bindPopup("<b>Gasolinera</b>") );                         //5
		MAPA.markers.push( L.marker([37.132800654989910, -6.4890445938110350], {icon: iconPoliciaLocal}      ).addTo(MAPA.map).bindPopup("<b>Policia Local</b>") );                      //6
		MAPA.markers.push( L.marker([37.132800654989910, -6.4850445938110350], {icon: iconFarmacia}          ).addTo(MAPA.map).bindPopup("<b>Farmacia</b>") );                           //7
		MAPA.markers.push( L.marker([37.133400654989910, -6.4820445938110350], {icon: iconCentroCoordinacion}).addTo(MAPA.map).bindPopup("<b>Centro Coordinador</b>") );                 //8
		MAPA.markers.push( L.marker([37.133500654989910, -6.4870445938110350], {icon: iconGuardiaCivil}      ).addTo(MAPA.map).bindPopup("<b>Guardia Civil</b>") );                      //9
		MAPA.markers.push( L.marker([37.130950654989910, -6.4897445938110350], {icon: iconServiciosPublicos} ).addTo(MAPA.map).bindPopup("<b>Servicios Públicos</b>") );                 //10
		MAPA.markers.push( L.marker([37.132099654989910, -6.4767445938110350], {icon: iconServiciosPublicos} ).addTo(MAPA.map).bindPopup("<b>Servicios Públicos</b>") );                 //11
		MAPA.markers.push( L.marker([37.130490654989910, -6.4850045938110350], {icon: iconErmita}            ).addTo(MAPA.map).bindPopup("<b>Ermita</b>") );                             //12
		MAPA.markers.push( L.marker([37.141599802297320, -6.4899546890258790], {icon: iconBaseEmergencias1}  ).addTo(MAPA.map).bindPopup("<b>Grupo de Emergencias de Andalucía</b>") );  //13
		MAPA.markers.push( L.marker([37.141599802297320, -6.4895546890258790], {icon: iconBaseEmergencias2}  ).addTo(MAPA.map).bindPopup("<b>Unidad Policía Andalucía</b>") );           //14
		MAPA.markers.push( L.marker([37.141599802297320, -6.4891546890258790], {icon: iconBaseEmergencias3}  ).addTo(MAPA.map).bindPopup("<b>Bomberos</b>") );                           //15
		MAPA.markers.push( L.marker([37.141599802297320, -6.4887546890258790], {icon: iconBaseEmergencias4}  ).addTo(MAPA.map).bindPopup("<b>Asistencia Sanitaria</b>") );               //16
		MAPA.markers.push( L.marker([37.141599802297320, -6.4883546890258790], {icon: iconBaseEmergencias5}  ).addTo(MAPA.map).bindPopup("<b>Emergencias</b>") );                        //17
		MAPA.markers.push( L.marker([37.257659125315890, -6.9575107097625730], {icon: iconAutobus}           ).addTo(MAPA.map).bindPopup("<b>Estación Autobuses de Huelva</b>") );               //18
		MAPA.markers.push( L.marker([37.131550654989910, -6.4893445938110350], {icon: iconAutobus}           ).addTo(MAPA.map).bindPopup("<b>Estación Autobuses de El Rocío</b>") );             //19
		MAPA.markers.push( L.marker([37.390558412654740, -6.0051763057708740], {icon: iconAutobus}           ).addTo(MAPA.map).bindPopup("<b>Estación Autobuses de Sevilla</b>") );              //20
		MAPA.markers.push( L.marker([37.010093300614870, -6.5639444091796875], {icon: iconAutobus}           ).addTo(MAPA.map).bindPopup("<b>Estación Autobuses de Matalascañas</b>") );         //21
		MAPA.markers.push( L.marker([37.262782535145300, -6.5232717990875240], {icon: iconAutobus}           ).addTo(MAPA.map).bindPopup("<b>Estación Autobuses de Almonte</b>") );              //22
		MAPA.markers.push( L.marker([37.340188949678920, -6.5367794036865230], {icon: iconAutobus}           ).addTo(MAPA.map).bindPopup("<b>Estación Autobuses de Bollullos</b>") );            //23
		MAPA.markers.push( L.marker([37.390780041993416, -6.5546643733978270], {icon: iconAutobus}           ).addTo(MAPA.map).bindPopup("<b>Estación Autobuses de la Palma del Condado</b>") ); //24
	}

	MAPA.map.on('popupopen', function(e) {
		var evento = 'ontouchstart' in window?'tap':'click';

		$('#ver-googlemaps').show();
		function googleString(A, B) {
			return function(e){
				var dir = A?('http://maps.google.com/?saddr='+A.lat+','+A.lng+'&daddr='+B.lat+','+B.lng+'&dirflg=w'):('http://maps.google.com/?daddr='+B.lat+','+B.lng+'&dirflg=w');
				if(navigator.app) navigator.app.loadUrl(dir, { openExternal:true });
				else window.open(dir, '_blank')
				e.stopPropagation();
				return false;
			}
		}

		$('#ver-googlemaps').unbind(evento);
		$('#ver-googlemaps').bind(evento, googleString(MAPA.currentPosition?MAPA.currentPosition.getLatLng():null, e.popup._latlng));
	});

	MAPA.map.on('popuclose', function(e) {
		$('#generar-ruta').hide();
		$('#ver-googlemaps').hide();
	});
	
	if(MAPA.currentPosition)
		MAPA.currentPosition.addTo(MAPA.map).bindPopup("<b>Te encuentras aquí</b>");

	if(MAPA.onFinish) MAPA.onFinish();
	else MAPA.map.setView([37.13366902512038, -6.482534408569336], 9);

	if(MAPA.dataCamino) MAPA.muestraCamino();
};

MAPA.showMarker = function(id) {
	var onFinish = (function(i) {
		return function() {
			MAPA.map.setView(MAPA.markers[i].getLatLng(), 17);
			var pop = (function(j) {
				return function() {
					MAPA.markers[j].openPopup();
				};
			})(i);
			setTimeout(pop, 1000);
		}
	...
	if(MAPA.map) onFinish();
	else MAPA.onFinish = onFinish;
};

MAPA.showServicio = function(id) {
	MAPA.showMarker(id+MAPA.serviciosOffset);
};

MAPA.onPosicion = function(position) {
	var iconPosicion = L.icon({
		iconUrl: 'js/images/iconPosicion.png',
		iconSize: [25, 31],  iconAnchor: [17.5, 15.5], popupAnchor: [-5, -12]
	});

	MAPA.currentPosition = L.marker(
	...
	var evento = 'ontouchstart' in window?'tap':'click';
	$('#buscar-aceptar').unbind(evento);
	$('#buscar-aceptar').bind(evento, function() {
		var dir = 'https://maps.google.es/maps?&saddr='+position.coords.latitude+','+position.coords.longitude+'&daddr='+encodeURIComponent($('#buscar-mapa').val())+'&dirflg=w';
		if(navigator.app) navigator.app.loadUrl(dir, { openExternal:true });
		else window.open(dir, '_blank')
	});
};

MAPA.muestraCamino = function() {
	var puntos = {};
	var caminoUnico = null;

	for(var i in MAPA.dataCamino) {
		var p = MAPA.dataCamino[i];

		if(p.lat && p.lng) {
			if(!puntos[p.id]) puntos[p.id] = [];
			puntos[p.id].push(new L.LatLng(
				p.lat,
				p.lng
			));
		}
	...
	if(MAPA.caminos) MAPA.caminos.clearLayers();
	else MAPA.caminos = L.layerGroup();
	for(var i in puntos) {
		MAPA.caminos.addLayer(new L.Polyline(puntos[i], {
			weight: 3,
			opacity: 0.5,
			smoothFactor: 1,
			color: MAPA.coloresCaminos[parseInt(i)%MAPA.coloresCaminos.length]
		}));

		var icon = L.icon({
			iconUrl: 'js/images/iconCarreta.png',
			iconSize: [50, 53],  iconAnchor: [25, 52], popupAnchor: [0, -50]
		});
		MAPA.caminos.addLayer(
			L.marker(puntos[i][0], {icon: icon})
				.bindPopup('<div class="imagen" style="background-image: url(\'images/simpecados/'+MAPA.hermandades[i].simpecado+'\')"></div><b>'+MAPA.hermandades[i].nombreCorto+'</b>')
		);

		caminoUnico = caminoUnico===null?puntos[i]:false;
	}

	caminoUnico&&MAPA.map.setView(caminoUnico[0], 17);
	MAPA.dataCamino.blockCamino||MAPA.caminos.addTo(MAPA.map);
	MAPA.dataCamino = null;
};

MAPA.loadCamino = function(data) {
	MAPA.dataCamino = data;
	MAPA.map&&MAPA.muestraCamino();
};

MAPA.caminoHermandad = function(id) {
	MAPA.loadingHermandad = true;
	$.getJSON('http://www.oxik.net/proyectos/iRocio/tracker/getData.php?h='+id, MAPA.loadCamino)
	.fail(function() { console.log( 'JSON load error' ); });
};

MAPA.caminosHermandades = function() {
	MAPA.loadingHermandad = true;
	$.getJSON('http://www.oxik.net/proyectos/iRocio/tracker/getData.php', MAPA.loadCamino)
	.fail(function() { console.log( 'JSON load error' ); });
};

MAPA.onError = function(error) {
	if($.mobile.activePage.attr('id') == 'mapa') {
		console.log('code: '+error.code+'\n' +'message: '+error.message+'\n');
		alert('No se ha podido obtener tu ubicación, por favor activa tu ubicación dentro de las opciones de seguridad de tu navegador.');
	}
};

MAPA.ready = function() {
	if(!MAPA.map && !MAPA.tiemout) {
		MAPA.tiemout = setTimeout(function() {
			$.getJSON('data/hermandades.json', MAPA.loadData)
			.fail(function() { console.log( 'JSON load error' ); });
		}, 2000);
	}

	MAPA.loadingHermandad||MAPA.caminosHermandades();
};
...
$(function() {
	...
	if(!MAPA.currentPosition) {
		$('#buscar-aceptar').unbind(evento);
		$('#buscar-aceptar').bind(evento, function() {
			var dir = 'https://maps.google.es/maps?q='+encodeURIComponent($('#buscar-mapa').val())+'&dirflg=w';
			if(navigator.app) navigator.app.loadUrl(dir, { openExternal:true });
			else window.open(dir, '_blank')
		});
	}
...
MENU.colores = ['#D7A72B'];
MENU.lightVersion = false;

MENU.getParameterByName = function(name) {
...
MENU.core = (function() {
	...
	function init(c) {
		...
		$('#menu-principal-canvas').remove();
		$('#menu-principal-fondo').remove();

		var canvas0 = $('<canvas id="menu-principal-canvas0">')
			.attr('height', h)
			.attr('width', w);

		var canvas1 = $('<canvas id="menu-principal-canvas1">')
			.attr('height', h)
			.attr('width', w);

		var canvas2 = $('<canvas id="menu-principal-canvas2">')
			.attr('height', h)
			.attr('width', w);

		container.append('<div id="menu-principal-fade"></div>');
		container.append('<div id="menu-principal-fondo"></div>');
		container.append(canvas0);

		container.append(canvas1);
		container.append(canvas2);

		ctx = canvas0[0].getContext('2d');

		MENU.slaidTop.init(canvas1[0].getContext('2d'), w, h);
		MENU.slaidBottom.init(canvas1[0].getContext('2d'), w, h);
		MENU.slaidLeft.init(canvas2[0].getContext('2d'), w, h);
		MENU.slaidRight.init(canvas2[0].getContext('2d'), w, h);

		dragHandler = (function(w, h) {
			var x0, y0;
			...
			function start(e) {
				...
				var x = (touch.pageX-w/2)/(w/2);
				var y = (touch.pageY-h/2)/(h/2);
			...
			function stop(e) {
				...
				var absX = Math.abs(x0-x);
				var absY = Math.abs(y0-y);

				removeMenuEvents();
				if( absX+absY>.2 ) {
					$('.sobre-todo').hide(MENU.lightVersion?0:500);
					...
					if(elem==0) MENU.slaidTop.show(onTop);
					else        MENU.slaidTop.hide();

					if(elem==1) MENU.slaidBottom.show(onBottom);
					else        MENU.slaidBottom.hide();

					if(elem==2) MENU.slaidLeft.show(onLeft);
					else        MENU.slaidLeft.hide();

					if(elem==3) MENU.slaidRight.show(onRight);
					else        MENU.slaidRight.hide();
				} else {
					// Click
					var elem;
					if(Math.abs(x0)>Math.abs(y0)) elem = x0<0?2:3;
					else                          elem = y0<0?0:1;

					if(elem==0) {
						if(MENU.slaidTop.isVisible()) {
							$('.sobre-todo').hide(MENU.lightVersion?0:500);
							MENU.slaidTop.show(onTop);
						} else MENU.slaidTop.showHalf(setMenuEvents);
						container.find('.top>ul>li').show();
					} else {
						MENU.slaidTop.hide();
						container.find('.top>ul>li').hide();
					}

					if(elem==1) {
						if(MENU.slaidBottom.isVisible()) {
							$('.sobre-todo').hide(MENU.lightVersion?0:500);
							MENU.slaidBottom.show(onBottom);
						} else MENU.slaidBottom.showHalf(setMenuEvents);
						container.find('.bottom>ul>li').show();
					} else {
						MENU.slaidBottom.hide();
						container.find('.bottom>ul>li').hide();
					}

					if(elem==2) {
						if(MENU.slaidLeft.isVisible()) {
							$('.sobre-todo').hide(MENU.lightVersion?0:500);
							MENU.slaidLeft.show(onLeft);
						} else MENU.slaidLeft.showHalf(setMenuEvents);
						container.find('.left>ul>li').show();
					} else {
						MENU.slaidLeft.hide();
						container.find('.left>ul>li').hide();
					}

					if(elem==3) {
						if(MENU.slaidRight.isVisible()) {
							$('.sobre-todo').hide(MENU.lightVersion?0:500);
							MENU.slaidRight.show(onRight);
						} else MENU.slaidRight.showHalf(setMenuEvents);
						container.find('.right>ul>li').show();
					} else {
						MENU.slaidRight.hide();
						container.find('.right>ul>li').hide();
					}
				}
			}
		...
		if(!MENU.getParameterByName('intro')) {
			MENU.intro.setReady(ctx, w, h);
			setMenuEvents();
		}else
			introLoop();
	}
	...
	function introLoop() {
		...
		if(MENU.intro.draw(ctx, w, h) && !MENU.lightVersion) {
			requestAnimFrame(introLoop);
		} else {
			if(MENU.lightVersion)
				setTimeout(function() {
					MENU.intro.setReady(ctx, w, h);
					setMenuEvents();
				}, 3000);
	...
	function setMenuEvents() {
		$('#logo-app').bind('touchend', function() {
			window.location = 'secciones.html#introduccion';
		});
		if('ontouchstart' in document.documentElement) {
			container.bind('touchstart', dragHandler.start);
			container.bind('touchend', dragHandler.stop);
		} else {
			container.bind('mousedown', dragHandler.start);
			container.bind('mouseup', dragHandler.stop);
		}
	...
	function onTop()    { window.location = 'secciones.html#hermandades'; }
	function onBottom() { window.location = 'secciones.html#galeria'; }
	function onRight()  { window.location = 'secciones.html#mapa'; }
	function onLeft()   { window.location = 'secciones.html#noticias'; }
...
MENU.slaid = function(dir) {
	...
	arrayImage[0]='images/logos/logo_canalsur_alpha.png';
	arrayImage[1]='images/logos/brikasa.png';
	arrayImage[2]='images/logos/saramerino.png';
	arrayImage[3]='images/logos/macha.png';
	arrayImage[4]='images/logos/logoJamones.png';

	var image=new Image();
	image.src=arrayImage[getRandom(0, 4)];

	var tiempos = [700, 800, 1000, 1000];
	var offset;
	var direccion = dir;
	...
	function forma() {
		...
		switch(direccion) {
			case 0:
				o = offset-mH;
				imgW = w/2-image.width/2;
				imgH = o-image.height/2-h/2;

				if(estado===1 || estado===3) ctx.clearRect(0, 0, w, mH);
				else if(estado===4) ctx.clearRect(0, 0, w, h);
			...
			case 1:
				o = 1.5*h-offset;
				imgW = w/2-image.width/2;
				imgH = o+h/2-image.height/2;

				if(estado===1 || estado===3) ctx.clearRect(0, mH, w, h);
				else if(estado===4) ctx.clearRect(0, 0, w, h);
			...
			case 2:
				o = offset-mW;
				imgW = o-image.width/2-w/2;
				imgH = h/2-image.height/2;

				if(estado===1 || estado===3) ctx.clearRect(0, 0, mW, h);
				else if(estado===4) ctx.clearRect(0, 0, w, h);
			...
			case 3:
				o = 1.5*w-offset;
				imgW = o+w/2-image.width/2;
				imgH = h/2-image.height/2;

				if(estado===1 || estado===3) ctx.clearRect(mW, 0, w, h);
				else if(estado===4) ctx.clearRect(o+w, 0, w, h);
		...
		ctx.closePath();
		ctx.fillStyle = MENU.colores[0];
		ctx.fill();

		ctx.drawImage(image, imgW, imgH);
	...
	var drawStep = [
		function() {
			// 0. De menu limpio a triangulo medio
			if(MENU.lightVersion) {
				estado++;
				time = new Date();
				onToggle&&onToggle();
				offset = ( direccion<2?(h/2):(w/2) );
				forma();
				return false;
			}

			var lapse = new Date()-time;
			var percent = lapse/tiempos[estado];
			...
			offset = percent*( direccion<2?(h/2):(w/2) );
			forma();

			return true;
		},
		function() {
			// 1. De triangulo medio a pequeño retroceso
			var lapse = new Date()-time;
			var percent = lapse/tiempos[estado];
			...
			offset = ( direccion<2?(h/2):(w/2) )-percent*10;
			forma();

			return true;
		},
		function() {
			// 2. De retroceso a triangulo medio
			var lapse = new Date()-time;
			var percent = lapse/tiempos[estado];
			...
			offset = ( direccion<2?(h/2):(w/2) )-10+percent*10;
			forma();

			return true;
		},
		function() {
			// 3. De donde estemos a nada
			var lapse = new Date()-time;

			offset -= lapse/100;
			if(MENU.lightVersion) offset = -1;
			forma();

			return estado===3;
		},
		function() {
			// 4. De donde estemos a triangulo full
			var lapse = new Date()-time;
			offset += MENU.lightVersion?(direccion<2?h:w*1.5):lapse/100;
			forma();
	...
	function drawLoop() {
		if(drawStep[estado]) 
			drawStep[estado](ctx, w, h)&&requestAnimFrame(drawLoop);
	}
...
MENU.slaidTop    = MENU.slaid(0);
MENU.slaidBottom = MENU.slaid(1);
MENU.slaidLeft   = MENU.slaid(2);
MENU.slaidRight  = MENU.slaid(3);
...
MENU.intro = (function() {
	...
	var tiempos = [2000, 2000, 1400, 2000, 2000];

	function drawCruz(ctx, w, h, x1, y1, x2, y2) {
		ctx.lineWidth = 3;
	...
	var drawStep = [
		...
		function(ctx, w, h) {
			...
			if(percent>1) {
				...
				time = new Date();
				estado++;
				$('#menu-principal-fondo-cero').fadeOut(4000);
			}

			var y1 = h-(h*percent)/2;
			var y2 = y1;

			var x1 = (w*percent)/2;
			var x2 = w-(w*percent)/2;

			drawCruz(ctx, w, h, x1, y1, x2, y2);

			return true;
		},
		function(ctx, w, h) {
			// De lineas en triangulo a circulo+lineas en triangulo
			var mH = h/2;
			var mW = w/2;

			var rChico = (w>h?h:w)/12;
			var rGrande = (w>h?h:w)/2;
			...
			drawCruz(ctx, w, h, mW, mH, mW, mH)

			ctx.beginPath();
			ctx.arc(w/2, h/2, rChico, 0, 2*Math.PI*percent, false);
			ctx.stroke();
			ctx.closePath();
		...
		function(ctx, w, h) {
			// De circulo+lineas en triangulo a todos los elementos
			var lapse = new Date()-time;
			var percent = lapse/tiempos[estado];
			...
			var y1 = h-(h*percent)/2-mH;
			var y2 = y1;

			var x1 = (w*percent)/2+mW;
			var x2 = w-(w*percent)/2-mW;
			...
			if(percent>1) {
				...
				time = new Date();
				estado++;
				$('#menu-principal-fade').css('background-color', 'rgba(255,255,255,.5)');
			}

			ctx.beginPath();
			ctx.lineWidth = 8;
			ctx.strokeStyle = 'rgba(255,255,255,'+percent*percent*.4+')';
			ctx.arc(w/2, h-rGrande-h/8, rGrande, 0, Math.PI, false);
			ctx.stroke();
			ctx.clearRect(0, 0, w, h-2*h/8);
			...
			ctx.beginPath();
			ctx.arc(w/2, h/2, rChico, 0, 2*Math.PI, false);
			ctx.stroke();
			ctx.closePath();
		...
		function(ctx, w, h) {
			// De todos los elementos a fuera lanza y dentro fade
			var lapse = new Date()-time;
			var percent = lapse/tiempos[estado];
			...
			if(percent>1) {
				...
				$('.sobre-todo').delay(1000).fadeIn(500);
				$('#menu-principal-fondo-cero').remove();
			}

			ctx.beginPath();
			ctx.lineWidth = 8;
			ctx.strokeStyle = 'rgba(255,255,255,'+((1-percent)*.4)+')';
			ctx.arc(w/2, h-rGrande-h/8, rGrande, 0, Math.PI, false);
			ctx.stroke();
			ctx.clearRect(0, 0, w, h-2*h/8);

			drawCruz(ctx, w, h, w, 0, 0, 0);
	...
	function draw(ctx, w, h) {
		...
		return drawStep[estado](ctx, w, h);
	}

	function setReady(ctx, w, h) {
		time = null;
...
EVENTOS.loadData = function(d) {
	var contenedor = $('#listado-eventos').empty();

	for(var i in d) {
		var evento = $('<li></li>').addClass('recuadro').addClass('evento');
		evento.append($('<div></div>').addClass('titulo-full').text(d[i].titulo));
		evento.append($('<div></div>').addClass('fecha-full').text(d[i].fecha));
		evento.append($('<div></div>').addClass('subtitulo-full').html(d[i].texto));
		evento.appendTo(contenedor);		

		var e = 'ontouchstart' in window?'tap':'click';
		evento.bind(e, function() {
			var h1 = $(this).children('.titulo-full').height();
			h1 += $(this).children('.fecha-full').outerHeight(true);

			if($(this).height()>h1+5)
				$(this).height(h1);
			else {
				var h2 = $(this).children('.subtitulo-full').outerHeight(true);
...
EVENTOS.ready = function() {
	if(!$('#listado-eventos').children('li').length) {
		$.getJSON('data/eventos.json', EVENTOS.loadData)
		.fail(function() { console.log( 'JSON load error' ); });
	}
};
...
GALERIA.ready = function() {
	var galeria = $('#contenedor-galeria');

	if(!galeria.children('.contenedor-imagen').length) {
		for(var i=1; i<43; i++) {
			var img = $('<img>')
				.attr('src', 'images/galeria/galeria_'+(i<10?('0'+i):i)+'.jpg')
				.addClass('imagen-contenido');
			var contenedorDeImagen = $('<div></div>')
				.addClass('contenedor-imagen-galeria');
			img.appendTo(contenedorDeImagen);
			contenedorDeImagen.appendTo(galeria);

			resize = (function(alturaInicial) {
				return function() {
					var h = $(this).children('img').height();
					var esto = $(this);
					if(esto.height() == alturaInicial) {
						esto.height(h);
						esto.css('margin', '20px 0');
					} else {
						esto.height(alturaInicial);
						esto.css('margin', '0');
					}
				};
			})(contenedorDeImagen.height());

			var evento = 'ontouchstart' in window?'tap':'click';
			contenedorDeImagen.one(evento, resize);
		}
	}
};
...
TWEETS.core = (function() {
	...
	var template = 
		'<div class="item">\
			<div class="tweet-wrapper">\
				...
				por <span class="user">{USER}</span>\
			</div>\
			{IMG}\
		</div>';

	function init(a) { api = a; }
	function setOnLoad(f) { onLoad = f; }

	function loadTweets(search, user, numTweets, appendTo){
		var request;
		if(search) {
			request = {
				q: search,
			...
			success: function(data, textStatus, xhr) {
				...
				if(data.httpstatus == 200) {
					...
					for(var i in data) {
						...
						$(appendTo).append( template.replace('{TEXT}', ify.clean(data[i].text) )
	...
	function timeAgo(dateString) {
		...
		if(diff<second*2)           return 'ahora mismo';
		if(diff<minute)             return 'hace '+Math.floor(diff / second)+' segundos';
		if(diff<minute*2)           return 'hace ~1 minuto'; 
		if(diff<hour)               return 'hace '+Math.floor(diff / minute)+' minutos';
		if(diff<hour*2)             return 'hace ~1 hora';
		if(diff<day)                return 'hace '+Math.floor(diff / hour)+' horas'; 
		if(diff>day && diff<day* 2) return 'ayer';
		if(diff<day*365)            return 'hace '+Math.floor(diff / day)+' días'; 
	...
	var ify = {
		...
		hash: function(tweet) {
			...
			return tweet.replace(/(^|\s+)#([a-zA-Zñáéíóú0-9]+)/gi, function(m, before, hash) {
	...
	return {
		loadUser: function(user, appendTo){ loadTweets(null, user, 25, appendTo); },
		loadHashTag: function(search, appendTo){ loadTweets(search, null, 25, appendTo); },
		setOnLoad: setOnLoad,
		init: init
	}
})();

TWEETS.loadHashTag = function() {
	$('#tweets-tt').empty();
	$('<div></div>').addClass('twitter-loading').appendTo($('#tweets-tt'));
	TWEETS.core.loadHashTag('elRocio13', '#tweets-tt');
};

$(function() {
	TWEETS.core.init('http://www.oxik.net/proyectos/iRocio/twitter/t.php');
	TWEETS.core.setOnLoad(function() { $('.twitter-loading').remove(); });
});
