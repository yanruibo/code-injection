...
$('#ExpertsPage').bind('pageshow', function (event) {
    $('h1').text( "LP Experts" );
});
function getExpertsList(keyword) {
   $.mobile.showPageLoadingMsg();
	$('#expertsList li').remove();
    var url = 'http://apis.liveperson.com/search.svc/1.1/jsonp?query=' + keyword +''
    var jqxhr = $.ajax({
    	 cache: true,
         type: 'GET',
         url: url,
...
var expertss;
var jsonRecord ;
function api_experts(obj) {
	var catname ;
	expertss = obj.experts;

    $.each(expertss, function (index, expert) {
    	
    	var catname = escape(expert.category.name);
        jsonRecord = encodeURIComponent(JSON.stringify(expert));
        $('#expertsList').append('<li><a  href="javascript:editPage(\''+jsonRecord+'\')"   data-rel="dialog" data-transition="pop" >' +
...
function editPage(_jsonRecord){

	var jsonRow = $.parseJSON(_jsonRecord)
	 
	$('h2').text( jsonRow.name );
	
	var rateImg = "http://www.liveperson.com/images/rating/rate" + Math.round(jsonRow.averageRating) + ".gif"
	 
	$('#expertPic').attr('src', jsonRow.thumbnail);
    $('#experties').text(jsonRow.category.name);

    $('#ratePic').attr('src', rateImg);
    $('#languages').text(jsonRow.languages);
    $('#expertdescription').text(jsonRow.description);
    $('#expertreviewsCount').text(jsonRow.reviewsCount + ' reviews ');
    $('#callfee').text(jsonRow.fee + ' per minute ');
    $('#emailfee').text(jsonRow.emailFee); 
	  
    $.mobile.changePage("#ExpertDetailsPage");
};

$("div#ExpertDetailsPage").live("pageshow", function(){
