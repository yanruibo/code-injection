...
if (!isNaN(n))
   {
     ...
     s = String(n);     s += (s.indexOf(".") == -1? ".": "") + String(Math.pow(10, nDec)).substr(1);
...
if (form.sexo[0].checked)
	{
	form.metabasal.value =bmr= redondea((66+(13.7*form.pesoc.value)+(5*form.alturacm.value)-(6.8*form.edad.value)),2);
	}
	else
	{
	form.metabasal.value =bmr= redondea((655+(9.6*form.pesoc.value)+(1.8*form.alturacm.value)-(4.7*form.edad.value)),2);
	}
		
if (form.actividad[0].checked)
	{
	form.caldiaria.value = redondea(bmr*1.2,2);
	}
	else
	{
	if (form.actividad[1].checked)
		{
		form.caldiaria.value = redondea(bmr*1.375,2);
		}
		else
		{
		if (form.actividad[2].checked)
			{
			form.caldiaria.value = redondea(bmr*1.55,2);
			}	
			else
			{
			form.caldiaria.value = redondea(bmr*1.725,2);
			}
		}
	}
