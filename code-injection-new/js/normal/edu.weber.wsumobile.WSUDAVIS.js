...
﻿
// APPLICATION STATUS EVENT HANDLERS

CUSTOM.onResume = function () {
    // executed when app is launched (or relaunched)
    if (API.debug == true) {
        console.log("");
        console.log("CUSTOM.onResume executing...");
    }
    CUSTOM.setSemester();
}
...
CUSTOM.setSemester = function () {

    if (API.debug == true) {
        console.log("");
        console.log("CUSTOM.setSemester executing...");
        //console.log("");
    }
    ...
    if (currentMonth >= 8) {
        document.getElementById("semester").innerHTML = "WSU Davis - Fall " + currentYear;
    } else if (currentMonth >= 5) {
        document.getElementById("semester").innerHTML = "WSU Davis - Summer " + currentYear;
    } else {
        document.getElementById("semester").innerHTML = "WSU Davis - Spring " + currentYear;
    }
}

CUSTOM.showService = function (app, form, pageID, containerID, unavailableID) {

    if (API.debug == true) {
        console.log("");
        console.log("CUSTOM.showService executing...");
        console.log("APP = " + app);
        console.log("TOOLKIT_FORM = " + form);
        console.log("pageID = " + pageID);
        console.log("containerID = " + containerID);
...
CUSTOM.showEvents = function (app, page, containerID, headerTitle, swatch) {
    ...
    //var request = new API.datastoreRequest({ App: app, Action: "GetEvents", Key1: "Event" });
    var request = new API.datastoreRequest({ App: app, Action: "GetData", Key1: "Event", Key2: page, StartDate: startDate });
    ...
    var callback = function (data) {
        ...
        try {
            ...
            if (events.length == 0) {
                ...
                noEvents.appendChild(document.createTextNode("(no events to display)"));
                $("#" + containerID + "")[0].appendChild(noEvents);
            }
            ...
            for (var i = 0; i < events.length; i++) {
                ...
                var eventDate = document.createElement("p");
                eventDate.appendChild(document.createTextNode(events[i].getAttribute("day") + " " + $("time", events[i])[0].firstChild.nodeValue));
                eventDate.style.fontSize = "12px";
                eventDate.style.marginLeft = "20px";
                li.appendChild(eventDate);
                ...
                page.id = "pg_" + events[i].getAttribute("RecordNumber");
                page.setAttribute("data-url", page.id);
                page.setAttribute("data-add-back-btn", "true");
...
API.showHighlightedStudentActivities = function (pageID, containerID, unavailableID, fIncrement, fDecrement) {
    API.showHighlightedGoogleCalendar(pageID, containerID, unavailableID, "Activity", "No activities listed...", fIncrement, fDecrement, 90, "SHARED/Calendars/SHARED_Activities.xml");
        ...
        API.app = "WSUDAVIS";
        API.appType = "native";
        API.appReady = "edu_weber_wsumobile_WSUDAVIS_Ready";
        API.appRefreshed = "edu_weber_wsumobile_WSUDAVIS_Refreshed";
        ...
        var f4home = function () {
            API.showPage(CUSTOM.Home); // URL may be hard-coded or (preferably) constant defined in Toolkit
        }
        var f5home = function () { 
        ...
        $("#home").bind("pagebeforecreate", function () {
            ...
            var image1024 = "images/Student-1024.png";
            var image768 = "images/Student-768.png";
            var image480 = "images/Student-480.png";
            var image320 = "images/Student-320.png";
        ...
        var f0campus = function () {
            API.showPage(CUSTOM.Map);
        }
        $("#campus").bind("pagebeforecreate", function () {
            $("#campus a").each(function () {
                this.onclick = eval(this.getAttribute("data-onlineclick"));
            });
        });
        $("#campus").bind("pagebeforeshow", function () {
            if ($("#campus").attr("data-dirty") == "true") {
                try {
                    API.showUnstructuredContent("WSUDAVIS", "Campus Information", "campus", "div0campus", "p0campus");
                }
                catch (e) {
                    $("#p0campus").removeClass("hidden");
                    if (API.debug == true) {
    ...
    var f2news_submenu = function () {
        $.mobile.changePage("#student_activities");
    }
    var f3news_submenu = function () {
        $.mobile.changePage("#workshops");
    }
    var f4news_submenu = function () {
        $.mobile.changePage("#sms");
    }
    var f5news_submenu = function () {
        $.mobile.changePage("#wsutoday");
    }
    var f6news_submenu = function () {
    ...
    $("#davis_activities").bind("pagebeforeshow", function () {
        if ($("#davis_activities").attr("data-dirty") == "true") {
            try {
                // DETAIL_HEADER is the header that should appear on the detail pages
                // NO_EVENTS is the message that should be displayed if there are no events on the calendar
                ...
                // DAYS is the number of days that should be included in the calendar feed
                // CONFIG_FILE is the url of the configuration file relative to http://m.weber.edu/platform2/apps/ (e.g., "VERSION2/calendars/VERSION2.xml")
                API.showHighlightedGoogleCalendar("davis_activities", "div0davis_activities", "p0davis_activities", "Activities", "No activities posted", API.doNothing, API.doNothing, 120, "WSUDAVIS/Calendars/WSUDAVIS.xml");
            }
            catch (e) {
                $("#p0davis_activities").removeClass("hidden");
                if (API.debug == true) {
                    console.log(e.message);
                }
            }
    ...
    $("#student_activities").bind("pagebeforeshow", function () {
        if ($("#student_activities").attr("data-dirty") == "true") {
            try {
                // fINCREMENT is the function that should be executed when a new item is found; typically it will increment a counter
                // fDECREMENT is the function that should be executed when a new item is viewed; typically it will decrement a counter
                API.showHighlightedStudentActivities("student_activities", "div0student_activities", "p0student_activities", API.doNothing, API.doNothing);
            }
            catch (e) {
                $("#p0student_activities").removeClass("hidden");
                if (API.debug == true) {
                    console.log(e.message);
                }
            }
    ...
    $("#workshops").bind("pagebeforeshow", function () {
        if ($("#workshops").attr("data-dirty") == "true") {
            try {
                // DETAIL_HEADER is the header that should appear on the detail pages
                //API.showAgenda("WSUDAVIS", "Workshops", "workshops", "div0workshops", "p0workshops", "Workshop");
                CUSTOM.showEvents("WSUDAVIS", "Workshops", "div0workshops", "Workshop", "a");
                $("#div0workshops").removeClass("hidden");
                $("#workshops").attr("data-dirty", "false");
            }
            catch (e) {
                $("#p0workshops").removeClass("hidden");
                if (API.debug == true) {
                    console.log(e.message);
                }
            }
    ...
    $("#sms").bind("pagebeforeshow", function () {
        if ($("#sms").attr("data-dirty") == "true") {
            try {
                API.showUnstructuredContent("WSUDAVIS", "Text Messaging", "sms", "div0sms", "p0sms");
            }
            catch (e) {
    ...
    $("#signpost").bind("pagebeforeshow", function () {
        ...
        $("#services").bind("pagebeforecreate", function () {
            $("#services ul a").each(function () {
                $(this).bind("click", function () {
                    CUSTOM.service = this.getAttribute("data-form");
                    $.mobile.changePage("#service_display");
                });
            });
        });
    

    $("#service_display").bind("pagebeforeshow", function () {
        if ($("#service_display").attr("data-dirty") == "true") {
            try {
                CUSTOM.showService("WSUDAVIS", CUSTOM.service, "service_display", "div0service_display", "p0service_display");
            }
            catch (e) {
                $("#p0service_display").removeClass("hidden");
                if (API.debug == true) {
                    console.log(e.message);
                }
            }
