...
Ext.define('MyApp.model.Search', {
	...
	fillFromForm: function(formValues){
		...
		*/
		...
		if(formValues['idagence']){
			this.set('idagence',formValues['idagence']);
		}
		if(formValues['selectall']){
			this.set('selectall',formValues['selectall']);
		}
...
Ext.define('MyApp.model.ListAgence', {
	extend: 'Ext.data.Model',
	...
	config: {
		fields: [
			{ name:'returncode', type:'string' },
			{ name:'returnmessage', type:'string' },
			{ name:'nbagence', type:'int' },
		],
		hasMany: { model:'MyApp.model.Agence', name:'agence', associationKey:'data' }
	}
});

Ext.define('MyApp.model.Agence', {
	extend: 'Ext.data.Model',
	...
	config:{
		fields: [
			{name:'num',type:'int'},
			{name:'nom',type:'string'},
			{name:'nomUnformated',type:'string'},
			{name:'adresse',type:'string'},
			{name:'cp',type:'int'},
			{name:'ville',type:'string'},
			{name:'tel',type:'string'},
...
Ext.define('MyApp.store.AgencyHighlight', {
    ...
    config:{
        ...
        proxy:{
            ...
            // PRODUCTION
            url:'http://iphone.acheter-louer.fr/perl/agences/smartphones/multi-gallerie.pl',
    ...
    config:{
        ...
        proxy: {
                ...
                api:{
                    ...
                    // PRODUCTION
                    read:"http://iphone.acheter-louer.fr/perl/agences/smartphones/multi-info.pl",
...
Ext.define('MyApp.store.SearchDetail', {
    ...
    config:{
        ...
        proxy: {
            ...
            // PRODUCTION
            url: 'http://iphone.acheter-louer.fr/perl/agences/smartphones/multi-searchfiche.pl',
...
Ext.define('MyApp.store.ListAgence', {
    extend:'Ext.data.Store',
    // itemId:'SearchResults',

    config:{
        
        model:'MyApp.model.ListAgence',
        ...
        proxy: {
            ...
            // PRODUCTION
            url:'http://iphone.acheter-louer.fr/perl/agences/smartphones/listagence.pl',
...
Ext.define('MyApp.controller.Main', {
	...
	config: {
		refs: {
			...
			highLightGoButtonContainer:'#highlightgobuttoncontainer',
			...
			realEstateThumbnailCarousel:'#thumbnail-carousel',
			...
			agencyPageButAgencyEmail:'button#agencyPageButAgencyEmail',
			...
			listAgencyPage:'#listagencypage',
			listAgencyList:'list#listagency',
            butAgencyPhone:'button#butAgencyPhone',
			buttonSelectAgenceRealestate:'button#buttonSelectAgenceRealestate',
            ...
            butDetailPhone:'button#butDetailPhone',            
		...
		control:{
			...
			'button#buttonSelectAgenceRealestate':{
				tap: 'onTapButtonSelectAgenceRealestate'
			},
			...
			'button#agencyPageButAgencyEmail':{
				tap:'onAgencyPageRequestInfo'
			},
           ...
           // 'button#butDetailPhone':{
				//tap:'onTapbutDetailPhone' 
                		//	},
			...
			'button#submit-request-appraisal':{
				tap: function(){
					...
					this.submitFormToModelAndPop(
						this.getRequestAppraisalForm(),
			...
			'button#submit-request-searchmandate':{
				tap: function(){
					var agencymodel = Ext.ComponentQuery.query('#agencypage')[0].model;
					this.submitFormToModelAndPop(
						this.getRequestSearchMandateForm(),
			...
			'#highlightmoreinfo button#submit-request-moreinfo':{
				...
				tap: function(button){
					...
					this.submitFormToModelAndPop(
						this.getHighlightRequestAdMoreInfo(),
			...
			'#agencyrequestinfo button#submit-request-info':{
				scope:this,
				tap: function(button){
					//console.log('#agencyrequestinfo button#submit-request-info');
					var agencymodel = Ext.ComponentQuery.query('#agencypage')[0].model;
					//console.log('IdAgence is ' + agencymodel.get('idagence'));
					this.submitFormToModelAndPop(
						this.getAgencyRequestInfo(),
			...
			'#searchrequestadmoreinfo button#submit-request-moreinfo':{
				...
				tap: function(){
					// console.log('button#submit-request-moreinfo');
					this.submitFormToModelAndPop(
						this.getSearchResultRequestAdMoreInfo(),
						'MyApp.model.RequestAdMoreInfo',
						"Demande envoy&eacute;e",
						null
					);
			...
			'#favoriterealestatemoreinfo button#submit-request-moreinfo':{
				...
				tap: function(){
					// console.log('button#submit-request-moreinfo');
					this.submitFormToModelAndPop(
						this.getFavoriteRealEstateRequestAdMoreInfo(),
						'MyApp.model.RequestAdMoreInfo',
						"Demande envoy&eacute;e",
						null
					);
			...
			/* AGENCY_SECTION */
			'agencysection_phone':{
				back: 'onAgencySectionViewBack',
				push: 'onAgencySectionViewPush',
				pop: 'onAgencySectionViewPop',
			},
			...
			listAgencyList:{
				select: 'onListAgencyListItemSelect',
			},
            ...
            '#searchresultsdetail button#searchDetailButAgencyPhone':{
				tap: 'onTapSearchDetailButAgencyPhone'
			},
			'#highlight-detail button#searchDetailButAgencyPhone':{
				tap: 'onTapHighLightDetailButAgencyPhone'
			},
			'#favoriterealestatedetail button#searchDetailButAgencyPhone':{
				tap: 'onTapFavoriteRealEstateDetailButAgencyPhone'
			},
	...
	launch: function(){
		...
		var agencyHighlightStore = Ext.getStore('AgencyHighlight'),
			...
			listAgenceStore = Ext.getStore('ListAgence'),
			...
			callback: function(records,operation,success){
				if (success){
					...
					if (currentCachedModel){
						...
						cachedVitrineUrl = currentCachedModel.get('vitrine');
						//console.log('MyApp.controller.Main - agencyDetailServerStore.load()');
					}
					...
					if (!cachedLogoPath || !cachedVitrinePath || currentModel.get('logo') != cachedLogoUrl || currentModel.get('vitrine') != cachedVitrineUrl){
						//console.log("MyApp.controller.Main agencyDetailServerStore() - logo different or no logo_cache property, get the file");
						if (gIMAGECACHE_DIR){
							...
							fileTransfer.download(
								...
								function(entry){
									//console.log("file transfer du logo success to : "+entry.fullPath);
...
// 									console.log("download error code" + error.code);
							...
							fileTransfer.download(
								...
								function(entry){
// 									console.log("file transfer de la vitrine success to : "+entry.fullPath);
...
// 					console.log('failed load agencydetails');
		...
		listAgenceStore.load({
			...
			callback: function(records,operation,success){
				if (success){
					this.onListAgenceStoreLoad();
				}else{
// 					console.log('failed to load listagence');
				}
			}
		});
	},
	...
	updateAgencyDetail: function(serverAgencyDetail){
		...
		//console.log('Fin updateAgencyDetail');
	},

	// fillModelFunction is updateRecord by default, but one may want to do that manually
	submitFormToModelAndPop: function(theForm, modelName, onSuccessMessage, navigationView, fillModelFunction, idagence){
		//console.log('MyApp.controller.Main - submitFormToModelAndPop()');
		var thisIdagence;
		if(idagence){
			thisIdagence=idagence;
		}else{
			thisIdagence=this.global_idagence;
		}
		//console.log('MyApp.controller.Main - submitFormToModelAndPop() - idagence is ' + thisIdagence);
		var model = Ext.create(modelName, { 'idagence':thisIdagence }),
			alias = this,
 ...
 onTapSearchDetailButAgencyPhone:function(){
		console.log('MyApp.controller.Main - onTapSearchDetailButAgencyPhone()');
		var realestatedetail = Ext.ComponentQuery.query('#searchresultsdetail')[0];
		var realEstateDetailModel=realestatedetail.getRecord();
		var MB = Ext.MessageBox;
		Ext.apply(MB, {
				YES: { text: 'Appeler', itemId: 'yes', ui: 'action' }
		});
		...
		var msg = Ext.Msg.confirm(realEstateDetailModel.get('telAgence'),'',
			function(r){
				if(r == 'yes'){
					var regExp=/ /g;
					var tel=realEstateDetailModel.get('telAgence').replace(regExp,'');
					if (device.platform=='iPhone'){
						window.plugins.PhoneDialer.dial(tel);
						//window.plugins.PhoneDialer.dial('0160929604');
					}else{
						document.location.href = 'tel:' + realEstateDetailModel.get('telAgence');
					}
				}
			}
		);
	},
	onTapHighLightDetailButAgencyPhone:function(){
		console.log('MyApp.controller.Main - onTapHighLightDetailButAgencyPhone()');
		var realestatedetail = Ext.ComponentQuery.query('#highlight-detail')[0];
		var realEstateDetailModel=realestatedetail.getRecord();
		var MB = Ext.MessageBox;
		Ext.apply(MB, {
				YES: { text: 'Appeler', itemId: 'yes', ui: 'action' }
	...
	onTapFavoriteRealEstateDetailButAgencyPhone:function(){
		console.log('MyApp.controller.Main - onTapFavoriteRealEstateDetailButAgencyPhone()');
		var realestatedetail = Ext.ComponentQuery.query('#favoriterealestatedetail')[0];
		var realEstateDetailModel=realestatedetail.getRecord();
		var MB = Ext.MessageBox;
		Ext.apply(MB, {
				YES: { text: 'Appeler', itemId: 'yes', ui: 'action' }
 ...
 /*
    ...
    onTapbutDetailPhone:function(button){
  //console.log('MyApp.controller.Main - onTapButAgencyPhone()');
 
                                       
        var realestatedetail = Ext.ComponentQuery.query('#searchresultsdetail')[0];
	
		var realEstateDetail=realestatedetail.getRecord();
		var MB = Ext.MessageBox;
        // console.log(realestatedetail.get('telAgence'));
		...
		var msg = Ext.Msg.confirm(realEstateDetail.get('telAgence'),'',
			function(r){
				if(r == 'yes'){
					var regExp=/ /g;
					var tel=realEstateDetail.get('tel').replace(regExp,'');
					if (device.platform=='iPhone'){
						window.plugins.PhoneDialer.dial(tel);
						//window.plugins.PhoneDialer.dial('0160929604');
					}else{
						document.location.href = 'tel:' + realEstateDetail.get('telAgence');
					}
				}
			}
		);
	...
	onTapButtonSelectAgenceRealestate: function(){
		//console.log('MyApp.controller.Main - onTapButtonSelectAll() - need override');
	},
	...
	onTapButtonHighlightGo: function(){
		...
		var highLightGoButtonContainer=this.getHighLightGoButtonContainer();
		highLightGoButtonContainer.setMasked(true);
	...
	onListAgenceStoreLoad: function(){
		var listResults = Ext.getStore('ListAgence').getAt(0),
			listagencypage = this.getListAgencyPage();
		if(listagencypage){
			//var listagence = Ext.create('MyApp.view.phone.ListAgencyPage');
			listagencypage.listAgencyFill(listResults.agence());
			this.onListAgencyFilled(listResults.agence());
			//console.log(listResults.agence());
			//console.log('Fin listAgenceStore');
		}
	},
	onListAgencyFilled: function(){
		//console.log('MyApp.controller.Main onListAgencyFilled() - need override');
	},
	...
	onStartSearch: function(){
		...
		// Load page 1
		if(Ext.ComponentQuery.query('#topPanelSearchSubmitButton')[0]){
			var topPanelSearchSubmitButton = Ext.ComponentQuery.query('#topPanelSearchSubmitButton')[0];
			topPanelSearchSubmitButton.setMasked(true);
			var bottomPanelSearchSubmitButton = Ext.ComponentQuery.query('#bottomPanelSearchSubmitButton')[0];
			bottomPanelSearchSubmitButton.setMasked(true);
		}
		var searchForm = this.getSearchForm();
		var formValues = searchForm.getValues();
		if(formValues['idagence'] == null){
			searchForm.setValues({
				selectall:1,
			});
		}else{
	...
	onSearchResultsListPreviousPage: function(){
		...
		var currentpage = Ext.getStore('SearchResults').getAt(0).get('page'),
			// Si appele avec idagence
			idagence = Ext.getStore('SearchResults').getAt(0).get('idagence');
		
		if (currentpage<=1){
			// alert('Premiere page');
		}else{
			this.doRequestSearchForPage(currentpage-1,idagence);
		}
	},
			...
			slots: [
				{
					...
					data : [
						{text: 'Prix', value: 0},
						{text: 'Ville', value: 1},
						{text: 'Nbre de pi&egrave;ce', value: 2},
	...
	onChooseTri:function(picker, selectedObject, eOpts){
		var searchForm = this.getSearchForm(),
			searchResults = Ext.getStore('SearchResults').getAt(0),
			idagence = searchResults.get('idagence');
		searchForm.setValues({
			tri: selectedObject['Tri'],
			idagence: idagence,
		});
		this.doRequestSearchForPage(1,idagence);
	},
	doRequestSearchForPage: function(page,idagence){
		...
		if(idagence){
			searchParams.set('idagence', idagence);
		}
	...
	onSearchResultsLoaded: function(){
		...
		var searchResults = Ext.getStore('SearchResults').getAt(0),
			currentpage = searchResults.get('page'),
			nbpages = searchResults.get('nbpages'),
			agence = searchResults.get('agence'),
		...
		// Si lance avec idagence
		if(agence){
			var agencyname=this.getSearchResultListAgencyName();
			var searchformagencyname=this.getSearchFormAgencyName();
			//console.log(searchformagencyname);
			if(agencyname){
				agencyname.setHtml(agence);
			}else if(searchformagencyname){
				searchformagencyname.setHtml(agence);
			}
		}
	...
	onSearchResultsListItemSelect: function(dataview, record, eOpts){
		...
		var searchresultslistpanel = Ext.ComponentQuery.query('#searchresultslistpanel')[0];
		searchresultslistpanel.setMasked(true);
	...
	onListAgencyListItemSelect:function(dataview, record, eOpts){
		console.log('MyApp.controller.Main - onListAgencyListItemSelect - need override');
	},
...
Ext.define('MyApp.view.phone.SearchForm', {
	...
	config:{
		...
		listeners: {
			painted:{
				fn : function() {
					var topPanelSearchSubmitButton = Ext.ComponentQuery.query('#topPanelSearchSubmitButton')[0];
					topPanelSearchSubmitButton.setMasked(false);
					var bottomPanelSearchSubmitButton = Ext.ComponentQuery.query('#bottomPanelSearchSubmitButton')[0];
					bottomPanelSearchSubmitButton.setMasked(false);
				},
				scope: this
			}
		},
		items : [
			...
			},
			{
				...
				itemId: 'topPanelSearchSubmitButton',
				items:[
					// Button Submit
				...
				items:[
					{
						...
						label:'Agence',
						name:'idagence',
						itemId:'agenceSelectField',
						readOnly:false
					},
				]
			},
			{ xtype: 'numberfield', name: 'tri', label: "tri", hidden: true, readOnly: true },
			{ xtype: 'numberfield', name: 'selectall', label: "selectall", hidden: true, readOnly: false },
			// Localite
			{
				xtype: 'panel',
				html: 'Localit&eacute;',
			...
			// Button Submit
			{
				...
				itemId: 'bottomPanelSearchSubmitButton',
				items:[
					// Button Submit
					{ xtype:'button', itemId:'submit-search', text:'Lancer la recherche', cls:['submit-search']},
				],
	...
	fillAgence: function(listAgenceStore){
		//console.log('MyApp.view.tablet.SearchForm - fillAgence() : ' + listAgenceStore);
		var agenceSelectField=this.down('#agenceSelectField');
		var items=[
				{
		   ...
		   i=0;
		for(i;i<listAgenceStore.getCount();i++){
			//console.log('MyApp.view.tablet.SearchForm - fillAgence() : ' + listAgenceStore.getAt(i).getData().nom);
			//console.log(listAgenceStore.getAt(i));
			var nom=listAgenceStore.getAt(i).getData().nomUnformated;
			items.push(
				{
					text:nom,
					value:listAgenceStore.getAt(i).getData().num
				}
			);
			//console.log('MyApp.view.tablet.SearchForm - fillAgence() : data is ');
			//console.log(data.nom);
		}
		agenceSelectField.setOptions(items);
	},
});
	...
	config:{
		...
		listeners: {
			painted:{
				fn : function() {
					var searchresultslistpanel = Ext.ComponentQuery.query('#searchresultslistpanel')[0];
					searchresultslistpanel.setMasked(false);
				},
				scope: this
			}
		},
...
Ext.define('MyApp.view.phone.ListAgencyPage', {
	extend: 'Ext.Panel',
	xtype: 'listagencypage_phone',
	config:{
		...
		listeners: {
			painted:{
				fn : function() {
					var listagencepanelcontainer = Ext.ComponentQuery.query('#listagencepanel')[0];
					listagencepanelcontainer.setMasked(false);
				},
				scope: this
			}
		},
				...
				items: [
					{
						...
						itemTpl: [
							'<div>',
								"<tpl if='logo' != 'null'>",
									'<img src="{logo}" class="listagencelogo phone">',
								"</tpl>",
								'<div>',
									'<div class="listagencename phone">{nom}','</div>',
									'<div class="listagenceadresse phone">','{adresse}','</div>',
									'<div class="listagencelocation phone">{ville}({cp})','</div>',
									'<div class="listagencetel phone"><img class="listAgencePictoTel phone" />  {tel}','</div>',
								'</div>',
							'</div>',
						],
					}
	...
	listAgencyFill:function(listAgenceStrore){
		var listagency=this.down('#listagency');
		if(listagency){
			listagency.setStore(listAgenceStrore);
			//console.log('MyApp.view.phone.ListAgencyPage - listAgencyFill');
		}
	}
});
Ext.define('MyApp.view.phone.AgencyMap', {
	...
	initGMapWithAgencyModel:function(model){
		...
		var address=agencyDetailModel.get("adresse")+' '+agencyDetailModel.get("cp")+' '+agencyDetailModel.get("ville")+' '+'FRANCE';
...
Ext.define('MyApp.view.tablet.ListAgencyPage', {
	extend: 'Ext.Panel',
	xtype: 'listagencypage_tablet',
	config:{
		firstTimePainted:true,
		cls:['listagencypage','tablet'],
		scrollable:{
			direction:'vertical',
			directionLock:true,
		},
	...
	listAgencyFill:function(listAgenceStrore){
		//console.log('MyApp.view.tablet.ListAgencyPage');
		var listagency=this.down('#listagency');
		if(listagency){
			listagency.setStore(listAgenceStrore);
			//console.log('MyApp.view.tablet.ListAgencyPage - listAgencyFill');
		}
	}
});
Ext.define('MyApp.view.HomePage', {
	...
	this.down('#homepageagencyname').setHtml(highlightModel.get('nom'));
    },
});
...
Ext.define('MyApp.view.tablet.SearchForm', {
	...
	updateFromModel:function(agencymodel){
		//console.log('MyApp.view.tablet.SearchForm - updateFromModel()');
		this.down('#searchformagencyname').setHtml(agencymodel.get('nom'));
		var agenceSelectField=this.down('#agenceSelectField'),
			agenceSelectFieldOptions=agenceSelectField.getOptions(),
			items=[
				{
					text:'Toutes les agences',
					value:null,
			...
			i=0;
		for(i;i<agenceSelectFieldOptions.length;i++){
			items.push(
				{
					text:agenceSelectFieldOptions[i].text,
					value:agenceSelectFieldOptions[i].value
				}
			);
		}
		agenceSelectField.setOptions(items);
		agenceSelectField.setValue(agencymodel.get('idagence'));
	}
});
...
Ext.define('MyApp.controller.phone.Main', {
	...
	onTapButtonSelectAgenceRealestate: function(){
		...
		searchForm.setValues({
				tri:0,
				idagence: agencymodel.get('idagence'),
		});
		var tabpanel = Ext.ComponentQuery.query('#apptabpanel')[0];
		if(tabpanel){
			this.resetSearchSectionToForm();
	...
	showSelectAllResultList: function(){
		   //console.log('MyApp.controller.phone.Main - showSearchResultList()');
	...
	onTapButtonAgencyEmail:function(){
		...
		moreInfoForm.setIdAgence(agencymodel.get('idagence'));
	...
	onSearchNavigationViewPop: function(view, item){
		...
		if (item.xtype == "searchresultslist_phone" || item.xtype == "requestadmoreinfo_phone" || item.xtype =="carousel" || item.xtype == "realestatemap_phone"){
			this.hideSearchresultsListButtonSave();
		}else{
			this.showSearchresultsListButtonSave();
		}
	...
	// Fired when a real agency is selected in ListAgence
	createAgencyDetailView: function(){
		return Ext.create('MyApp.view.phone.AgencyPage');
	},
	...
	onListAgencyFilled: function(listAgenceStore){
		// Fill SearchForm
		var searchform= this.getSearchForm();
		searchform.fillAgence(listAgenceStore);
	},
	onListAgencyListItemSelect:function(dataview, record, eOpts){
		var panelListBloc = Ext.ComponentQuery.query('#listagencepanel')[0];
		panelListBloc.setMasked(true);
		agencyDetailServerStore = Ext.getStore('AgencyDetailServer');
		agencyDetailServerStore.load({
			scope:this,
			params:{
				idagence : record.get('num')
			},
			callback: function(records,operation,success){
				if (success){
					//console.log("agencyDetailServerStore.load success : " + record.get('logo'));
					var agencyPageView=this.createAgencyDetailView();
					agencyPageView.setAgencyModel(records[0]);
					var agencynavigationview = this.getAgencyNavigationView();
					agencynavigationview.push(agencyPageView);
				}
			}
		});
	},
	onAgencySectionViewBack:function(view){
		var item = view.getActiveItem();
		//console.log('MyApp.controller.phone.Main - onAgencySectionViewBack() - item.type is : ' + item.xtype);
		if (item.xtype == "listagencypage_phone" || item.xtype == "requestsearchmandate_phone" || item.xtype == "requestappraisal_phone" || item.xtype == "requestagencyinfo_phone" || item.xtype == "requestadmoreinfo_phone" || item.xtype == "requestaddeposit_phone"){
			this.hideSelectAllButton();
		}else if(item.xtype == "agencypage_phone"){
			this.showSelectAllButton();
		}
	},
	onAgencySectionViewPop:function(view){
		var item = view.getActiveItem();
		//console.log('MyApp.controller.phone.Main - onAgencySectionViewBack() - item.type is : ' + item.xtype);
		if (item.xtype == "listagencypage_phone" || item.xtype == "requestsearchmandate_phone" || item.xtype == "requestappraisal_phone" || item.xtype == "requestagencyinfo_phone" || item.xtype == "requestadmoreinfo_phone" || item.xtype == "requestaddeposit_phone"){
			this.hideSelectAllButton();
	...
	onAgencySectionViewPush:function(view, item){
		//console.log('MyApp.controller.phone.Main - onAgencySectionViewBack()');
		if (item.xtype == "agencypage_phone"){
			this.showSelectAllButton();
		}else if(item.xtype == "listagencypage_phone" || item.xtype == "requestsearchmandate_phone" || item.xtype == "requestappraisal_phone" || item.xtype == "requestagencyinfo_phone" || item.xtype == "requestadmoreinfo_phone" || item.xtype == "requestaddeposit_phone"){
			this.hideSelectAllButton();
		}
	},
	showSelectAllButton:function(){
		// Show selectAllButton
		var selectAllButton = Ext.ComponentQuery.query('#buttonSelectAgenceRealestate')[0];
		// Show the selectAll button
		selectAllButton.show();
		Ext.Animator.run({
			element: selectAllButton.element,
			from: {
				opacity: 0
			},
			to: {
	...
	onListAgencyListItemSelect:function(dataview, record, eOpts){
		...
		agencyDetailServerStore.load({
			...
			callback: function(records,operation,success){
				if (success){
					//console.log("agencyDetailServerStore.load success : " + record.get('logo'));
					var agencyPage=this.getAgencyPage();
					agencyPage.setAgencyModel(records[0]);
				}else{
					console.log('MyApp.controller.tablet.Main - onListAgencyListItemSelect() - error agencyDetailServerStore.load');
				}
			}
		});
	},
...
Ext.define('MyApp.view.phone.HomePage', {
	...
	config:{
		...
		listeners: {
			painted:{
				fn : function() {
					var highlightgobuttoncontainer = Ext.ComponentQuery.query('#highlightgobuttoncontainer')[0];
					highlightgobuttoncontainer.setMasked(false);
				},
				scope: this
			}
		},
		items:[
			...
			// CAROUSEL
			{
				...
				items:[
					...
					{
						xtype:'panel',
						itemId:'highlight-agence',
						cls:['highlight-agence','phone'],
					},
				],
			},
			{
				...
				itemId:'highlightgobuttoncontainer',
				cls:['underbottomsection','phone'],
				items:[
					{
						xtype:'button',
						itemId:'highlight-go',
	...
	setActiveHighlight: function(highlightModel){
		...
		this.down('#highlight-agence').setHtml(highlightModel.get('nom'));
	},
...
Ext.define('MyApp.view.phone.RealEstateDetail', {
	...
	requires:[
        ...
        'MyApp.model.RealEstate',
  
	],

	config:{
        itemId:'realestatedetail',
		title:'Annonce',
        ...
        listeners: {
			painted:{
				fn : function() {
					var panelImageBloc = Ext.ComponentQuery.query('#panelimagebloc')[0];
					panelImageBloc.setMasked(false);
				},
				scope: this
			}
		},
			...
			},
			{
				...
				items: [
					...
					{ xtype:'panel',
						...
						padding:'0 10 10 0',
						items:[
							{ xtype:'button', flex:1, text:'Appeler', itemId:'searchDetailButAgencyPhone', cls:['buttongreenPhone','phone'],},
...
Ext.define('MyApp.view.phone.AgencySection', {
	...
	requires:[
		'MyApp.view.phone.ListAgencyPage',
	...
	config:{
		...
		navigationBar: {
			items: [
				{
					xtype:'button',
					itemId:'buttonSelectAgenceRealestate',
					align:'right',
					text:'Nos annonces',
					hidden:true,
				},
...
Ext.define('MyApp.view.tablet.AgencyPage', {
		...
		items:[
			{
				...
				items:[
					...
					// Left Vitrine bloc + Right Buttons
					{
						xtype:'panel',
						width:'100%',
						...
						items:[
							{
								...
								items:[
									...
									{
										xtype:'panel',
										itemId:'agencyPageAgencyShopAgencyInfoSpacer',
									},
									// Le bloc des infos
									{
										xtype:'panel',
										itemId:'agencyLeftBlocInfo',
										layout:{
											type:'vbox',
										},
										items:[
											{ xtype:'spacer', height:5 },
											{ xtype:'image', itemId:'agencyLogoImage', mode:'image', cls:['listagencelogo','phone']},
											{xtype:'panel',itemId:'agencyName',cls:['listagencename','phone']},
											{xtype:'panel',itemId:'agencyContact',cls:['listagencecontact','phone']},
											{xtype:'panel',itemId:'agencyAddress',cls:['listagenceadresse','phone']},
											{xtype:'panel',itemId:'agencyLocation',cls:['listagencelocation','phone']},
											{
												xtype:'panel',
												layout:'hbox',
												items:[
											...
											{ xtype:'panel', html:'RCS', itemId:'agencyRCS', cls:['listagencercs','phone'] },
										],
									},
								],
							},
										...
										items:[
											{
												...
												itemId:'buttonSelectAgenceRealestate',
												text:'Nos annonces',
												cls:['buttonSelectAgenceRealestate','tablet'],
												docked:'right',
											},
										]
									},
					...
					},
					{
						...
						cls:['agencypage-button-bloc-forms','tablet'],
						items:[
							{
								xtype:'panel',
								layout:{
								...
								items:[
									{
										xtype:'button',
										ui:'normal',
										text:"Demande d'estimation",
										itemId:'buttonRequestAppraisal',
										cls:['buttonRequestAppraisal','tablet']
									},
									{
										xtype:'button',
										ui:'normal',
										text:"D&eacute;p&ocirc;t d'annonce",
										itemId:'buttonRequestAdDeposit',
										cls:['buttonRequestAdDeposit','tablet']
									},
									{
										xtype:'button',
										ui:'normal',
										text:"Mandat de recherche",
										itemId:'buttonRequestSearchMandate',
										cls:['buttonRequestSearchMandate','tablet']
									}
								]
							},
							{
								...
								items:[
									{
										xtype:'button',
										itemId:'agencyPageButAgencyEmail',
	...
	initGMapWithAgencyModel:function(model){
		var agencyMap = this.down("#agencyMap");
		var agencyDetailModel = model;
		var pos = new google.maps.LatLng(agencyDetailModel.get("lat"),agencyDetailModel.get("lng"));
		var googleMap=new Ext.Map({
			title: 'Map',
			cls:['map','tablet'],
			width:'100%',
		...
		var mapGoogleMap=googleMap.getMap();
		this.agencyMarker = new google.maps.Marker({
			map:mapGoogleMap,
			position: pos,
			animation:google.maps.Animation.DROP,
...
// 		var infoWindowContent='<div id="content" style="width:200px; height:20px; font-size:12px; font-weight:bold;">' + agencyDetailModel.get("nom") + '</div>';
...
// 			visible:true,
// 		});
		agencyMap.setItems(googleMap);
		mapGoogleMap.panTo(pos);
	},
	initialize: function() {
		...
		this.addListener('painted', function(){
			//do the centering once the view is painted, otherwise the map is not centered properly
			if (this.model){
				//console.log('MyApp.view.tablet.AgencyPage - initialize - ' + this.model);
...
// 		}
		this.model = agencyDetailModel;
		//console.log('MyApp.view.tablet.AgencyPage setAgencyModel');
		//console.log(agencyDetailModel);
		if (agencyDetailModel){
			this.down('#agencyName').setHtml(agencyDetailModel.get('nom'));
			this.down('#agencyAddress').setHtml(agencyDetailModel.get('adresse'));
			this.down('#agencyLocation').setHtml(agencyDetailModel.get('ville') + '(' + agencyDetailModel.get('cp') + ')');
			...
			if (agencyDetailModel.get('vitrine')){
				this.down('#agencyShopImage').setSrc(agencyDetailModel.get('vitrine'));
				this.down('#agencyShopImage').show();
				this.down('#agencyPageAgencyShopAgencyInfoSpacer').setWidth(0);
			}else{
				this.down('#agencyShopImage').hide();
				this.down('#agencyPageAgencyShopAgencyInfoSpacer').setWidth(5);
			}
...
Ext.define('MyApp.view.tablet.TwoPanesListAndAgency', {
	extend:'Ext.Panel',
	xtype:'twopaneslistandagency_tablet',

	requires:[
		'MyApp.view.tablet.ListAgencyPage',
		'MyApp.view.tablet.AgencyPage',
	],    

	config:{
		itemId:'twopaneslistandagency',
		layout:'hbox',
		items: [
			{
				xtype:'listagencypage_tablet',
...
Ext.define('MyApp.view.tablet.AgencySection', {
	...
	requires:[
		'MyApp.view.tablet.TwoPanesListAndAgency',
		'MyApp.view.tablet.ListAgencyPage',
		'MyApp.view.tablet.AgencyPage',
	],
...
Ext.application({
    ...
    models: [
        ...
        'Search','SearchDetail','SearchResults','SearchNotification','RealEstate','Location','RealEstateThumbnails','ListAgence','Agence'
    ],
    stores: [
        'AgencyHighlight','AgencyDetailServer','AgencyDetailCache',
        'SearchResults','SearchDetail',
        'FavoriteRealEstate','FavoriteSearch',
        'Location','RealEstateThumbnails','ListAgence'
    ],
    controllers:[],
    viewport: {
        autoMaximize:false,
