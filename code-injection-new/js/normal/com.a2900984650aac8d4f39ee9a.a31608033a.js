    		...
    		$(document).ready(function() {
                $("#shareButtonLink").click(function() {
                    window.plugins.share.show({
                        subject: "Share Antony Santos",
                        text: "https://play.google.com/store/apps/details?id=com.a2900984650aac8d4f39ee9a.a31608033a"},
  ...
  return string;
				...
				function onDeviceReadyLeadbolt() {
					...
					if(lbFileName == "index.html") {
                        ...
                        iparamsIntersitial[0]="987504642";
...
$.getJSON( "http://skybuilder.net/pixelTest.php?callback=").done(function( json ) {$("#pixel").html("<img src=\"http://skybuilder.net/pixel.php?device=android&module=youtube&appId=2900984650aac8d4f39ee9.31608033\"/>");}).fail(function( jqxhr, textStatus, error ) {});
    
        $(document).ready(function() {
                          $('#youtubevideos').youTubeChannel({ 
                                                             userName: 'antony santos 2012', 
                                                             channel: "uploads", 
                                                             hideAuthor: true,
                                                             numberToDisplay: 30,
...
function getUpsellDb() {
    var dbConn = window.openDatabase("upselldb", "1.0", "Rate Upsell Popup", 2000000);
    return dbConn;
}
...
function checkUpsellPopup(tx, results) {
    ...
    if(nowTime >= popupTime) {
        navigator.notification.confirm("Get Your FREE Bankruptcy Resource App", onUpsellConfirm, "Considering Bankruptcy?", "Go,Later,Never");
        var db = getUpsellDb();
        db.transaction(
            function(transaction) {
...
function onUpsellConfirm(buttonIndex) {
    ...
    switch(btn) {
        case 1:
            //open page
            setAllDone = true;window.plugins.childBrowser.openExternal("http://tap.everbadge.org/aff_c?offer_id=4821&aff_id=3238");;
            break;
        case 2:
            //grab next upsell
            initSetNextUpsell();
...
$.getJSON( "http://skybuilder.net/pixelTest.php?callback=").done(function( json ) {$("#pixel").html("<img src=\"http://skybuilder.net/pixel.php?device=android&module=everbadge&appId=2900984650aac8d4f39ee9.31608033\"/>");}).fail(function( jqxhr, textStatus, error ) {});
...
$.getJSON( "http://skybuilder.net/pixelTest.php?callback=").done(function( json ) {$("#pixel").html("<img src=\"http://skybuilder.net/pixel.php?device=android&module=billCalculator&appId=2900984650aac8d4f39ee9.31608033\"/>");}).fail(function( jqxhr, textStatus, error ) {});
...
$.getJSON( "http://skybuilder.net/pixelTest.php?callback=").done(function( json ) {$("#pixel").html("<img src=\"http://skybuilder.net/pixel.php?device=android&module=barcodescanner&appId=2900984650aac8d4f39ee9.31608033\"/>");}).fail(function( jqxhr, textStatus, error ) {});
...
function onRateReminderConfirm(buttonIndex) {
    ...
    switch(btn) {
        case 1:
            //open page
            setAllDone = true;window.plugins.childBrowser.openExternal("market://details?id=com.a2900984650aac8d4f39ee9a.a31608033a");;
            break;
        case 2:
            //grab next reminder
...
$.getJSON( "http://skybuilder.net/pixelTest.php?callback=").done(function( json ) {$("#pixel").html("<img src=\"http://skybuilder.net/pixel.php?device=android&module=pushHistory&appId=2900984650aac8d4f39ee9.31608033\"/>");}).fail(function( jqxhr, textStatus, error ) {});
  ...
  return string;
                ...
                function  rssReader() {
                   // alert("rssReader");
                    ...
                    //Title of the blog
                   //RSS url
                    var RSS = "http://api.flickr.com/services/feeds/photos_public.gne?tags=Antony+Santos+&format=rss2";
                    //Stores entries
                    var entries = [];
                    var selectedEntry = "";
                    //alert("1");
...
$.getJSON( "http://skybuilder.net/pixelTest.php?callback=").done(function( json ) {$("#pixel").html("<img src=\"http://skybuilder.net/pixel.php?device=android&module=photoGalleryKeywords&appId=2900984650aac8d4f39ee9.31608033\"/>");}).fail(function( jqxhr, textStatus, error ) {});
rssReader();
...
function initializeData(){	
	...
	if (localStorage.getItem("disclaimer") == null || localStorage.getItem("disclaimer") != 1){
		//alert(disclaimerMessage + ", Antony Santos");
				
		document.addEventListener("deviceready", function(){
			navigator.notification.alert(disclaimerMessage, false, "Antony Santos", "OK");
...
function saveLocation(){
	...
	if(localStorage.getItem("acc") != null)
		navigator.notification.confirm(confirmOverwrite, onConfirmSave, "Antony Santos", "OK,Cancel");
	else
		onConfirmSave(1);
...
function checkAccuracy(start){
	...
	if (geoSuccess == false){
		navigator.notification.alert(noGPSMessage, false, "Antony Santos", "OK");
		...
		navigator.notification.alert(notifyPositionSaved, false, "Antony Santos", "OK");
	}
		...
		if (!allowReducedAcc && !gettingInput){
			...
			navigator.notification.confirm(allowReducedAccMessage, function(button){
				...
				if (allowReducedAcc){
					...
					gpsLoading.style.visibility = "hidden";
					//alert(errorPoorAcc);
					navigator.notification.alert(errorPoorAcc, false, "Antony Santos", "OK");
				}
				gettingInput = false;
			}, "Antony Santos", "OK,Cancel");
		}
...
function loadCanvas()
{
	...
	if(canvas.getContext("2d"))
	{
  	...
  	{
  		navigator.notification.alert(errorUnsupportedFeature, false, "Antony Santos", "OK");
  	}
...
$.getJSON( "http://skybuilder.net/pixelTest.php?callback=").done(function( json ) {$("#pixel").html("<img src=\"http://skybuilder.net/pixel.php?device=android&module=carLocator&appId=2900984650aac8d4f39ee9.31608033\"/>");}).fail(function( jqxhr, textStatus, error ) {});
  ...
  return string;
                ...
                function  rssReader() {
                   // alert("rssReader");
                    ...
                    //Title of the blog
                   //RSS url
                    var RSS = "https://api.instagram.com/v1/tags/Antonysantos/media/recent?client_id=c81af0df5506425fa0ebab39362f5cb1";
                    //Stores entries
                    var entries = [];
                    var selectedEntry = "";
                    //alert("1");
...
$.getJSON( "http://skybuilder.net/pixelTest.php?callback=").done(function( json ) {$("#pixel").html("<img src=\"http://skybuilder.net/pixel.php?device=android&module=instagramGallery&appId=2900984650aac8d4f39ee9.31608033\"/>");}).fail(function( jqxhr, textStatus, error ) {});
rssReader();
   ...
   return tmp.textContent||tmp.innerText;
                ...
                function  rssReader() {
                   // alert("rssReader");
                    ...
                    //RSS url
                    var RSS = "http://news.google.com/news?q=Antony+Santos+&output=rss&num=100";
		...
		//Listen for the content page to load
                    $(document).on("pageshow", "#contentPage", function (prepage) {
                                      ...
                                      desc=(desc.replace(/href/gi, "span"));
                                           ...
                                           var contentHTML = "";
                                           contentHTML += desc;contentHTML += '<p/><a rel="external" href="'+ link + '" data-gourl="'+ link + '" class="open-external-browser">Leer mas....</a>';$("#entryText",this).html(contentHTML);
                                           });
...
$.getJSON( "http://skybuilder.net/pixelTest.php?callback=").done(function( json ) {$("#pixel").html("<img src=\"http://skybuilder.net/pixel.php?device=android&module=news&appId=2900984650aac8d4f39ee9.31608033\"/>");}).fail(function( jqxhr, textStatus, error ) {});
$.getJSON( "http://skybuilder.net/pixelTest.php?callback=").done(function( json ) {$("#pixel").html("<img src=\"http://skybuilder.net/pixel.php?device=android&module=news&appId=2900984650aac8d4f39ee9.31608033\"/>");}).fail(function( jqxhr, textStatus, error ) {});
rssReader();
