...
var rootLink = "http://fae.ctrl.fo/plane_script/";

$(document).ready(function(e) {
    getTravels();
	
    $("#FAQback").live("click",function(){
    	getFAQ();
    });
	
    $("#faqTrigger").click(function(e) {
        getFAQ();
        $('[data-role="footer"]').addClass("faqFix");
    });
    
    $("#flightsTrigger").click(function(e) {
    	$('[data-role="footer"]').removeClass("faqFix");
	getTravels();
    });
    
    $("#subTrigger").click(function(e) {
    	$('[data-role="page"]').addClass("blackNwhite");
    	viewSub();
    });
    
    $("#camTrigger").click(function(e) {
	$('#content').html('<img src="http://webcam.fae.fo/image.jpg?random=' + Math.random() + '" id="camPic" />');
    	$('[data-role="page"]').removeClass("blackNwhite");
    	$('[data-role="footer"]').removeClass("faqFix");
    });
    
    $("#guideTrigger").click(function(e) {
	$('#content').html('<img src="data/images/guide.png" id="frame" />');
    	$('[data-role="page"]').removeClass("blackNwhite");
    	$('[data-role="footer"]').removeClass("faqFix");
    });
    
    $("#FAQlist li ul li").live("click", function (){
...
function viewSub()
{
	var myStr = '<ul id="subList">';
	if (localStorage.subData)
    		var allSub = JSON.parse(localStorage.subData);
    		else
    		var allSub = new Array();
	
	for (j=0; j < allSub.length; j++)
	{
		myStr += "<li>" + allSub[j][1] + "</li>";
	}
...
function myResize() {
    ...
    var thirtypc = (10 * height) / 100;
    thirtypc = parseInt(thirtypc) + 'px';
    
    $('[data-role="footer"]').css('height',thirtypc);
}

function alertSub(title, text)
{
	$('[data-role="page"]').removeClass("blackNwhite");
	$('#alert').find(".text").html('Do you want to subscribe to ' + title + '?' + '<div class="htitle">' + title + '</div><div class="htext">' + text +'</div>');
	$('#alert').fadeIn(100);
}
...
{
	$('[data-role="page"]').addClass("blackNwhite");
	$("#content").html($("#hiddenFaq").html());
}

function getFAQcategory(id)
{
	$.ajax({
		url: rootLink + "getContent.php?id=" + id + "&callback=?",
		dataType: 'json',
		timeout: 3000,
		success: function(data) {
			console.log(data);
			var myString = '<div id="FAQback">< Aftur</div>';
			
			myString += data["data"]["Html"];
			
			$("#content").html(myString);
		},
		error: function(jqXHR, status, errorThrown){
			$("#content").html("Internet connection error. Try again later.");
		}
	});
}
...
{
	$.ajax({
		url: rootLink + "index.php?callback=?",
		dataType: 'json',
		timeout: 3000,
		success: function(data) {
			console.log(data);
			...
			{
				if (i==0)
					myString += '<div class="title"><img src="data/images/icon_land.jpg" />Komur</div>';
					else
					myString += '<div class="title"><img src="data/images/icon_takeoff.jpg" />Fráferðir</div>';
				myString += '<table>';
				...
				for (j=0; j<data[i].length; j++)
					{
						if (j==0)
							...
							myString += '<tr class="subscription">';
						
						
						// save data for sub list
						myString += '<td class="htitle">' + data[i][j][3] + '</td>';
						myString += '<td class="htext">';
						
						var ctext = 'Plane ' + data[i][j][3] + ' time: ' + data[i][j][1];
						if (data[i][j][2])
							ctext += ' expected: ' + data[i][j][2];
						if (data[i][j][5])
							ctext += ' remark: ' + data[i][j][5];
						...
						// check data
						if (j != 0)
							checkData(data[i][j][3], ctext, data[i][j][5]);
						
						
						for (k=0; k<data[i][j].length; k++)
							{
								if (j == 0)
									myString += '<th>' + data[i][j][k] + '</th>';
								else
									myString += '<td>' + data[i][j][k] + '</td>';
							}
...
function checkData(title, text, remark)
{
	...
	if (localStorage.subData)
	{
		...
		var head = title.substring(0,2);
		var tail = title.substring(title.length-3);
		
		
		for (p=0; p < allSub.length; p++)
		{
			if (allSub[p][0].substring(0,2) == head && allSub[p][0].substring(allSub[p][0].length-3) == tail)
			// this means that is the same flight
			{
				if (title[2] != allSub[p][0][2])
				{
					// it means that the flight was changed
					allSub[p][0] = title;
					allSub[p][1] = text;
					console.log(text);
					
				}
				if (trim1(remark) == 'LANDED' || trim1(remark) == 'Departed')
				{
					// delete this node because it landed
					allSub.pop(p);
		...
		localStorage.subData = JSON.stringify(allSub);
