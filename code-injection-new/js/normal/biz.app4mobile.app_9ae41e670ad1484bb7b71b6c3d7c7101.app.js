        ...
        _renderTab: function (index, navRequest) {
            if (PLATFORM === platformEnum.simulator ){
                ...
                var genericPageModel = new GenericPageModel({
                    title: _T("QRScannerWebAppHeader", [], this.model.get("meta").overrideTranslation),
                    text: _T("QRScannerWebAppText", [], this.model.get("meta").overrideTranslation),
...
{	
	...
	initialize: function()
	{
		...
		this.set(
		{
			'descriptionTitle': _T("HtmlTextAboutUsDescriptionTitle", [], this.get('metaTab').overrideTranslation)
		});
		this.set(
		{
			'readMoreTitle': _T("HtmlTextAboutUsReadMore", [], this.get('metaTab').overrideTranslation)
		});
	...
	_handleDataList: function()
	{
		...
		for (var i = 0; i < dataList.length; i++) 
		{
			...
			else if (params.dataType === "hours") 
			{
				...
				{
					params.text = _T('HtmlTextAboutUsItemHoursAlwaysOpen',[],this.get('metaTab').overrideTranslation);
				} 
				else if (blocks && blocks.length) 
				{
					var daysNames = 
					[
						_T("_day1s",[],this.get('metaTab').overrideTranslation),
						_T("_day2s",[],this.get('metaTab').overrideTranslation),
						_T("_day3s",[],this.get('metaTab').overrideTranslation),
						_T("_day4s",[],this.get('metaTab').overrideTranslation),
						_T("_day5s",[],this.get('metaTab').overrideTranslation),
						_T("_day6s",[],this.get('metaTab').overrideTranslation),
						_T("_day7s",[],this.get('metaTab').overrideTranslation)
					];
					var firstDayOfWeek = _T("_firstDayOfWeek",[],this.get('metaTab').overrideTranslation);
					for (var j = 0; j < blocks.length; j++) 
					{
                        ...
                        var startHour = (new Date(Utils.TimeDate.toUTCTime(Utils.TimeDate.timeToSeconds(blocks[j].startHour)))).format('shortTime');
                        var endHour = (new Date(Utils.TimeDate.toUTCTime(Utils.TimeDate.timeToSeconds(blocks[j].endHour)))).format('shortTime');
                        blocks[j].hoursStr = startHour + ' - ' + endHour;
					}
				}
				else
				{
					params.text = _T('HtmlTextAboutUsItemHoursNoHours', [], this.get('metaTab').overrideTranslation);
				} 
			}
...
//        aboutUsModel.bind('itemMoreClicked', this._onItemMoreClicked, this);
    ...
    render: function () {
        ...
        },this.model.get('metaTab').overrideTranslation);
    ...
    _getShareInfo: function () {
		...
		shareInfo.shareButtonText = _T('HtmlTextShareAppButtonText', [], this.get('metaTab').overrideTranslation);
		return shareInfo;
	}
}); /**
...
var AboutUsView = Backbone.View.extend(
{
    ...
    updateDescriptions: function ()
    {
        ...
        var params =
        {
            staticTitle: _T("HtmlTextAboutUsListItemReadMore", [], this.model.get('metaTab').overrideTranslation)
        }
...
webModuleTemplates.pageStaticTemplate = _.template(
    ...
    _renderTab: function (index, navRequest) {
        var templateMetaData = this.model.get('meta'),
            ...
            model = new Backbone.Model(jModel),
            viewType = WebModuleView;
        var id = Utils.Helpers.generateGUID();

        if (staticPage)
        {
            model = new GenericPageModel(staticPage);//staticPage can include title,text,img params for the "generic page":
...
//		wideTemplateViewName: "AudioWideTemplateView",
		nativeControllerName: "AudioController",
		itemsName: "items"
	},
...
//		wideTemplateViewName: "BlogWideTemplateView",
		nativeControllerName: "BlogController",
		itemsName: "feeds"
	},
...
//		wideTemplateViewName: "EventsWideTemplateView",
		nativeControllerName: "EventsController",
		itemsName: "items"
	},
...
//		wideTemplateViewName: "FacebookWideTemplateView",
		nativeControllerName: "FacebookController",
		itemsName: "channels"
	},
	...
	"79eec590-f806-f7ac-946b-1fd9c90283ba": {
		...
		//wideTemplateViewName: "FormWideTemplateView",
		itemsName: "items"
	},
	...
	"fc6700a7-a11e-de90-93f8-7357f9f0037f": {
		...
		//        wideTemplateViewName: "LinksWideTemplateView",
		cssClass: "links",
		itemsName: "items"
	},
	...
	"26ae8ccc-5464-7979-4fdf-3a13f166ffff": {
		...
		nativeControllerName: "PhotosController",
		itemsName: "feeds"
	},
...
//        wideTemplateViewName: "LoyaltyCardsWideTemplateView",
	...
	"aca2f190-b22b-920d-f12a-998101ad4b70": {
		...
		nativeControllerName: "MapController",
		itemsName: "items"
	},
...
//		wideTemplateViewName: "ReviewsWideTemplateView"
	},
...
//		wideTemplateViewName: "TwitterWideTemplateView",
	...
	"a95e67d5-4816-11c2-318d-fe64a33e32d2": {
		...
		//wideTemplateViewName: "UsersWideTemplateView",
		nativeControllerName: "UsersController",
		itemsName: "items"
	},
...
//		wideTemplateViewName: "VideoWideTemplateView",
		nativeControllerName: "VideoController",
...
//		wideTemplateViewName: "VideoWideTemplateView",
	...
	"38ab2b78-a1ad-42f8-8cb7-9475498c0f30": {
		packageName: "reports",
		cssClass: "reports",
		nativeControllerName: "ReportsController",
	...
	"8d7507ff-317e-44b1-9ad3-776ad52d6ee2": {
		...
		cssClass: "homepage",
		nativeControllerName: "HomepageController",
		itemsName: "items"
	},
...
//    }
    ...
    initialize: function () {
        this.overrideTranslation = CurrentPageTracker.getCurrentPage().get('meta').overrideTranslation
        ...
        var defaults = {
            ...
            {
                icon: 'default',
                freebieName: _T("HtmlTextLoyaltyCardsDefaultFreebieName", [], this.overrideTranslation), // ='freebie'
        ...
        // temporary (=until will be supported) add alias
        defaults.alias = Utils.Html.fixAlias(defaults.alias || defaults.id);
...
//        var slot = this.getActiveSlot();
    ...
    getShareInfo: function () {
        ...
        if (this.get('verificationState') === loyaltyCardsEnums.verificationState.SUCCESS)
        {
            ...
            shareInfo.title = shareInfo.title || _T('HtmlTextLoyaltyCardsShareWinTitle', params, this.overrideTranslation);

            // title for twitter:
            shareInfo.twitterTitle = shareInfo.twitterTitle || _T('HtmlTextLoyaltyCardsShareWinTwitterTitle', params, this.overrideTranslation);

            // we have different texts for email:
            shareInfo.emailTitle = shareInfo.emailTitle || _T('HtmlTextLoyaltyCardsShareWinEmailTitle', params, this.overrideTranslation);
            shareInfo.emailDesc = shareInfo.emailDesc || _T('HtmlTextLoyaltyCardsShareWinEmailText', params, this.overrideTranslation);
        }
        else
        {
            // title for facebook (= we use different texts for email and twitter):
            shareInfo.title = shareInfo.title || _T('HtmlTextLoyaltyCardsShareTitle', params, this.overrideTranslation);

            // title for twitter:
            shareInfo.twitterTitle = shareInfo.twitterTitle || _T('HtmlTextLoyaltyCardsShareTwitterTitle', params, this.overrideTranslation);

            // we have different texts for email:
            shareInfo.emailTitle = shareInfo.emailTitle || _T('HtmlTextLoyaltyCardsShareEmailTitle', params, this.overrideTranslation);
            shareInfo.emailDesc = shareInfo.emailDesc || _T('HtmlTextLoyaltyCardsShareEmailText', params, this.overrideTranslation);
        }
        return shareInfo;
    }
...
//    className: 'item_container',
    ...
    render: function () {
        ...
        var overrideTranslation = CurrentPageTracker.getCurrentPage().get('meta').overrideTranslation;
        // if loyaltyCard is expired
        if (isExpired || isComingSoon) {
            var params = {
                loyaltyCardExpired: _T(isExpired?'HtmlTextLoyaltyCardsExpired':'HtmlTextLoyaltyCardsComingSoon' , [], overrideTranslation)
            };
        ...
        if (/*this.options.isGrid ||*/ validFrom || validUntil) {
            ...
            if (validFrom) {
                ...
                params.startStr = _T('HtmlTextLoyaltyCardsValidFrom', [], overrideTranslation);
            }
            if (validUntil) {
                params.formattedValidUntil = new Date(validUntil * 1000).format("shortDate");
                params.endStr = _T('HtmlTextLoyaltyCardsValidUntil', [], overrideTranslation);
            }
...
//            }.hitch(this), 1);
    ...
    render: function()
    {
        var params = this.model.toJSON();
        var overrideTranslation = CurrentPageTracker.getCurrentPage().get('meta').overrideTranslation;
        params.infoTitle = _T("HtmlTextLoyaltyCardsInfoTitle", [], overrideTranslation);
        params.validFrom = null;
        params.validUntil = null;
        var validity = this.model.get('validity');
        if (validity.validFrom || validity.validUntil)
        {
            params.validityTitle = _T("HtmlTextLoyaltyCardsValidityTitle", [], overrideTranslation);
            if (validity.validFrom)
            {
                params.validFrom = new Date(validity.validFrom * 1000).format("shortDate");
                params.validFromTitle = _T('HtmlTextLoyaltyCardsValidFrom', [], overrideTranslation);
            }
            if (validity.validUntil)
            {
                params.validUntil = new Date(validity.validUntil * 1000).format("shortDate");
                params.validUntilTitle = _T('HtmlTextLoyaltyCardsValidUntil', [], overrideTranslation);
            }
        }
        params.cancel = _T("HtmlTextLoyaltyCardsInfoButtonCancel", [], overrideTranslation);
...
//        }
    ...
    render: function()
    {
        ...
        if (isExpired || isComingSoon) {
            var params = {
                loyaltyCardExpired: _T(isExpired?'HtmlTextLoyaltyCardsExpired':'HtmlTextLoyaltyCardsComingSoon', [], this.overrideTranslation)
            };
            this.$el.append(loyaltyCardsTemplates.loyaltyCardExpired(params));
            var $item_container = this.$('.item_container');
            $item_container.addClass('expired');
    ...
    addButtons: function ()
    {
        ...
        if (addInfoBtn)
        {
            ...
            var showMoreButton = new FlatButtonView({
                text: _T('HtmlTextLoyaltyCardsShowMoreButton', [], this.overrideTranslation ),
                icon: 'info22'
            });
...
//        var params = this.model.toJSON();
        this.overrideTranslation = CurrentPageTracker.getCurrentPage().get('meta').overrideTranslation;
        var params = {
            ok: _T('HtmlTextLoyaltyCardsDialogButtonOk', [], this.overrideTranslation),
            cancel: _T('HtmlTextLoyaltyCardsDialogButtonCancel', [], this.overrideTranslation)
        };
...
//        console.log('AAAAAAAAAA');
        this.$input.val('').attr({placeholder: _T('HtmlTextLoyaltyCardsDialogPlaceholder', [], this.overrideTranslation)});
    ...
    renderHeader: function()
    {
        ...
        params = _.extend(params, {
            CongratsCaption: _T("HtmlTextLoyaltyCardsDialogCongratsCaption", [], this.overrideTranslation),
            FreebieText: _T("HtmlTextLoyaltyCardsDialogFreebieText", {freebie: '<div class="freebie size_title_4 clr_contTypeB_hdlTxt">' + params.freebieName + '</div>'}, this.overrideTranslation),
            DialogStampCaption: _T("HtmlTextLoyaltyCardsDialogStampCaption", [] , this.overrideTranslation),
            MidFreebieText: _T("HtmlTextLoyaltyCardsDialogMidFreebieText", {freebie: '<span class="mid_freebie clr_contTypeB_hdlTxt">' + params.freebieName + '</span>'}, this.overrideTranslation),
            StampCaption: _T("HtmlTextLoyaltyCardsDialogStampCaption", [] , this.overrideTranslation),
            NormalText: _T("HtmlTextLoyaltyCardsDialogNormalText", [] , this.overrideTranslation)
        })
    ...
    updateState: function ()
    {
        ...
        {
            this._disableStamp();
            this.$input.val('').attr({placeholder: _T('HtmlTextLoyaltyCardsDialogPlaceholderLocked', [], this.overrideTranslation)});
            this.$input.addClass('mandatory');

            var delta = userData.blockUntil - nowTime;
            var hours = "" + Math.ceil(delta / 1000 / 60 / 60 ) ;
            var text = _T("HtmlTextLoyaltyCardsDialogServiceCoolDownText", {hours: hours}, this.overrideTranslation);
...
//            var text = _T("HtmlTextLoyaltyCardsDialogServiceCoolDownText", {time: time});
        ...
        else if (verificationState === loyaltyCardsEnums.verificationState.VERIFYING)
        {
            ...
            var text = _T("HtmlTextLoyaltyCardsDialogLoading", [], this.overrideTranslation );
        ...
        {
            this._enableStamp();
            var text = _T("HtmlTextLoyaltyCardsDialogServiceFailText", [], this.overrideTranslation);
            this.$status.html(text).removeClass('clr_contTypeB_txt').addClass('clr_punch_errTxt');
        }
...
//            uiDriver.showToastMessage(null, null, "Wrong code!", this.$el);
            ...
            var text = _T("HtmlTextLoyaltyCardsDialogWrongCodeText", [], this.overrideTranslation);
            this.$status.html(text).removeClass('clr_contTypeB_txt').addClass('clr_punch_errTxt');
        }
        else if (verificationState === loyaltyCardsEnums.verificationState.SUCCESS)
        {
            ...
            if (afterPunchType === loyaltyCardsEnums.afterPunchType.CONGRATS)
            {
                ...
                this.$header.html(loyaltyCardsTemplates.successHeader_tpl(_.extend(slot, {
                    CongratsSuccessCaption: _T("HtmlTextLoyaltyCardsDialogCongratsSuccessCaption", [], this.overrideTranslation),
                    FreebieSuccessText: _T("HtmlTextLoyaltyCardsDialogFreebieSuccessText", {freebie: '<div class="freebie size_title_4 clr_contTypeB_hdlTxt">' + slot.freebieName + '</div>'}, this.overrideTranslation)})));
        ...
        else // if (verificationState === loyaltyCardsEnums.verificationState.NORMAL)
        {
            ...
            {
                this._disableStamp();
                var text = _T("HtmlTextLoyaltyCardsDialogLimitPerDayText", {punchesPerDay: limits.punchesPerDay}, this.overrideTranslation);
                this.$status.html(text).removeClass('clr_contTypeB_txt').addClass('clr_punch_errTxt');
    ...
    _enableStamp: function ()
    {
        ...
        this.$input.attr({placeholder: _T('HtmlTextLoyaltyCardsDialogPlaceholder', [] , this.overrideTranslation)});
 ...
 var CouponModel = ItemModel.extend(
{
	...
	{
        // temporary (=until will be supported) add alias
        var alias = Utils.Html.fixAlias(this.get('alias') || this.get('id'));
		...
		{
			productDiscount.upperTitle = _T('HtmlTextCouponsTitleOffer', [], overrideTranslation);
		}
		if (productDiscount.lowerTitle === null)
		{
			if (productDiscount.offerType === couponsEnums.offerType.DISCOUNT)
			{
				productDiscount.lowerTitle = _T('HtmlTextCouponsPriceOff', [], overrideTranslation);
			}
			else
			{
   ...
   getShareInfo: function()
    {
        ...
        var shareTitle = _T('HtmlTextCouponsShareTitle', [], overrideTranslation);
		var appName = AppManager.app().get('appLabel');
        var shareDescription;
        if (_.isNull(offerTitle)){
            shareDescription = _T('HtmlTextCouponsShareDescriptionNoTitle', {appName: appName}, overrideTranslation);

        }
        else {
            shareDescription = _T('HtmlTextCouponsShareDescription', {offerTitle: offerTitle, appName: appName}, overrideTranslation);
        }
...
var CouponsTemplateView = TemplateView.extend(
{
    ...
    _renderTab : function(index, navRequest) {
        ...
        model.set(
        {
            ...
            overrideTranslation: meta.overrideTranslation
        });
 ...
 var CouponsItemView = ListItemView.extend(
{
    ...
    render: function ()
    {
        ...
        if (this.options.isGrid || validFrom || validUntil)
        {
			...
			{
				params.formattedValidFrom = new Date(validFrom * 1000).format("shortDate");
				params.startStr = _T('HtmlTextCouponsValidFrom', [], overrideTranslation);
			}
			if (validUntil)
			{
				params.formattedValidUntil = new Date(validUntil * 1000).format("shortDate");
				params.endStr = _T('HtmlTextCouponsValidUntil', [], overrideTranslation);
			}
...
}); var CouponPrimaryView = Backbone.View.extend(
{
	...
	_showClaim: function(){
        if (this.options.isListItem && this.model.get('isClaimed')){
            this.$el.find(".couponClaimContainer").html(couponsTemplates.couponShowClaimTemplate({CouponsCouponClaimed: _T("HtmlTextCouponsCouponClaimed", [], CurrentPageTracker.getCurrentPage().get('meta').overrideTranslation)}));//??
        }
    },
	render: function ()
    {
...
var CouponDataView = Backbone.View.extend(
{
	...
	render: function()
	{
		...
		,validUntil: 0
		,hours: null
		,validityTitle: _T('HtmlTextCouponsTitleValidity', [], this.overrideTranslation)
			,dealDetails: dealDetails
			,detailsTitle: _T('HtmlTextCouponsTitleDealDetails', [], this.overrideTranslation)
			,linkUrl: linkUrl
        ...
        if (productDiscount.price && productDiscount.discount)    {
            ...
            params.pricesBanner = {
                titleA: _T('HtmlTextCouponsTitleOriginalPrice', [], this.overrideTranslation)
                ...
                ,titleC: _T('HtmlTextCouponsTitleSaving', [], this.overrideTranslation)
            };

            if (productDiscount.offerType === couponsEnums.offerType.FINAL_PRICE)
            {
                params.pricesBanner.titleB = _T('HtmlTextCouponsTitleDiscount', [], this.overrideTranslation);
            ...
            {
                params.pricesBanner.titleB = _T('HtmlTextCouponsTitleNewPrice', [], this.overrideTranslation);
                params.pricesBanner.symbolB = productDiscount.currencySymbol;
                if (productDiscount.discountType === couponsEnums.discountType.MONEY)
                {
        ...
        if (validity.validFrom || validity.validUntil)
        {
            ...
            {
                params.validFrom = new Date(validity.validFrom * 1000).format("shortDate");
                params.validFromTitle = _T('HtmlTextCouponsValidFrom', [], this.overrideTranslation);
            }
            if (validity.validUntil)
            {
                params.validUntil = new Date(validity.validUntil * 1000).format("shortDate");
                params.validUntilTitle = _T('HtmlTextCouponsValidUntil', [], this.overrideTranslation);
            }
        }
        ...
        if (validity.hours)
        {
            ...
            //			move it to "UTILS" function or something like this.
                ...
                if (hoursOptions && hoursOptions.length)
                {
                    var daysNames =
                    [
                        _T("_day1s", [], this.overrideTranslation),
                        _T("_day2s", [], this.overrideTranslation),
                        _T("_day3s", [], this.overrideTranslation),
                        _T("_day4s", [], this.overrideTranslation),
                        _T("_day5s", [], this.overrideTranslation),
                        _T("_day6s", [], this.overrideTranslation),
                        _T("_day7s", [], this.overrideTranslation)
                    ];
                    var firstDayOfWeek = _T("_firstDayOfWeek", [], this.overrideTranslation);
                ...
                {
                    params.hours = _T('HtmlTextCouponsNoHours', [], this.overrideTranslation );
                }
        }
...
var CouponData2View = Backbone.View.extend(
{
	...
	render: function()
	{
		...
		var params =
        {
			...
			,dealDetails: dealDetails
			,detailsTitle: _T('HtmlTextCouponsTitleDealDetails', [], CurrentPageTracker.getCurrentPage().get('meta').overrideTranslation)
            ,imageUrl: this.model.get('imageUrl')
        };
...
//            this.$claimed.removeClass('out');
	...
	getNotClaimed: function()
	{
        var params = 
        {
        	txt: _T('HtmlTextCouponsClaimButton', [], this.overrideTranslation)
        };
    ...
    getClaimed: function()
	{
        var params = 
        {
        	txt: _T('HtmlTextCouponsCouponClaimedLongText', [], this.overrideTranslation)
        };
...
var CollectionsItemView = Backbone.View.extend({
	...
	initialize: function () {
		...
		this.$scroller.append(itemTemplate({
            ...
            buyNowText: _T("CollectionsBuyNow", [], this.model.get('tabMeta').overrideTranslation),
            currentPriceText: _T("CollectionsCurrentPrice", [], this.model.get('tabMeta').overrideTranslation),
            CollectionsItemDetails: _T("CollectionsItemDetails", [], this.model.get('tabMeta').overrideTranslation),
...
//                model.setSearch(term, type);
        ...
        getNoItemsView : function(settings)
        {
            ...
            if (term) {
                // no items for search
                var title = _T("CollectionsSearchNoResultsFound", [], this.model.get("meta").overrideTranslation);
                var text = _T("CollectionsSearchNoResultsFoundSubtitle", {node_name: node_name, query_string: term}, this.model.get("meta").overrideTranslation);
                var img = "app/interface/web/img/search.svg";
            } else {
                // empty category
                var title = _T("CollectionsSearchNoItemsFound", [], this.model.get("meta").overrideTranslation);
                var text = _T("CollectionsSearchNoItemsFoundSubtitle", {node_name: node_name}, this.model.get("meta").overrideTranslation);
...
{	
	...
	{	
		...
		if (this.model.get('isCategory')) 
		{
			...
			{
				params.itemsCountStr = _T('CollectionsCategoryUnknownItemsCount', [], this.model.get('tabMeta').overrideTranslation);
			}
			else
			{
				params.itemsCountStr = _T('CollectionsCategoryItemsCount', {count: params.itemsCount}, this.model.get('tabMeta').overrideTranslation);
			}
		}
		else 
		{
...
//        this.model.on('change:breadCrumbs', this.render, this); // currently we don't get the breadCrumbs from CMS, so it will never be changed
    ...
    render: function () {
        ...
        params = {
            placeholder: _T('CollectionsSearchPlaceholder2', [], this.model.get('tabMeta').overrideTranslation)
        };
...
var CollectionsCategoryRightView = ItemsView.extend(
{
    ...
    onNoResults: function () {
        ...
        if (term) {
            // no items for search
            var title = _T("CollectionsSearchNoResultsFound", [], this.model.get('tabMeta').overrideTranslation);
            var text = _T("CollectionsSearchNoResultsFoundSubtitle", {node_name: node_name, query_string: term}, this.model.get('tabMeta').overrideTranslation);
            var img = "app/interface/web/img/search.svg";
        } else {
            // empty category
            var title = _T("CollectionsSearchNoItemsFound", [], this.model.get('tabMeta').overrideTranslation);
            var text = _T("CollectionsSearchNoItemsFoundSubtitle", {node_name: node_name}, this.model.get('tabMeta').overrideTranslation);
            var img = "app/interface/web/img/noItems.svg";
        }
...
var CollectionsItemRightView = Backbone.View.extend(
{
	...
	initialize: function () 
	{
        var collectionItemRightNowParameter = _.extend({}, this.model.toJSON(), {buyNow:  _T("CollectionsBuyNow", [] , this.model.get('tabMeta').overrideTranslation)} )
...
contactUsTemplates.detailsPageTemplate = _.template(
    '<div class="scroll_wrapper"><div class="scroller">' +
		...
		// image logo
        '<% if (typeof(logoImgUrl) != "undefined" && logoImgUrl) { %>' +
	        '<img class="logoImgCenter" src="<%= logoImgUrl %>" />' +
        '<% } %>' +
...
// image
        '<% if ((typeof(imgUrl) != "undefined" && imgUrl) || (typeof(logoImgUrl) != "undefined" && logoImgUrl)) { %>' +
            '<div class="sidebarImg">' +
                '<% if(typeof(logoImgUrl) != "undefined" && logoImgUrl){ %>' +
                    '<div class="logoImgUrl" style="background-image: url(<%= logoImgUrl %>)"></div>' +
                '<% } %>' +
                '<% if(typeof(imgUrl) != "undefined" && imgUrl){ %>' +
...
var ContactUsView = Backbone.View.extend(
{
    ...
    _onLinkTap: function (e)
	{
        ...
        appDriver.openLink(null, null, { url: url,useExternalBrowser:true });
                ...
                '<% if (showPrice || showBuyButton) { %>' +
                    '<div class="price_container clr_contTypeA_subBg">' +
                        ...
                        '<div class="info_container">' +
                            ...
                            //'<span class="price clr_contTypeA_hdlTxt"><%= price %></span>' +
                            '<div class="narrow_price_container clr_contTypeA_hdlTxt" dir="ltr">' +
                                '<% if (payment.meta.currencySign) { %>' +
                                    '<span class="currency"><%= payment.meta.currencySign %></span>' +
                                '<% } %>' +
...
'<% } %>' +
'<% if (subtitle) { %>' +
    '<div class="description size_1 clr_contTypeA_subTxt"><%= subtitle %></div>' +
'<% } %>' +
'</div></div>'
);

    storeTemplates.CollectionsItemRightView = _.template(
'<div class="scroll_wrapper"><div class="scroller">' +
'<div class="data_content">' +
'<% if (images.gallery && images.gallery.length) { %>' +
    '<div class="headerImage sliced_image" style="background-image:url(\'<%= images.gallery[0].url %>\');">' +
        '<div class="catImage clr_images_catImage_bg"></div>' +
        '<div class="full_screen_hint sprite sprite-FullScreenIcon"></div>' +
    '</div>' +
'<% } %>' +
'<% if (images.gallery && images.gallery.length > 1) { %>' +
    '<div class="images_container clr_contTypeB_subBg">' +
        '<div class="item_thumbnail sliced_image" style="background-image:url(\'<%= images.gallery[0].url %>\');"><div data-role="button" class="catImage clr_images_catImage_bg selected" data-index="0"></div></div>' +
        '<div class="item_thumbnail sliced_image" style="background-image:url(\'<%= images.gallery[1].url %>\');"><div data-role="button" class="catImage clr_images_catImage_bg" data-index="1"></div></div>' +
        '<% if (images.gallery.length > 2) { %>' +
            '<div class="item_thumbnail sliced_image" style="background-image:url(\'<%= images.gallery[2].url %>\');"><div data-role="button" class="catImage clr_images_catImage_bg" data-index="2"></div></div>' +
        '<% } %>' +
    '</div>' +
'<% } %>' +
'<% if (name) { %>' +
...
'<% } %>' +
'<% if (subtitle) { %>' +
    '<div class="description size_1 clr_contTypeB_subTxt"><%= subtitle %></div>' +
'<% } %>' +
'<% if (showPrice || showBuyButton) { %>' +
    '<div class="price_container clr_contTypeB_subBg">' +
        '<% if (showPrice) { %>' +
...
'<% } %>' +
'<% if (description) { %>' +
    '<div class="details size_1 clr_contTypeB_txt"><%= description %></div>' +
'<% } %>' +
'</div>' +
'<div class="scrollingBug"></div>' + // we have scrolling issue. I don't know why. so I use this as walkaround...
/*'<div class="clearfix">A</div>' +
...
'<div class="vertical_center"><div>' +
    '<% if (name) { %>' +
        '<div class="name size_1 <% if (showPrice) { %>max_2_lines<% } else { %>max_3_lines<% } %> clr_contTypeA_hdlTxt"><%= name %></div>' +
    '<% } %>' +
    '<% if (showPrice) { %>' +
        '<div class="price clr_contTypeA_subTxt" dir="ltr">' +
            '<% if (payment.meta.currencySign) { %>' +
 ...
 var StoreNodeModel = ItemsModel.extend({
    ...
    initialize: function () {
        ...
        var defaults = {
            ...
            images: {},
            meta: {},
            showBuyButton: false, // do we need a buy button for this item?
    ...
    _updateFromJSON: function (data) {
        ...
        // make simple access to headerImage:
        data.headerImage = (data.images && data.images.main && data.images.main.url) || data.defaultImage;

        // split the price, e.g.: 29.5 to "29","50"
        if (typeof data.payment.price === "number") {
            data.priceArr = ("" + data.payment.price).split(".");
...
//        //filter unsupported store types, + remove store if no relevant types:
////        if (meta.store){
////            var providers = meta.store.providers || [];
////
////            for (var i = providers.length - 1; i > -1; i-- )
////            {
////                var type = providers[i].type;
////                var found = false;
////                for(var prop in paymentProvidersTypesEnum) {
////                    if (paymentProvidersTypesEnum[prop] === type)
////                    {
////                        found = true;
////                        break;
////                    }
...
////        }
        ...
        //we need "buy now" button in case of url-link, or in case of "store item" (= e.g.: buy with paypal)
//        data.needBuyButton = (meta.links && meta.links.store) || (data.store && data.store.id);
        data.showBuyButton = !!(data.payment.providers && data.payment.providers.length) && !data.payment.meta.hideBuy;
        //NOTE: we display "0$" if we get 0. if we don't want to display, it shouldn't be number
        data.showPrice = (typeof data.payment.price === "number");
...
//                    text: _T('ButtonClose'),
            ...
            uiDriver.showDialogBox(null, null,
                {
                    ...
                    message: _T('DialogMessageSuccessBuy', {itemTitle: that.get('name')}),
                    buttons: btns
                });
        ...
        StorePurchaseManager.buy(cbSuccess, cbFail, storeItemId, paymentProvider);
    },
    ...
    {
        var that = this;
        return _.map(this.get('images').gallery, function (item) {
...
var StoreDescendantSummaryView = /*ItemView*/Backbone.View.extend(
{
	...
	render: function () {
		...
		this.$el.append(template({title: this.model.get("name"), subtitle: this.model.get("subtitle"), price: price, thumbnail: thumbnail}));
...
var StoreItemView = Backbone.View.extend({
	...
	initialize: function () {
		...
		var itemTemplate = storeTemplates["item_view"];
		var meta = this.model.get("meta");
		var images = this.model.get("images");
		var description = this.model.get("description");
		var links = meta.links || {};
		...
		this.$scroller.append(itemTemplate({
            ...
            showBuyButton: this.model.get("showBuyButton"),
            showPrice: this.model.get("showPrice"),
			gallery: (images && images.gallery) || [],
			//price: this.model.getPriceStr(),
			//TODO: What is all this? we have a controller for this! change it!
	...
	collectionItemThumbnailClicked: function (e) {
		var $selected = $(e.target);
		this.imageIndex = parseInt($selected.attr("data-index"));
		var image = this.model.get('images').gallery[this.imageIndex].url;
...
//        this._updateHeader();
	...
	_updateHeader: function ()
	{
        ...
        this.$('.header').html(headerTemplate({
            name: Utils.String.sanitizeText(name, true),
            subtitle: Utils.String.sanitizeText(subtitle, true),
            headerImage: this.model.get("headerImage"),
            isSearch: isSearch
        }));
...
var StoreItemRightView = Backbone.View.extend(
{
	...
	initialize: function () 
	{
		...
		this.$el.html(storeTemplates.CollectionsItemRightView(collectionItemRightNowParameter));
		var meta = this.model.get('meta');
		var description = this.model.get('description');
		...
		if (description)
		{
			...
			htmlContainer.renderHtml($containerElement, description, {changeColors: true, colorType: "B"});
		}
