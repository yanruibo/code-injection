...
var localizationData = {
	"sv":	
	{
		"global": 
		{
			...
			"RESET": "Nollställ appen",
			"DELETE" : "Ta bort inläsning"
		},
		"initial_login":
		{
			"RETRIEVE_SETTINGS" : "Hämta inställningar",
			"RETRIEVE_SETTINGS_HELP" : "Inställningar för företaget måste installeras när du startar appen för första gången",
			"COMPANYID": "Företag/Organisation",
			"SETTINGS": "Inställningar",
			"GET_SETTINGS": "Hämta",
			"GETFAIL" : "Hämtning misslyckades",
			"WRONGCOMPANY" : "Fel företag"
		},
		"login":
		{
			"HEADER" : "Logga in",
		...
		"list":
		{
			"COUNTERNO" : "Mätarnummer",
			"MEDIATYPE" : "Media",
			"READINGVALUETEXT" : "Föregående ställning",
			"CALCREADINGVALUETEXT" :  "Beräknad ställning",
			"CURVALUETEXT" : "Mätarställning",
			"CGNO" : "Anläggningsnr",
			"AREA" : "Område",
			"ESTATE" : "Byggnad",
			"PLACEMENT" : "Placering",
			"oldmeter" : "Gamla mätaren",
			"GOTO" : "Gå till",
			"DATEFORCHANGE" :  "Datum för mätarbyte",
			"FINALVALUE" : "Slutställning",
			"CHANGEDBEFORE" : "Omställd före ändring",
			"NEWMETER" : "Nya mätaren",
			"STARTVALUE" : "Startställning",
			"CONSTANT" : "Konstant",
			"ACTUALVALUE" : "Aktuell mätarställning",
			"DATE" : "Datum",
			"SAME" : "Samma",
			"SAVENEXT" : "Spara och gå till nästa",
			"SAVE" : "Spara",
			"NEXT" : "Nästa mätare",
			"PREV" : "Föregående mätare",
			"LASTREADING" : "Föregående avläsning",
			"DIFF" : "Diff. mot beräknad ställning",
			"WRAPPED" : "Slagit runt",
			"CHANGEDMETER" : "Mätaren är utbytt",
			"NOMETERS" : "Tryck på synkronisera för att ladda mätare för den här användaren.",
			"NOMETERSCATEGORY" : "Det finns inga mätare i denna kategori",
			"NUMWRONGS" : "fel",
			"STATUSDONE" : "Klar",
			"STATUSCHANGED" : "Utbytt",
			"STATUSNOTREAD" : "Ej avläst",
			"NOMETERSTITLE" : "Det finns inga mätare",
			"NOMETERSTEXT" : "Det finns inga mätare att ladda för tillfället"
		},
		"actionbar":
		{
			"back" : "Tillbaka"
		...
		"settings":
		{
			"URL" : "URL till inställningsservern",
			"VERSION" : "Version",
			"SAVE" : "Spara",
			"SAVED" : "Sparat",
			"FETCH" : "Hämta",
			"SELECTUSERFORLIST" : "Hämta avläsningslista för:",
			"ALLMETERDATALOST" : "Om du byter användare att hämta lista för så kommer all din osynkade mätardata försvinna",
			"AREYOUSURE" : "Är du säker?",
			"LOSEDATA" : "Om du byter användare att hämta lista för så kommer all din osynkade mätardata försvinna",
			"SYNCFIRST" : "Synkronisera föregående användares avläsningar och ladda lista för ny användare",
			"NOSYNC" : "Ladda ny lista utan att synkronisera föregående användares avläsningar",
			"LASTWARNING" : "Om du byter användare att hämta lista för, kommer all osynkad mätardata att försvinna."
		},
		"select" :
		{
			"ALL" : "Alla",
			"NOTREAD" : "Inte avlästa",
			"READ" : "Avlästa",
			"ERROR" : "Felaktiga",
			"READING" : "Avläsning",
			"INFO" : "Information",
...
/** @Static */
{
	...
	getLang : function()
	{
		...
		Rc.Models.Localization.findAll(function(lang){
			localization = lang;
		});
		
		if(sessionGet("selectedLang") != null)
		{
			return localization[sessionGet("selectedLang")];
		} else {
			sessionSet("selectedLang", config.defaultLang);
			return localization[config.defaultLang];
		}
	},
	
	/* H�mtar r�tt spr�k f�r en label, kollar f�rst sidspecifika men finns den inte
	 * d�r kollar den �ven i global efter labelen.
	 */
	getLabel : function(id)
	{
		var strings = this.getLang();
        if(strings != null)
        {
             var label = strings[id];
             if(label != null)
             {
                return label;
             } else 
             {
                label = strings["global"][id];
                if(label != null)
                    return label;
                return "["+sessionGet("selectedLang")+", Global, "+id+"]";
             }
        }
	...
	getPageLabel : function(page,id)
	{
		var strings = this.getLang();
        if(strings != null)
           var label = strings[page][id];
        else
             return;
		if(label != null)
		{
		...
		} else 
		{
			...
			if(label != null)
				return label;
			return "["+sessionGet("selectedLang")+", "+ page +", "+id+"]";
		}
	},
	
	getPageLabels : function(page)
	{
		var strings = this.getLang();
        if(strings != null)
           return strings[page];
        else
			return "["+sessionGet("selectedLang")+", "+ page +"]";
	}
},
/** @Prototype */
{
...
f.cur().path;this.options={fn:function(){f.cur(b);a(d.send||h.jQuery||h.Zepto||d)},rootSrc:b,orig:a,type:"fn"};this.waits=true;this.unique=false}else{this.orig=a;this.options=d.makeOptions(l({},typeof a=="string"?{src:a}:a));this.waits=this.options.waits||false;this.unique=true}else{this.options={};this.waits=false;this.pack="production.js"}},complete:function(){this.completed=true},loaded:function(a){var b,c;a=a&&a.src||this.options.src;f.cur(this.options.rootSrc);this.isLoaded=true;if(r.interactive&&
a)b=n[a];if(!b){b=s.slice(0);s=[]}if(b.length){var e=this,g,k=d.options.env=="production",i=[],v=function(E,u,M,N){var O=[M,N];m(E,function(S,da){O.unshift(da,u)});w.apply(d,O)},T=function(E,u,M,N){m(M,function(O,S){w(E,u,S,N)})};m(b.reverse(),function(E,u){if(!(k&&u.ignore)){c=d.p.make(u);e.dependencies.unshift(c);if(c.waits===false)i.push(c);else{if(g){v(i.concat(c),"complete",g,"load");T(c,"complete",i.length?i:[g],"load")}else{v(i.concat(c),"complete",e,"complete");i.length&&T(c,"complete",i,
"load")}g=c;i=[]}}});if(i.length){g?v(i,"complete",g,"load"):v(i,"complete",e,"complete");m(i.reverse(),function(){this.load()})}else g?g.load():e.complete()}else this.complete()},load:function(){if(!(this.loading||this.isLoaded)){this.loading=true;var a=this;d.require(this.options,function(b){a.loaded(b)},function(){h.clearTimeout&&h.clearTimeout(a.completeTimeout);throw"steal.js : "+a.options.src+" not completed";})}}};d.p.init.prototype=d.p;var U;l(d,{root:f(""),rootUrl:function(a){if(a!==undefined){d.root=
...
4){if(e.status===500||e.status===404||e.status===2||e.status<0||e.status===0&&e.responseText==="")c&&c(e.status);else b(e.responseText);k()}};e.open("GET",a.src,a.async===false?false:true);e.setRequestHeader("Content-type",g);e.overrideMimeType&&e.overrideMimeType(g);e.onreadystatechange=function(){i()};try{e.send(null)}catch(v){if(k){console.error(v);c&&c();k()}}};var Z=function(a){var b,c;for(b in d.mappings){c=d.mappings[b];if(c.test.test(a))return a.replace(b,c.path)}return a};f.prototype.mapJoin=
function(a){a=Z(a);return f(a).joinFrom(this.path)};d.makeOptions=q(d.makeOptions,function(a){a.src=d.root.join(a.rootSrc=Z(a.rootSrc))});d.mappings={};d.map=function(a,b){if(typeof a=="string")d.mappings[a]={test:new RegExp("^("+a+")([/.]|$)"),path:b};else for(var c in a)d.map(c,a[c]);return this};var G;l(d,{after:function(){if(!G){var a=G=new d.p.init,b=function(){d.trigger("start",a);w(a,"complete",function(){d.trigger("end",a)});a.loaded()};h.setTimeout?setTimeout(b,0):b()}},_before:p,_after:q});
d.p.complete=p(d.p.complete,function(){if(this===G)G=null});(function(){var a=false,b,c=false;d.p.loaded=p(d.p.loaded,function(){var e=typeof jQuery!=="undefined"?jQuery:null;if(e&&"readyWait"in e)if(!a){b=e;e.readyWait+=1;a=true}});d.bind("end",function(){if(a&&!c){b.ready(true);c=true}})})();d.p.load=q(d.p.load,function(){if(h.document&&!this.completed&&!this.completeTimeout&&!d.isRhino&&(this.options.protocol=="file:"||!r.error)){var a=this;this.completeTimeout=setTimeout(function(){throw"steal.js : "+
a.options.src+" not completed";},5E3)}});d.p.complete=q(d.p.complete,function(){this.completeTimeout&&clearTimeout(this.completeTimeout)});l(Q.prototype,{called:function(){this.calls--;this.go()},add:function(a,b){a=C(a,b);if(!a.called){a.callbacks.push(this);this.calls++}},go:function(){this.calls===0&&this.obj[this.meth]()}});var H={load:function(){},end:function(){}},I=false;(function(a,b,c){if(a.addEventListener)a.addEventListener(b,c,false);else a.attachEvent?a.attachEvent("on"+b,c):c()})(h,
"load",function(){H.load()});d.one("end",function(a){H.end();I=a;d.trigger("done",I)});w(H,"load",H,"end",function(){d.trigger("ready");d.isReady=true});d.events.done={add:function(a){if(I){a(I);return false}else return a}};d.p.make=q(d.p.make,function(a){if(a.options.has)a.isLoaded?a.loadHas():d.loading.apply(d,a.options.has);return a},true);d.p.loaded=p(d.p.loaded,function(){this.options.has&&this.loadHas()});d.p.loadHas=function(){var a,b,c=f.cur();for(b=0;b<this.options.has.length;b++){f.cur(c);
a=d.p.make(this.options.has[b]);C(a,"complete");a.loaded()}};var J,n={},$=function(){var a,b,c=j.getElementsByTagName("script");for(a=c.length-1;a>-1&&(b=c[a]);a--)if(b.readyState==="interactive")return b},aa=function(){var a;if(J&&J.readyState==="interactive")return J;if(a=$())return J=a;if(F&&F.readyState=="interactive")return F;return null};r.interactive=j&&!!$();if(r.interactive){d.after=q(d.after,function(){var a=aa();if(!(!a||!a.src||/steal\.(production|production\.[a-zA-Z0-9\-\.\_]*)*js/.test(a.src))){a=
a.src;n[a]||(n[a]=[]);if(a){n[a].push.apply(n[a],s);s=[]}}});d.preloaded=p(d.preloaded,function(a){a=a.options.src;var b=aa().src;n[a]=n[b];n[b]=null})}var ba=/steal\.(production\.)?js.*/,ha=function(){if(j)for(var a=j.getElementsByTagName("script"),b=0,c=a.length;b<c;b++){var e=a[b].src;if(e&&ba.test(e))return a[b]}};d.getScriptOptions=function(a){a=a||ha();var b={};if(a){a=a.src;var c=a.replace(ba,"");b.rootUrl=/steal\/$/.test(c)?c.substr(0,c.length-6):c+"../";if(/steal\.production\.js/.test(a))b.env=
"production";if(a.indexOf("?")!==-1){a=a.split("?")[1];a=a.split(",");if(a[0])b.startFile=a[0];if(a[1]&&d.options.env!="production")b.env=a[1]}}return b};x=q(x,function(){var a=d.options,b=[];l(a,d.getScriptOptions());typeof D=="object"&&l(a,D);var c=h.location&&decodeURIComponent(h.location.search);c&&c.replace(/steal\[([^\]]+)\]=([^&]+)/g,function(g,k,i){g=i.split(",");if(g.length>1)i=g;a[k]=i});d.rootUrl(a.rootUrl);if(a.startFile&&a.startFile.indexOf(".")=="-1")a.startFile=a.startFile+"/"+a.startFile.match(/[^\/]+$/)[0]+
".js";if(!a.logLevel)a.logLevel=0;if(!a.production&&a.startFile)a.production=f(a.startFile).dir()+"/production.js";if(a.production)a.production+=a.production.indexOf(".js")==-1?".js":"";m(a.loaded||[],function(g,k){d.loaded(k)});if(typeof a.startFiles==="string")b.push(a.startFiles);else if(a.startFiles&&a.startFiles.length)b=a.startFiles;c=[];if(b.length){d.options.startFiles=b;c.push.apply(c,b)}try{if(a.instrument||!a.browser&&h.top&&h.top.opener&&h.top.opener.steal&&h.top.opener.steal.options.instrument)c.push(function(){},
{src:"steal/instrument",waits:true})}catch(e){}if(a.env=="production"&&a.loadProduction)a.production&&d({src:a.production,force:true});else{a.loadDev!==false&&c.unshift({src:"steal/dev/dev.js",ignore:true});a.startFile&&c.push(a.startFile)}c.length&&d.apply(null,c)});d.when=w;h.steal=d;x()})();
