...
//Create the map then make 'displayDirections' request on page init only
 ...
 /*****************************/
            ...
            $('#index_page').live('pagebeforeshow',function(event, ui){     
                var date= new Date();
                $("#temperature").html("Πάτρα "+ date.getDate().toString() + '/' + (date.getMonth()+1).toString() + '/' +  date.getFullYear().toString());
            });
...
{  
 ...
 $('#index_page').live('pagebeforeshow',function(){
      ...
      //-----------init configuration------------------------------------------
        sessionStorage.setItem("sitename", 'http://patra.livecity.gr');
        sessionStorage.setItem("city", '25322');
        sessionStorage.setItem("lang", '1');
        sessionStorage.setItem("newsrootpage", '11');
        
         sessionStorage.setItem("latmap","38.217413");
         sessionStorage.setItem("longmap","21.741600");
