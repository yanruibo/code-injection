...
var BaseScene, BonusStageScene, BonusStageText, Box, Button, Chara, ClassDown, ClassUp, ConnectError, CountDownScene, Counter, Diploma, EtcButton, EtcScene, FloorScene, FriendButton, FriendScene, GameoverScene, GradeButton, Graph, ImageCharacter, ImageStageScene, InputNicknameScene, LogButton, LogScene, MyCore, NetWorkErrorScene, PauseButton, PauseScene, PlayButton, PreGame1, PreGame2, PreGameScene, Problems, RankButton, RankFromGrade, RankSelfButton, RankTopButton, RankingScene, RateUp, RatingLogScene, Result, Score, StageScene, StageScore, StageScoreScene, Stamp, TextCharacter, TitleScene, TopButton, UpdateNotice, WhiteLabel, rating_jp,
...
MyCore = (function(_super) {
  ...
  function MyCore() {
    ...
    this.preload('character.png', 'buttons.png', 'images.png', 'help.png', 'effect.png');
    this.text = {};
    ...
    this.app_id = 'info.real_unreal.find_difference.g' + this.first + this.third;
...
Diploma = (function(_super) {
  ...
  Diploma.prototype.show = function(opt) {
    ...
    this.date.text = $('#text_heisei').text() + (rating_year - 1989) + $('#text_year').text() + rating_month + $('#text_month').text() + rating_day + $('#text_day').text();
...
FriendScene = (function(_super) {
  ...
  function FriendScene() {
    var button, core, description, description2, description3, etc, friend, log, ranking, top;
...
GameoverScene = (function(_super) {
  ...
  function GameoverScene(opt) {
    var box, core, diploma, gameover, rateup_effect, result, scene, score, top, tweet_explain, white;
  ...
  GameoverScene.prototype.send_score = function() {
    ...
    return $.ajax({
      ...
      error: function(error) {
        return scene.send_score_error();
      }
    });
  };

  GameoverScene.prototype.send_score_error = function() {
    var board, button_cancel, button_retry, core, description, description_title, description_warnings, scene;
    core = Core.instance;
    scene = this;
    scene.error_dialog = new Array;
    $('#loading').hide();
    board = new Sprite(250, 250);
    board.image = core.assets['images.png'];
    board.frame = 1;
    ...
    scene.addChild(button_retry);
    scene.error_dialog.push(button_retry);
    ...
    button_cancel.addEventListener(Event.TOUCH_END, function() {
      ...
      $('#loading').hide();
      scene.show_tweet_button();
      scene.removeChild(scene.white);
      _ref = scene.error_dialog;
      _results = [];
      for (_i = 0, _len = _ref.length; _i < _len; _i++) {
        item = _ref[_i];
        _results.push(scene.removeChild(item));
      }
      return _results;
    });
    scene.addChild(button_cancel);
    scene.error_dialog.push(button_cancel);
    ...
    scene.addChild(description_title);
    scene.error_dialog.push(description_title);
    description = new Label;
    ...
    scene.error_dialog.push(description);
    ...
    scene.addChild(description_warnings);
    return scene.error_dialog.push(description_warnings);
  };
...
RankingScene = (function(_super) {
  ...
  function RankingScene(opt) {
    var button, button_self, button_top, chara, core, etc, friend, grade, i, log, message, rank_button, ranking, scene, target_button, top, _i, _j, _len, _len1, _ref;
    ...
    if (core.ranking_available) {
      ...
      target_button = new Array;
      button_top = new RankTopButton;
      rank_button.push(button_top);
      target_button.push(button_top);
      button_self = new RankSelfButton;
      rank_button.push(button_self);
      target_button.push(button_self);
      scene.rank_button = rank_button;
      scene.target_button = target_button;
      for (_j = 0, _len1 = rank_button.length; _j < _len1; _j++) {
        button = rank_button[_j];
        scene.addChild(button);
        button.off();
      }
      scene.addEventListener(Event.ENTER_FRAME, function() {
        scene.removeEventListener(Event.ENTER_FRAME, arguments.callee);
        rank_button[scene.rank_num].on();
        target_button[scene.target_num].on();
        return scene.load_graph();
      });
    } else {
      chara = new Chara(5, 240, 160);
      chara.x = 40;
      chara.y = 130;
  ...
  RankingScene.prototype.load_graph = function() {
    var button, core, from, listview, scene, section, target, _i, _len, _ref;
    core = Core.instance;
    scene = this;
    ...
    return $.ajax({
      ...
      success: function(json) {
        var button, cp, cr, item, nickname, point, rank, rank_value, user, _j, _k, _len1, _len2, _ref1, _ref2, _results;
        $('#loading').hide();
        if (json.version && json.version > core.version) {
          scene.addChild(new UpdateNotice(1));
          return;
        }
        if (scene.listview && json.ranking) {
          _ref1 = json.ranking;
      ...
      error: function(error) {
        var button, _j, _len1, _ref1;
        $('#loading').hide();
        _ref1 = scene.rank_button;
        for (_j = 0, _len1 = _ref1.length; _j < _len1; _j++) {
          button = _ref1[_j];
...
ConnectError = (function(_super) {
  ...
  function ConnectError() {
    ...
    message = new Label;
    message.text = $('#text_error_description').html();
    message.font = '14px sans-serif';
    message.textAlign = 'center';
    message.x = 10;
    this.addChild(message);
...
TitleScene = (function(_super) {
  ...
  function TitleScene() {
    var chara, core, etc, friend, grade, guest_play, log, ranking, scene, start, title, top, version;
    ...
    guest_play.addEventListener(Event.TOUCH_END, function() {
      ...
      core.guest_play = 1;
      core.removeScene(core.currentScene);
      pregame = new PreGameScene;
      core.pushScene(pregame);
      pregame.pre2();
      return $('#default_line').hide();
    });
