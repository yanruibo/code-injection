    		...
    		$(document).ready(function() {
                $("#shareButtonLink").click(function() {
                    window.plugins.share.show({
                        subject: "Share Take One Videos",
                        text: "https://play.google.com/store/apps/details?id=com.a156444959951d2d3ba84bfb6a.a90470007a"},
...
var end = "17411 N. Eureka Ct Surprise AZ 85374";
  calcRoute(start,end);
}
  ...
  return string;
                      ...
                      $(document).ready(function() {
                          $("#submit").click(function() {
                              ...
                              extras[WebIntent.EXTRA_EMAIL] = "info@takeonevideos.com";
                              window.plugins.webintent.startActivity({
                                  action: WebIntent.ACTION_SEND,
                                  type: 'text/plain', 
                                  extras: extras
...
function initializeData(){	
	...
	if (localStorage.getItem("disclaimer") == null || localStorage.getItem("disclaimer") != 1){
		//alert(disclaimerMessage + ", Take One Videos");
				
		document.addEventListener("deviceready", function(){
			navigator.notification.alert(disclaimerMessage, false, "Take One Videos", "OK");
...
function saveLocation(){
	...
	if(localStorage.getItem("acc") != null)
		navigator.notification.confirm(confirmOverwrite, onConfirmSave, "Take One Videos", "OK,Cancel");
	else
		onConfirmSave(1);
...
function checkAccuracy(start){
	...
	if (geoSuccess == false){
		navigator.notification.alert(noGPSMessage, false, "Take One Videos", "OK");
		...
		navigator.notification.alert(notifyPositionSaved, false, "Take One Videos", "OK");
	}
		...
		if (!allowReducedAcc && !gettingInput){
			...
			navigator.notification.confirm(allowReducedAccMessage, function(button){
				...
				if (allowReducedAcc){
					...
					gpsLoading.style.visibility = "hidden";
					//alert(errorPoorAcc);
					navigator.notification.alert(errorPoorAcc, false, "Take One Videos", "OK");
				}
				gettingInput = false;
			}, "Take One Videos", "OK,Cancel");
		}
...
function loadCanvas()
{
	...
	if(canvas.getContext("2d"))
	{
  	...
  	{
  		navigator.notification.alert(errorUnsupportedFeature, false, "Take One Videos", "OK");
  	}
