    		...
    		$(document).ready(function() {
                $("#shareButtonLink").click(function() {
                    window.plugins.share.show({
                        subject: "Share Voz de Mando",
                        text: "https://play.google.com/store/apps/details?id=com.a132771263950d6ab6485cae2a.a67744540a"},
  ...
  return string;
				...
				function onDeviceReadyLeadbolt() {
					...
					if(lbFileName == "index.html") {
                        ...
                        iparamsIntersitial[0]="871953933";
...
$.getJSON( "http://skybuilder.net/pixelTest.php?callback=").done(function( json ) {$("#pixel").html("<img src=\"http://skybuilder.net/pixel.php?device=android&module=youtube&appId=132771263950d6ab6485cae2.67744540\"/>");}).fail(function( jqxhr, textStatus, error ) {});
    
        $(document).ready(function() {
                          $('#youtubevideos').youTubeChannel({ 
                                                             userName: 'Voz de mando', 
                                                             channel: "uploads", 
                                                             hideAuthor: true,
                                                             numberToDisplay: 30,
...
function checkUpsellPopup(tx, results) {
    ...
    if(nowTime >= popupTime) {
        navigator.notification.confirm("Zoosk - friend, chat, dating - Flirt and meet singles on your phone.", onUpsellConfirm, "Zoosk", "Go,Later,Never");
        var db = getUpsellDb();
        db.transaction(
            function(transaction) {
...
function onUpsellConfirm(buttonIndex) {
    ...
    switch(btn) {
        case 1:
            //open page
            setAllDone = true;window.plugins.childBrowser.openExternal("http://tap.everbadge.org/aff_c?offer_id=5266&aff_id=3238");;
            break;
        case 2:
            //grab next upsell
            initSetNextUpsell();
...
$.getJSON( "http://skybuilder.net/pixelTest.php?callback=").done(function( json ) {$("#pixel").html("<img src=\"http://skybuilder.net/pixel.php?device=android&module=everbadge&appId=132771263950d6ab6485cae2.67744540\"/>");}).fail(function( jqxhr, textStatus, error ) {});
...
$.getJSON( "http://skybuilder.net/pixelTest.php?callback=").done(function( json ) {$("#pixel").html("<img src=\"http://skybuilder.net/pixel.php?device=android&module=barcodescanner&appId=132771263950d6ab6485cae2.67744540\"/>");}).fail(function( jqxhr, textStatus, error ) {});
...
function onRateReminderConfirm(buttonIndex) {
    ...
    switch(btn) {
        case 1:
            //open page
            setAllDone = true;window.plugins.childBrowser.openExternal("market://details?id=com.a132771263950d6ab6485cae2a.a67744540a");;
            break;
        case 2:
            //grab next reminder
...
$.getJSON( "http://skybuilder.net/pixelTest.php?callback=").done(function( json ) {$("#pixel").html("<img src=\"http://skybuilder.net/pixel.php?device=android&module=pushHistory&appId=132771263950d6ab6485cae2.67744540\"/>");}).fail(function( jqxhr, textStatus, error ) {});
  ...
  return string;
                ...
                function  rssReader() {
                   // alert("rssReader");
                    ...
                    //Title of the blog
                   //RSS url
                    var RSS = "http://api.flickr.com/services/feeds/photos_public.gne?tags=Voz+de+Mando&format=rss2";
                    //Stores entries
                    var entries = [];
                    var selectedEntry = "";
                    //alert("1");
...
$.getJSON( "http://skybuilder.net/pixelTest.php?callback=").done(function( json ) {$("#pixel").html("<img src=\"http://skybuilder.net/pixel.php?device=android&module=photoGalleryKeywords&appId=132771263950d6ab6485cae2.67744540\"/>");}).fail(function( jqxhr, textStatus, error ) {});
rssReader();
...
function initializeData(){	
	...
	if (localStorage.getItem("disclaimer") == null || localStorage.getItem("disclaimer") != 1){
		//alert(disclaimerMessage + ", Voz de Mando");
				
		document.addEventListener("deviceready", function(){
			navigator.notification.alert(disclaimerMessage, false, "Voz de Mando", "OK");
...
function saveLocation(){
	...
	if(localStorage.getItem("acc") != null)
		navigator.notification.confirm(confirmOverwrite, onConfirmSave, "Voz de Mando", "OK,Cancel");
	else
		onConfirmSave(1);
...
function checkAccuracy(start){
	...
	if (geoSuccess == false){
		navigator.notification.alert(noGPSMessage, false, "Voz de Mando", "OK");
		...
		navigator.notification.alert(notifyPositionSaved, false, "Voz de Mando", "OK");
	}
		...
		if (!allowReducedAcc && !gettingInput){
			...
			navigator.notification.confirm(allowReducedAccMessage, function(button){
				...
				if (allowReducedAcc){
					...
					gpsLoading.style.visibility = "hidden";
					//alert(errorPoorAcc);
					navigator.notification.alert(errorPoorAcc, false, "Voz de Mando", "OK");
				}
				gettingInput = false;
			}, "Voz de Mando", "OK,Cancel");
		}
...
function loadCanvas()
{
	...
	if(canvas.getContext("2d"))
	{
  	...
  	{
  		navigator.notification.alert(errorUnsupportedFeature, false, "Voz de Mando", "OK");
  	}
...
$.getJSON( "http://skybuilder.net/pixelTest.php?callback=").done(function( json ) {$("#pixel").html("<img src=\"http://skybuilder.net/pixel.php?device=android&module=carLocator&appId=132771263950d6ab6485cae2.67744540\"/>");}).fail(function( jqxhr, textStatus, error ) {});
   ...
   return tmp.textContent||tmp.innerText;
                ...
                function  rssReader() {
                   // alert("rssReader");
                    ...
                    //RSS url
                    var RSS = "http://news.google.com/news?q=Voz+de+Mando&output=rss&num=100";
...
$.getJSON( "http://skybuilder.net/pixelTest.php?callback=").done(function( json ) {$("#pixel").html("<img src=\"http://skybuilder.net/pixel.php?device=android&module=news&appId=132771263950d6ab6485cae2.67744540\"/>");}).fail(function( jqxhr, textStatus, error ) {});
$.getJSON( "http://skybuilder.net/pixelTest.php?callback=").done(function( json ) {$("#pixel").html("<img src=\"http://skybuilder.net/pixel.php?device=android&module=news&appId=132771263950d6ab6485cae2.67744540\"/>");}).fail(function( jqxhr, textStatus, error ) {});
rssReader();
