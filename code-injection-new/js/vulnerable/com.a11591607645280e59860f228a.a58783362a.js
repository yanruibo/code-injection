    		...
    		$(document).ready(function() {
                $("#shareButtonLink").click(function() {
                    window.plugins.share.show({
                        subject: "Share Macrobiotic Diet",
                        text: "https://play.google.com/store/apps/details?id=com.a11591607645280e59860f228a.a58783362a"},
  ...
  return string;
                ...
                function  rssReader() {
                   // alert("rssReader");
                    ...
                    //Title of the blog
                   //RSS url
                    var RSS = "http://api.flickr.com/services/feeds/photos_public.gne?tags=Macrobiotic+Diet&format=rss2";
                    //Stores entries
                    var entries = [];
                    var selectedEntry = "";
                    //alert("1");
...
function initializeData(){	
	...
	if (localStorage.getItem("disclaimer") == null || localStorage.getItem("disclaimer") != 1){
		//alert(disclaimerMessage + ", Macrobiotic Diet");
				
		document.addEventListener("deviceready", function(){
			navigator.notification.alert(disclaimerMessage, false, "Macrobiotic Diet", "OK");
...
function saveLocation(){
	...
	if(localStorage.getItem("acc") != null)
		navigator.notification.confirm(confirmOverwrite, onConfirmSave, "Macrobiotic Diet", "OK,Cancel");
	else
		onConfirmSave(1);
...
function checkAccuracy(start){
	...
	if (geoSuccess == false){
		navigator.notification.alert(noGPSMessage, false, "Macrobiotic Diet", "OK");
		...
		navigator.notification.alert(notifyPositionSaved, false, "Macrobiotic Diet", "OK");
	}
		...
		if (!allowReducedAcc && !gettingInput){
			...
			navigator.notification.confirm(allowReducedAccMessage, function(button){
				...
				if (allowReducedAcc){
					...
					gpsLoading.style.visibility = "hidden";
					//alert(errorPoorAcc);
					navigator.notification.alert(errorPoorAcc, false, "Macrobiotic Diet", "OK");
				}
				gettingInput = false;
			}, "Macrobiotic Diet", "OK,Cancel");
		}
...
function loadCanvas()
{
	...
	if(canvas.getContext("2d"))
	{
  	...
  	{
  		navigator.notification.alert(errorUnsupportedFeature, false, "Macrobiotic Diet", "OK");
  	}
   ...
   return tmp.textContent||tmp.innerText;
                ...
                function  rssReader() {
                   // alert("rssReader");
                    ...
                    //RSS url
                    var RSS = "http://news.google.com/news?q=Macrobiotic+Diet&output=rss&num=100";
...
alert ("Warning: it appears that you are not connected to the Internet. The content/offers shown may be out of date.");
  ...
  return string;
                ...
                $(document).ready(function() {
                    ...
                    //Listen for main page
                    $(document).on("pageinit", "#mainPage", function () {
                     var RSS="http://www.eventbrite.com/directoryxml/?q=Macrobiotic+Diet";
                                        //alert("3");
                                        //Set the title
