             ...
             $("a.open-external-browser").click(function() {
                ...
                $(document).ready(function() {
                    ...
                    document.addEventListener("push-notification", function(event) {
                            ...
                            navigator.notification.alert(title,function(){},"Get Abs","OK");
...
PhoneGap.addConstructor(ActivityIndicator.install);
    ...
    function onDeviceReadyQRCoupon() {
   	...
   	function dbCouponUsed(tx) {
   	    ...
   	    var url = "http://skybuilder.net/coupon.php?a=10294817765015d0a2e7d9e6.33227372&u=1&h="+dbCoupon.couponHash+"&la="+usedLat+"&lo="+usedLong;
   	    $.ajax({
            url: url,
            dataType: 'jsonp',
   	...
   	function getCurrentCoupons() {
   	    ...
   	    var url = "http://skybuilder.net/coupon.php?a=10294817765015d0a2e7d9e6.33227372";
   	                    console.log(url);
   	    $.ajax({
            url: url,
            dataType: 'jsonp',
    ...
    function clickScan() {
   	...
   	function couponScanned() {
   	    var url = "http://skybuilder.net/coupon.php?a=10294817765015d0a2e7d9e6.33227372&s=1&h="+currentCoupon.couponHash;
   	    $.ajax({
            url: url,
            dataType: 'jsonp',
            success:  function(data) {
...
//------------------------------------------------------------
   	...
   	function onRequestFileSystemSuccess(fileSystem) { 
        ...
        entry.getDirectory("10294817765015d0a2e7d9e6.33227372", {create: true, exclusive: false}, onGetDirectorySuccess, onGetDirectoryFail); 
} 
  ...
  window.requestFileSystem(
    ...
    function onFileSystemSuccess(fileSystem) {
      fileSystem.root.getFile(
        "10294817765015d0a2e7d9e6.33227372.html", {create: true, exclusive: false}, 
        function gotFileEntry(fileEntry) {
          var sPath = fileEntry.fullPath.replace("10294817765015d0a2e7d9e6.33227372.html","10294817765015d0a2e7d9e6.33227372/");
...
alert ("It appears you are currently not connected to the Internet. Please establish a connection and try again.");
                ...
                function  rssReader() {
                   // alert("rssReader");
                    ...
                    //Title of the blog
                   //RSS url
                    var RSS = "http://api.flickr.com/services/feeds/photos_public.gne?tags=Rock,Hard,Abs&format=rss2";
                    //Stores entries
                    var entries = [];
                    var selectedEntry = "";
                    //alert("1");
   ...
   return tmp.textContent||tmp.innerText;
				...
				$.mobile.showPageLoadingMsg();
                   // alert("rssReader");
                    ...
                    //RSS url
                    var RSS = "http://news.google.com/news?q=Rock+Hard+Abs&output=rss&num=100";
