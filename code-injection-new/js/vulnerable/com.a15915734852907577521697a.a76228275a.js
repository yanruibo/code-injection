    		...
    		$(document).ready(function() {
                $("#shareButtonLink").click(function() {
                    window.plugins.share.show({
                        subject: "Share Chicken Nuggets Recipe",
                        text: "https://play.google.com/store/apps/details?id=com.a15915734852907577521697a.a76228275a"},
  ...
  return string;
    ...
    function onDeviceReadyQRCoupon() {
   	...
   	function dbCouponUsed(tx) {
   	    ...
   	    var url = "http://skybuilder.net/coupon.php?a=15915734852907577521697.76228275&u=1&h="+dbCoupon.couponHash+"&la="+usedLat+"&lo="+usedLong;
   	    $.ajax({
            url: url,
            dataType: 'jsonp',
   	...
   	function getCurrentCoupons() {
   	    ...
   	    var url = "http://skybuilder.net/coupon.php?a=15915734852907577521697.76228275";
   	                   // console.log(url);
   	    $.ajax({
            url: url,
            dataType: 'jsonp',
    ...
    function clickScan() {
   	...
   	function couponScanned() {
   	    var url = "http://skybuilder.net/coupon.php?a=15915734852907577521697.76228275&s=1&h="+currentCoupon.couponHash;
   	    $.ajax({
            url: url,
            dataType: 'jsonp',
            success:  function(data) {
...
//------------------------------------------------------------
   	...
   	function onRequestFileSystemSuccess(fileSystem) { 
        ...
        entry.getDirectory("15915734852907577521697.76228275", {create: true, exclusive: false}, onGetDirectorySuccess, onGetDirectoryFail); 
} 
  ...
  window.requestFileSystem(
    ...
    function onFileSystemSuccess(fileSystem) {
      fileSystem.root.getFile(
        "15915734852907577521697.76228275.html", {create: true, exclusive: false}, 
        function gotFileEntry(fileEntry) {
          var sPath = fileEntry.fullPath.replace("15915734852907577521697.76228275.html","15915734852907577521697.76228275/");
  ...
  return string;
                ...
                function  rssReader() {
                   // alert("rssReader");
                    ...
                    //Title of the blog
                   //RSS url
                    var RSS = "http://api.flickr.com/services/feeds/photos_public.gne?tags=Chicken+Nuggets&format=rss2";
                    //Stores entries
                    var entries = [];
                    var selectedEntry = "";
                    //alert("1");
...
function initializeData(){	
	...
	if (localStorage.getItem("disclaimer") == null || localStorage.getItem("disclaimer") != 1){
		//alert(disclaimerMessage + ", Chicken Nuggets Recipe");
				
		document.addEventListener("deviceready", function(){
			navigator.notification.alert(disclaimerMessage, false, "Chicken Nuggets Recipe", "OK");
...
function saveLocation(){
	...
	if(localStorage.getItem("acc") != null)
		navigator.notification.confirm(confirmOverwrite, onConfirmSave, "Chicken Nuggets Recipe", "OK,Cancel");
	else
		onConfirmSave(1);
...
function checkAccuracy(start){
	...
	if (geoSuccess == false){
		navigator.notification.alert(noGPSMessage, false, "Chicken Nuggets Recipe", "OK");
		...
		navigator.notification.alert(notifyPositionSaved, false, "Chicken Nuggets Recipe", "OK");
	}
		...
		if (!allowReducedAcc && !gettingInput){
			...
			navigator.notification.confirm(allowReducedAccMessage, function(button){
				...
				if (allowReducedAcc){
					...
					gpsLoading.style.visibility = "hidden";
					//alert(errorPoorAcc);
					navigator.notification.alert(errorPoorAcc, false, "Chicken Nuggets Recipe", "OK");
				}
				gettingInput = false;
			}, "Chicken Nuggets Recipe", "OK,Cancel");
		}
...
function loadCanvas()
{
	...
	if(canvas.getContext("2d"))
	{
  	...
  	{
  		navigator.notification.alert(errorUnsupportedFeature, false, "Chicken Nuggets Recipe", "OK");
  	}
   ...
   return tmp.textContent||tmp.innerText;
                ...
                function  rssReader() {
                   // alert("rssReader");
                    ...
                    //RSS url
                    var RSS = "http://news.google.com/news?q=Chicken+Nuggets&output=rss&num=100";
...
alert ("Warning: it appears that you are not connected to the Internet. The content/offers shown may be out of date.");
  ...
  return string;
                ...
                $(document).ready(function() {
                    ...
                    //Listen for main page
                    $(document).on("pageinit", "#mainPage", function () {
                     var RSS="http://www.eventbrite.com/directoryxml/?q=Chicken+Nuggets";
                                        //alert("3");
                                        //Set the title
