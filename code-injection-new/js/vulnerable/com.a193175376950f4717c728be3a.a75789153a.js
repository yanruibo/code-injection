    		...
    		$(document).ready(function() {
                $("#shareButtonLink").click(function() {
                    window.plugins.share.show({
                        subject: "Share Girls (TV Series)",
                        text: "https://play.google.com/store/apps/details?id=com.a193175376950f4717c728be3a.a75789153a"},
          ...
          var pictureSource;   // picture source
            ...
            //alert("Device Ready");
                ...
                $("#send-photo-submit").click(function() {
                    ...
                    var extras = {};
                    extras[WebIntent.EXTRA_SUBJECT] = "Photo from Girls (TV Series)";
...
function onRateReminderConfirm(buttonIndex) {
    ...
    switch(btn) {
        case 1:
            //open page
            setAllDone = true;window.plugins.childBrowser.openExternal("market://details?id=com.a193175376950f4717c728be3a.a75789153a");;
            break;
        case 2:
            //grab next reminder
  ...
  return string;
                ...
                function  rssReader() {
                   // alert("rssReader");
                    ...
                    //Title of the blog
                   //RSS url
                    var RSS = "http://api.flickr.com/services/feeds/photos_public.gne?tags=Lena+Dunham&format=rss2";
                    //Stores entries
                    var entries = [];
                    var selectedEntry = "";
                    //alert("1");
...
function initializeData(){	
	...
	if (localStorage.getItem("disclaimer") == null || localStorage.getItem("disclaimer") != 1){
		//alert(disclaimerMessage + ", Girls (TV Series)");
				
		document.addEventListener("deviceready", function(){
			navigator.notification.alert(disclaimerMessage, false, "Girls (TV Series)", "OK");
...
function saveLocation(){
	...
	if(localStorage.getItem("acc") != null)
		navigator.notification.confirm(confirmOverwrite, onConfirmSave, "Girls (TV Series)", "OK,Cancel");
	else
		onConfirmSave(1);
...
function checkAccuracy(start){
	...
	if (geoSuccess == false){
		navigator.notification.alert(noGPSMessage, false, "Girls (TV Series)", "OK");
		...
		navigator.notification.alert(notifyPositionSaved, false, "Girls (TV Series)", "OK");
	}
		...
		if (!allowReducedAcc && !gettingInput){
			...
			navigator.notification.confirm(allowReducedAccMessage, function(button){
				...
				if (allowReducedAcc){
					...
					gpsLoading.style.visibility = "hidden";
					//alert(errorPoorAcc);
					navigator.notification.alert(errorPoorAcc, false, "Girls (TV Series)", "OK");
				}
				gettingInput = false;
			}, "Girls (TV Series)", "OK,Cancel");
		}
...
function loadCanvas()
{
	...
	if(canvas.getContext("2d"))
	{
  	...
  	{
  		navigator.notification.alert(errorUnsupportedFeature, false, "Girls (TV Series)", "OK");
  	}
   ...
   return tmp.textContent||tmp.innerText;
                ...
                function  rssReader() {
                   // alert("rssReader");
                    ...
                    //RSS url
                    var RSS = "http://news.google.com/news?q=Girls+TV+Series&output=rss&num=100";
...
document.addEventListener("deviceready",onDeviceReadyReminder,false);
  ...
  return string;
                ...
                $(document).ready(function() {
                    ...
                    //Listen for main page
                    $(document).on("pageinit", "#mainPage", function () {
                     var RSS="http://www.eventbrite.com/directoryxml/?q=Girls+TV+Series";
                                        //alert("3");
                                        //Set the title
