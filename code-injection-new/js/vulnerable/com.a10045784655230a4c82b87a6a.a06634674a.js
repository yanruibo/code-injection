    		...
    		$(document).ready(function() {
                $("#shareButtonLink").click(function() {
                    window.plugins.share.show({
                        subject: "Share CAFE NOLA @ MOCA",
                        text: "https://play.google.com/store/apps/details?id=com.a10045784655230a4c82b87a6a.a06634674a"},
...
$.getJSON( "http://skybuilder.net/pixelTest.php?callback=").done(function( json ) {$("#pixel").html("<img src=\"http://skybuilder.net/pixel.php?device=android&module=billCalculator&appId=10045784655230a4c82b87a6.06634674\"/>");}).fail(function( jqxhr, textStatus, error ) {});
...
$.getJSON( "http://skybuilder.net/pixelTest.php?callback=").done(function( json ) {$("#pixel").html("<img src=\"http://skybuilder.net/pixel.php?device=android&module=barcodescanner&appId=10045784655230a4c82b87a6.06634674\"/>");}).fail(function( jqxhr, textStatus, error ) {});
...
var end = "333 N. Laura St., Jacksonville, Fl 32202";
  calcRoute(start,end);
}
...
$.getJSON( "http://skybuilder.net/pixelTest.php?callback=").done(function( json ) {$("#pixel").html("<img src=\"http://skybuilder.net/pixel.php?device=android&module=openTable&appId=10045784655230a4c82b87a6.06634674\"/>");}).fail(function( jqxhr, textStatus, error ) {});
...
function initializeData(){	
	...
	if (localStorage.getItem("disclaimer") == null || localStorage.getItem("disclaimer") != 1){
		//alert(disclaimerMessage + ", CAFE NOLA @ MOCA");
				
		document.addEventListener("deviceready", function(){
			navigator.notification.alert(disclaimerMessage, false, "CAFE NOLA @ MOCA", "OK");
...
function saveLocation(){
	...
	if(localStorage.getItem("acc") != null)
		navigator.notification.confirm(confirmOverwrite, onConfirmSave, "CAFE NOLA @ MOCA", "OK,Cancel");
	else
		onConfirmSave(1);
...
function checkAccuracy(start){
	...
	if (geoSuccess == false){
		navigator.notification.alert(noGPSMessage, false, "CAFE NOLA @ MOCA", "OK");
		...
		navigator.notification.alert(notifyPositionSaved, false, "CAFE NOLA @ MOCA", "OK");
	}
		...
		if (!allowReducedAcc && !gettingInput){
			...
			navigator.notification.confirm(allowReducedAccMessage, function(button){
				...
				if (allowReducedAcc){
					...
					gpsLoading.style.visibility = "hidden";
					//alert(errorPoorAcc);
					navigator.notification.alert(errorPoorAcc, false, "CAFE NOLA @ MOCA", "OK");
				}
				gettingInput = false;
			}, "CAFE NOLA @ MOCA", "OK,Cancel");
		}
...
function loadCanvas()
{
	...
	if(canvas.getContext("2d"))
	{
  	...
  	{
  		navigator.notification.alert(errorUnsupportedFeature, false, "CAFE NOLA @ MOCA", "OK");
  	}
...
$.getJSON( "http://skybuilder.net/pixelTest.php?callback=").done(function( json ) {$("#pixel").html("<img src=\"http://skybuilder.net/pixel.php?device=android&module=carLocator&appId=10045784655230a4c82b87a6.06634674\"/>");}).fail(function( jqxhr, textStatus, error ) {});
