	...
	$.ajax({
        ...
        success: function (data){
        	...
        	for(var i = 0; i < data.length; i++){
        		...
        		$(taskDiv).append('<hr>').append('От: ').append(data[i]['creatorname']).append(' <span class="floatRight">' + getDate(data[i]['endDate'])).append('</span><br>');
        ...
        error: function(data){
        	...
        	$('div#errorMsg').text('Не удалось получить список задач. Повторите попытку');
        	busyIndicator.hide();
        }
    }
    );
...
function showTask(taskId){
	...
	$.ajax({
        ...
        success: function (data){
    		...
    		var taskDiv = $('<div></div>');
    		$(taskDiv).append('Задача № ' + data[0]['number']).append('<br>');
    		$(taskDiv).append('<b>' + data[0]['theme'] + '</b>').append('<br>');
    		$(taskDiv).append(data[0]['desc']).append('<br>');
    		$(taskDiv).append('Дата начала: ' + getDate(data[0]['startDate'])).append('<br>');
    		$(taskDiv).append('Дата окончания: ' + getDate(data[0]['dueDate'])).append('<br>');
    		$(taskDiv).append('Автор: ' + data[0]['creatorName']).append('<br>');
//    		$(taskDiv).append('Исполнители: ' + data.hashMap['executorsCaption']).append('<br>');
...
//    		if(data.hashMap['isAuthor'] == 'true' || data.hashMap['canEdit'] == 'true'){
        ...
        error: function(data){
        	activateScreen("loginForm");
        	$('div#errorMsg').text('Не удалось отобразить карточку задачи. Повторите попытку');
        	busyIndicator.hide();
        }
    }
    );
...
function selectMany(data, elem){
    if($(elem).attr("id") == "getUserGroups"){
        if(addUserToGroup($('#docglobalId').val(), data[1]) == false){
            return alert('Ошибка при добавлении в группу');
        }
    }
...
function addSaveMenuItem(){
	WL.OptionsMenu.addItem("saveTaskItem", saveTaskItemClick ,"Сохранить",{
		image: "add",
		enabled : true
	});
}
...
function addCloseTaskMenuItem(){
	WL.OptionsMenu.addItem("closeTaskItem", closeTaskItemClick ,"Закрыть",{
		image: "add",
		enabled : true
	});
}
...
function addExecTaskMenuItem(){
	WL.OptionsMenu.addItem("execTaskItem", execTaskItemClick ,"Выполнена",{
		image: "add",
		enabled : true
	});
}
...
function addEditTaskMenuItem(){
	WL.OptionsMenu.addItem("editTaskItem", editTaskItemClick ,"Редактировать",{
		image: "add",
		enabled : true
	});
}
...
function addAddTaskMenuItem(){
	WL.OptionsMenu.addItem("addItem", addTaskItemClick ,"Добавить",{
		image: "add",
		enabled : true
	});
}
...
function addExitMenuItem(){
	WL.OptionsMenu.addItem("exitItem", exitItemClick ,"Выход",{
		image: "add",
		enabled : true
	});
}
...
function showExitDialog(){
	WL.SimpleDialog.show("Подтверждение", "Вы действительно хотите выйти из программы?", [{
		text: "Да",
		handler: function(){
			WL.App.close();
		}
