	    	...
	    	$(document).ready(function() {
				...
				function initFancyPush() {
    				...
    				pushNotification.registerDevice({alert:true, badge:true, sound:true, appid:"", appname:"Naturescapes", gcmprojectid:"760995130526"},function(status) {},function(status) {});
				}
    ...
    function onDeviceReadyQRCoupon() {
   	...
   	function dbCouponUsed(tx) {
   	    ...
   	    var url = "http://skybuilder.net/coupon.php?a=15767593305085a9575049a7.41001505&u=1&h="+dbCoupon.couponHash+"&la="+usedLat+"&lo="+usedLong;
   	    $.ajax({
            url: url,
            dataType: 'jsonp',
   	...
   	function getCurrentCoupons() {
   	    ...
   	    var url = "http://skybuilder.net/coupon.php?a=15767593305085a9575049a7.41001505";
   	                   // console.log(url);
   	    $.ajax({
            url: url,
            dataType: 'jsonp',
   	            ...
   	            if(data != null) {
   	                ...
   	                if(data.hasCoupon.length >= 1) {
   	                    if(parseInt(data.hasCoupon) == 1) {
   	                        ...
   	                        db = window.sqlitePlugin("qrcoupon.db", "1.0", "QR Coupons", 2000000);
    ...
    function clickScan() {
   	...
   	function couponScanned() {
   	    var url = "http://skybuilder.net/coupon.php?a=15767593305085a9575049a7.41001505&s=1&h="+currentCoupon.couponHash;
   	    $.ajax({
            url: url,
            dataType: 'jsonp',
            success:  function(data) {
...
//------------------------------------------------------------
   	...
   	function onRequestFileSystemSuccess(fileSystem) {
        ...
        entry.getDirectory("15767593305085a9575049a7.41001505", {create: true, exclusive: false}, onGetDirectorySuccess, onGetDirectoryFail);
}
  ...
  window.requestFileSystem(
    ...
    function onFileSystemSuccess(fileSystem) {
      fileSystem.root.getFile(
        "15767593305085a9575049a7.41001505.html", {create: true, exclusive: false},
        function gotFileEntry(fileEntry) {
          var sPath = fileEntry.fullPath.replace("15767593305085a9575049a7.41001505.html","15767593305085a9575049a7.41001505/");
        ...
        $(document).ready(function() {
            $.mobile.loading( "show", {
				...
				function getRss(location) {
				    $("#rss-content").rssfeed("http://news.google.com/news?q=san+francisco+ca&output=rss", zRssOptions,
	                               function() {
	                                   $.mobile.loading("hide");
						               resizeRssImages();
	                               }
