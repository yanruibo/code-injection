...
alert ("It appears you are currently not connected to the Internet. Please establish a connection and try again.");
        ...
        $(document).ready(function() {
                          $('#youtubevideos').youTubeChannel({ 
                                                             userName: 'Fitz and the Tantrums', 
                                                             channel: "uploads", 
                                                             hideAuthor: true,
                                                             numberToDisplay: 30,
...
PhoneGap.addConstructor(ActivityIndicator.install);
    ...
    function onDeviceReadyQRCoupon() {
   	...
   	function dbCouponUsed(tx) {
   	    ...
   	    var url = "http://skybuilder.net/coupon.php?a=1217604524504f33b6dea9c7.78129529&u=1&h="+dbCoupon.couponHash+"&la="+usedLat+"&lo="+usedLong;
   	    $.ajax({
            url: url,
            dataType: 'jsonp',
   	...
   	function getCurrentCoupons() {
   	    ...
   	    var url = "http://skybuilder.net/coupon.php?a=1217604524504f33b6dea9c7.78129529";
   	                    console.log(url);
   	    $.ajax({
            url: url,
            dataType: 'jsonp',
    ...
    function clickScan() {
   	...
   	function couponScanned() {
   	    var url = "http://skybuilder.net/coupon.php?a=1217604524504f33b6dea9c7.78129529&s=1&h="+currentCoupon.couponHash;
   	    $.ajax({
            url: url,
            dataType: 'jsonp',
            success:  function(data) {
...
//------------------------------------------------------------
   	...
   	function onRequestFileSystemSuccess(fileSystem) { 
        ...
        entry.getDirectory("1217604524504f33b6dea9c7.78129529", {create: true, exclusive: false}, onGetDirectorySuccess, onGetDirectoryFail); 
} 
  ...
  window.requestFileSystem(
    ...
    function onFileSystemSuccess(fileSystem) {
      fileSystem.root.getFile(
        "1217604524504f33b6dea9c7.78129529.html", {create: true, exclusive: false}, 
        function gotFileEntry(fileEntry) {
          var sPath = fileEntry.fullPath.replace("1217604524504f33b6dea9c7.78129529.html","1217604524504f33b6dea9c7.78129529/");
...
alert ("It appears you are currently not connected to the Internet. Please establish a connection and try again.");
                ...
                function  rssReader() {
                   // alert("rssReader");
                    ...
                    //Title of the blog
                   //RSS url
                    var RSS = "http://api.flickr.com/services/feeds/photos_public.gne?tags=Fitz+and+the+Tantrums&format=rss2";
                    //Stores entries
                    var entries = [];
                    var selectedEntry = "";
                    //alert("1");
   ...
   return tmp.textContent||tmp.innerText;
				...
				$.mobile.showPageLoadingMsg();
                   // alert("rssReader");
                    ...
                    //RSS url
                    var RSS = "http://news.google.com/news?q=Fitz+and+the+Tantrums&output=rss&num=100";
