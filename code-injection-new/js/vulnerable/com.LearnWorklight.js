			...
			function exitOnScreenLoaderSuccess(exitOnSkinLoader){
				if (eval(exitOnSkinLoader)) {
					...
					cordova.exec(null, null, "Utils", "loadSkin", [skinName,"LearnWorklight.html"]);
				}
			};
		...
		WL.StaticAppProps = {
   "APP_DISPLAY_NAME": "LearnWorklight",
   "APP_SERVICES_URL": "\/apps\/services\/",
   "APP_VERSION": "1.0",
   "ENVIRONMENT": "android",
   "LOGIN_DISPLAY_TYPE": "embedded",
   "WORKLIGHT_ROOT_URL": "\/apps\/services\/api\/LearnWorklight\/android\/"
};
...
f[api.random]())];f.encryptor.rawEncrypt(f.keyCreationContext.DPK,a,b[0],function(a){localStorage.setItem(f.CIPHER_KEY,a);localStorage.setItem(f.SALT_KEY,f.keyCreationContext.salt);localStorage.setItem(f.VERSION,"1");a=f.keyCreationContext.onCompleteHandler;f.DPK=f.keyCreationContext.DPK;this.keyCreationContext=f.keyCreationContext=null;a&&a(OK)},function(a){console.log(a);a=f.keyCreationContext.onErrorHandler;this.keyCreationContext=f.keyCreationContext=null;if(a)a(ERROR_COULD_NOT_GENERATE_KEY);
else throw ERROR_COULD_NOT_GENERATE_KEY;})},function(a){console.log(a);a=f.keyCreationContext.onErrorHandler;this.keyCreationContext=f.keyCreationContext=null;if(a)a(ERROR_COULD_NOT_GENERATE_KEY);else throw ERROR_COULD_NOT_GENERATE_KEY;})},function(a){console.log(a);a=f.keyCreationContext.onErrorHandler;this.keyCreationContext=f.keyCreationContext=null;if(a)a(ERROR_COULD_NOT_GENERATE_KEY);else throw ERROR_COULD_NOT_GENERATE_KEY;})})}else this.keyCreationContext={onCompleteHandler:c,onErrorHandler:d},
f=this,this[api.keygen].deriveKey(a,h,f.CBK_KEY_DERIVATION_ITERATIONS,32,function(a){var b=f.keyCreationContext.onCompleteHandler,c=f.keyCreationContext.onErrorHandler;this.keyCreationContext=f.keyCreationContext=null;var d=localStorage.getItem(f.CIPHER_KEY);f.encryptor.rawDecrypt(d,a,function(d){f.DPK=d;var d=localStorage.getItem(f.VERSION),e=WL.Client.getEnvironment();("iphone"===e||"ipad"===e)&&("undefined"===typeof d||"string"!==typeof d)?reEncryptStorage(b,c,f,a):b&&b(OK)},function(a){console.log(a);
if(c)c(ERROR_CREDENTIALS_MISMATCH);else throw ERROR_CREDENTIALS_MISMATCH;})},function(a){console.log(a);a=f.keyCreationContext.onErrorHandler;this.keyCreationContext=f.keyCreationContext=null;if(a)a(ERROR_COULD_NOT_GENERATE_KEY);else throw ERROR_COULD_NOT_GENERATE_KEY;})};
function reEncryptStorage(a,b,c,d){function e(a){console.log(a);if(b)b(ERROR_MIGRATION);else throw ERROR_MIGRATION;}var g=[MD5(""+c[api.random]())];c.encryptor.rawEncrypt(c.DPK,d,g[0],function(b){localStorage.setItem(c.CIPHER_KEY,b);reEncryptKeys(a,e,c)},e)}
function reEncryptKeys(a,b,c){function d(){0<g.length?(h=g.pop(),readUsingHashedKey(h,c,e,b)):(localStorage.setItem(c.VERSION,"1"),a&&a(OK))}function e(a){writeUsingHashedKey(h,a,c,d,b)}for(var g=[],h="",f=localStorage.length-1;0<=f;f--){var m=localStorage.key(f);0===m.indexOf(c.STORAGE_PREFIX)&&g.push(m)}d()}function readUsingHashedKey(a,b,c,d){a=localStorage.getItem(a);b.encryptor.rawDecrypt(a,b.DPK,c,d)}
function writeUsingHashedKey(a,b,c,d,e){var g=c.STORAGE_PREFIX,h=a.replace(g,""),a=[MD5(h+c[api.random]())];c.encryptor.rawEncrypt(b,c.DPK,a[0],function(a){localStorage.setItem(g+h,a);d()},e)}
EncryptedCache.prototype[api.destroy]=function(a,b){WL.Validators.validateArguments([WL.Validators.validateFunctionOrNull,WL.Validators.validateFunctionOrNull],arguments,"WL.EncryptedCache.destroy");var c=null;this.close(function(){},function(a){c=a});if(null!==c){if(b)return b(c);throw c;}localStorage.removeItem(this.SALT_KEY);localStorage.removeItem(this.CIPHER_KEY);for(var d=localStorage.length-1;0<=d;d--){var e=localStorage.key(d);0===e.indexOf(this.STORAGE_PREFIX)&&localStorage.removeItem(e)}a&&
a(OK)};
EncryptedCache.prototype[api.changeCredentials]=function(a,b,c){try{WL.Validators.validateArguments(["string",WL.Validators.validateFunctionOrNull,WL.Validators.validateFunctionOrNull],arguments,"WL.EncryptedCache.changeCredentials")}catch(d){console.log(d);if(c)return c(ERROR_INVALID_PARAMETER);throw ERROR_INVALID_PARAMETER;}if(void 0===a||""===a){console.log("Error: new_credentials is undefined or empty.");if(c)return c(ERROR_INVALID_PARAMETER);throw ERROR_INVALID_PARAMETER;}if(this.keyCreationContext){if(c)return c(ERROR_KEY_CREATION_IN_PROGRESS);throw ERROR_KEY_CREATION_IN_PROGRESS;
}if(!this.DPK){if(c)return c(ERROR_EOC_CLOSED);throw ERROR_EOC_CLOSED;}this.keyCreationContext={credentials:a,salt:""+this[api.random](),onCompleteHandler:b,onErrorHandler:c};var e=this;this[api.keygen].deriveKey(this.keyCreationContext.credentials,this.keyCreationContext.salt,this.CBK_KEY_DERIVATION_ITERATIONS,32,function(a){var b=[MD5(""+e[api.random]())];e.encryptor.rawEncrypt(e.DPK,a,b[0],function(a){localStorage.setItem(e.CIPHER_KEY,a);localStorage.setItem(e.SALT_KEY,e.keyCreationContext.salt);
a=e.keyCreationContext.onCompleteHandler;this.keyCreationContext=e.keyCreationContext=null;a&&a(OK)},function(a){console.log(a);a=e.keyCreationContext.onErrorHandler;this.keyCreationContext=e.keyCreationContext=null;if(a)a(ERROR_COULD_NOT_GENERATE_KEY);else throw ERROR_COULD_NOT_GENERATE_KEY;})},function(a){console.log(a);a=e.keyCreationContext.onErrorHandler;this.keyCreationContext=e.keyCreationContext=null;if(a)a(ERROR_COULD_NOT_GENERATE_KEY);else throw ERROR_COULD_NOT_GENERATE_KEY;})};
EncryptedCache.prototype[api.write]=function(a,b,c,d){try{WL.Validators.validateArguments(["string",WL.Validators.validateStringOrNull,WL.Validators.validateFunctionOrNull,WL.Validators.validateFunctionOrNull],arguments,"WL.EncryptedCache.write")}catch(e){console.log(e);if(d)return d(ERROR_INVALID_PARAMETER);throw ERROR_INVALID_PARAMETER;}if(void 0===a||""===a){console.log("Error: key is undefined or empty.");if(d)return d(ERROR_INVALID_PARAMETER);throw ERROR_INVALID_PARAMETER;}if(!this.DPK)return d(ERROR_EOC_CLOSED);
a=SHA256(a);if(null===b){if(localStorage.removeItem(this.STORAGE_PREFIX+a),c)return c(OK)}else{var g=[MD5(a+this[api.random]())],h=this.STORAGE_PREFIX;this.encryptor.rawEncrypt(Base64.encode(b),this.DPK,g[0],function(b){localStorage.setItem(h+a,b);c&&c(OK)},d)}};
...
}var a=SHA256(a),e=localStorage.getItem(this.STORAGE_PREFIX+a);if(null===e){if(b)return b(null)}else this.encryptor.rawDecrypt(e,this.DPK,function(a){a=Base64.decode(a);b&&b(a)},c)};function NativeEncryptor(){}
...
var WL_CHECKSUM = {"checksum":1815104570,"date":1367981951121,"machine":"Chris-PC"};
/* Date: Tue May 07 19:59:11 PDT 2013 */
...
/* JavaScript content from js/dropdown-menu.js in folder common */
/**
 * 
 *
 * 
