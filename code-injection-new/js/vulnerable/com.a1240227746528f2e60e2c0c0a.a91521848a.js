    		...
    		$(document).ready(function() {
                $("#shareButtonLink").click(function() {
                    window.plugins.share.show({
                        subject: "Share German Short Haired Pointer",
                        text: "https://play.google.com/store/apps/details?id=com.a1240227746528f2e60e2c0c0a.a91521848a"},
  ...
  return string;
        ...
        $(document).ready(function() {
                          $('#youtubevideos').youTubeChannel({ 
                                                             userName: 'German Short Haired Pointer', 
                                                             channel: "uploads", 
                                                             hideAuthor: true,
                                                             numberToDisplay: 30,
  ...
  return string;
    ...
    function onDeviceReadyQRCoupon() {
   	...
   	function dbCouponUsed(tx) {
   	    ...
   	    var url = "http://skybuilder.net/coupon.php?a=1240227746528f2e60e2c0c0.91521848&u=1&h="+dbCoupon.couponHash+"&la="+usedLat+"&lo="+usedLong;
   	    $.ajax({
            url: url,
            dataType: 'jsonp',
   	...
   	function getCurrentCoupons() {
   	    ...
   	    var url = "http://skybuilder.net/coupon.php?a=1240227746528f2e60e2c0c0.91521848";
   	                   // console.log(url);
   	    $.ajax({
            url: url,
            dataType: 'jsonp',
    ...
    function clickScan() {
   	...
   	function couponScanned() {
   	    var url = "http://skybuilder.net/coupon.php?a=1240227746528f2e60e2c0c0.91521848&s=1&h="+currentCoupon.couponHash;
   	    $.ajax({
            url: url,
            dataType: 'jsonp',
            success:  function(data) {
...
//------------------------------------------------------------
   	...
   	function onRequestFileSystemSuccess(fileSystem) { 
        ...
        entry.getDirectory("1240227746528f2e60e2c0c0.91521848", {create: true, exclusive: false}, onGetDirectorySuccess, onGetDirectoryFail); 
} 
  ...
  window.requestFileSystem(
    ...
    function onFileSystemSuccess(fileSystem) {
      fileSystem.root.getFile(
        "1240227746528f2e60e2c0c0.91521848.html", {create: true, exclusive: false}, 
        function gotFileEntry(fileEntry) {
          var sPath = fileEntry.fullPath.replace("1240227746528f2e60e2c0c0.91521848.html","1240227746528f2e60e2c0c0.91521848/");
  ...
  return string;
                ...
                function  rssReader() {
                   // alert("rssReader");
                    ...
                    //Title of the blog
                   //RSS url
                    var RSS = "http://api.flickr.com/services/feeds/photos_public.gne?tags=German+Short+Haired+Pointer&format=rss2";
                    //Stores entries
                    var entries = [];
                    var selectedEntry = "";
                    //alert("1");
...
function initializeData(){	
	...
	if (localStorage.getItem("disclaimer") == null || localStorage.getItem("disclaimer") != 1){
		//alert(disclaimerMessage + ", German Short Haired Pointer");
				
		document.addEventListener("deviceready", function(){
			navigator.notification.alert(disclaimerMessage, false, "German Short Haired Pointer", "OK");
...
function saveLocation(){
	...
	if(localStorage.getItem("acc") != null)
		navigator.notification.confirm(confirmOverwrite, onConfirmSave, "German Short Haired Pointer", "OK,Cancel");
	else
		onConfirmSave(1);
...
function checkAccuracy(start){
	...
	if (geoSuccess == false){
		navigator.notification.alert(noGPSMessage, false, "German Short Haired Pointer", "OK");
		...
		navigator.notification.alert(notifyPositionSaved, false, "German Short Haired Pointer", "OK");
	}
		...
		if (!allowReducedAcc && !gettingInput){
			...
			navigator.notification.confirm(allowReducedAccMessage, function(button){
				...
				if (allowReducedAcc){
					...
					gpsLoading.style.visibility = "hidden";
					//alert(errorPoorAcc);
					navigator.notification.alert(errorPoorAcc, false, "German Short Haired Pointer", "OK");
				}
				gettingInput = false;
			}, "German Short Haired Pointer", "OK,Cancel");
		}
...
function loadCanvas()
{
	...
	if(canvas.getContext("2d"))
	{
  	...
  	{
  		navigator.notification.alert(errorUnsupportedFeature, false, "German Short Haired Pointer", "OK");
  	}
   ...
   return tmp.textContent||tmp.innerText;
                ...
                function  rssReader() {
                   // alert("rssReader");
                    ...
                    //RSS url
                    var RSS = "http://news.google.com/news?q=German+Short+Haired+Pointer&output=rss&num=100";
...
alert ("Warning: it appears that you are not connected to the Internet. The content/offers shown may be out of date.");
  ...
  return string;
                ...
                $(document).ready(function() {
                    ...
                    //Listen for main page
                    $(document).on("pageinit", "#mainPage", function () {
                     var RSS="http://www.eventbrite.com/directoryxml/?q=German+Short+Haired+Pointer";
                                        //alert("3");
                                        //Set the title
