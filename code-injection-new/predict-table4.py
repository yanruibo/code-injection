...
def cv_crnn1(method, frequency, normalization, k_folds, log_tag):
    ...
    log_file = "log_cv_" + log_tag + "_" + method + "_frequency_" + str(frequency) + "_normalization_" \
        + str(normalization) + "_crnn_" + otherStyleTime + "_table4"
...
def cv_crnn2(method, frequency, normalization, k_folds, log_tag):
    ...
    for i in range(k_folds):
        ...
        model.add(Convolution1D(input_shape=(1, trainX.shape[2]), nb_filter=200, filter_length=3, border_mode='same', activation='sigmoid', subsample_length=1))
        #model.add(Convolution1D(nb_filter=50, filter_length=3, border_mode='same', activation='relu', subsample_length=1))
        model.add(Lambda(max_1d, output_shape=(200,)))
        ...
        #model.add(Reshape((1, 550)))
        #model.add(LSTM(output_dim=550))
        
        '''
        model.add(Convolution1D(nb_filter=128,filter_length=3,border_mode='same',activation='relu',subsample_length=1))
        model.add(Convolution1D(nb_filter=128,filter_length=3,border_mode='same',activation='relu',subsample_length=1))
...
def cv_crnn3(method, frequency, normalization, k_folds, log_tag):
...
def cv_crnn4(method, frequency, normalization, k_folds, log_tag):
...
if __name__ == '__main__':
    ...
    cv_crnn1(method="infogain_five_add", frequency=False, normalization=False, k_folds=5, log_tag="")
    cv_crnn2(method="infogain_five_add", frequency=False, normalization=False, k_folds=5, log_tag="")
    cv_crnn3(method="infogain_five_add", frequency=False, normalization=False, k_folds=5, log_tag="")
    cv_crnn4(method="infogain_five_add", frequency=False, normalization=False, k_folds=5, log_tag="")
    pass
